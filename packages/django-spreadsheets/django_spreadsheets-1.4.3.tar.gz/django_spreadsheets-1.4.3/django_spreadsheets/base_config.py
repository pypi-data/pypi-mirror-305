...
class SheetConfig(metaclass=IndexedAttributesMeta):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Replace functions found in formula1 and formula2 by their return values
        for column_name, column_config in self.items():
            validation_config = column_config.get("validation")
            if validation_config:
                formula1 = validation_config.get("formula1")
                if callable(formula1):
                    self[column_name]["validation"]["formula1"] = formula1()
                formula2 = validation_config.get("formula2")
                if callable(formula2):
                    self[column_name]["validation"]["formula2"] = formula2()
