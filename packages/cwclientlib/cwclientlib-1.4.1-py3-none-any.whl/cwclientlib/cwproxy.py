...
class CWProxy:
    ...
    def _csrf_token(self):
        cookies = self._requests_session.cookies
        try:
            return cookies.get("csrf_token")
        except CookieConflictError:
            domain = urlparse.urlparse(self._base_url).netloc
            try:
                return cookies.get("csrf_token", domain=domain)
            except CookieConflictError as originalError:
                paths = []
                for cookie in cookies.iter():
                    if cookie.name == "csrf_token" and cookie.domain == domain:
                        paths.append(str(cookie.path))
                error_message = str(originalError)
                if len(paths) >= 2:
                    error_message += (
                        "\nThere are multiples paths for CSRF token:\n"
                        + "\n".join(paths)
                    )
