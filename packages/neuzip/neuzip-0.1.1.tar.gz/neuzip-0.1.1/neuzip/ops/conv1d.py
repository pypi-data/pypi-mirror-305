...
class CompressedConv1D(torch.autograd.Function):
    @staticmethod
    ...
    def backward(ctx, grad_output):
        ...
        grad_inputs = (
            torch.mm(grad_output, _weight.t() if _p._conv1d else _weight).reshape(inputs_shape).to(ctx._inputs_dtype)
        )
        if not _p._conv1d:
            _weight = torch.mm(grad_output.t(), inputs)
        else:
            _weight = torch.mm(inputs.t(), grad_output)
