...
def get_current_weather(location: str) -> str:
    ...
    :param location: The name or coordinates of the location
    :returns: A string describing the current weather (e.g., 'Sunny, 25°C')
    """
    return "Sunny, 25°C"
...
def get_forecast(location: str, days: int) -> str:
    """
    Gets the weather forecast for a specific location for the next number of days.

    :param location: The name of the location
    :param days: The number of days to retrieve the forecast
    :returns: Weather forecast as a string
    """
    return ""


def get_temperature_in_celsius(location: str) -> float:
    """
    Retrieves the current temperature in Celsius for a given location.
...
def get_weather_alerts(location: str) -> list:
    """
    Fetches active weather alerts for the specified location.

    :param location: The name of the location
    :returns: A list of active weather alerts
    """
    return []


def is_rain_expected(location: str) -> bool:
    """
    Determines whether rain is expected in the specified location.

    :param location: The name of the location
    :returns: True if rain is expected, False otherwise
    """
    return False


def get_sunrise_sunset_times(location: str) -> dict:
    """
    Gets the sunrise and sunset times for a specific location.

    :param location: The name of the location
    :returns: A dictionary with 'sunrise' and 'sunset' times as strings
    """
    return {"sunrise": "", "sunset": ""}


toolkit = ChatToolKit([get_current_weather, get_forecast, get_sunrise_sunset_times,
                      get_temperature_in_celsius, get_weather_alerts, is_rain_expected])


engine = ChainEngine(toolkit=toolkit)
engine.chain(
    "in short tell me if donald trump has a son, his name, and then get the current weather in new york")
engine.chain(
    "what about hong kong and sweeden, check those too")
