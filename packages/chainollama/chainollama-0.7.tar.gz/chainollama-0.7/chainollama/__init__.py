...
class ChainEngine(ChatEngine):
    ...
    def init_values(self):
        ...
        self.before_chain_handle_callback = None
        self.after_chain_handle_callback = None
        self.default_printing = True
    ...
    def handle_response(self, mode: int, delta: str, text: str):
        if callable(self.before_chain_handle_callback):
            stop = self.before_chain_handle_callback(mode, delta, text)
            if stop == True:
                return True

        if mode == 0:
            if self.default_printing:
                print("[AI]:")
        elif mode == 1:
            if self.default_printing:
                print(delta, end="")
        ...
        if callable(self.after_chain_handle_callback):
            stop = self.after_chain_handle_callback(mode, delta, text)
            if stop == True:
                return True
