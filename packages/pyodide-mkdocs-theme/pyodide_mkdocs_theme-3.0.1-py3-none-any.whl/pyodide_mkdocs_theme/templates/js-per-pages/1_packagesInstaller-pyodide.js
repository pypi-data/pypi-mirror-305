...
const installPythonPackages=(function(){
  ...
  const asyncJsScriptLoader = (name, scriptOptions)=> async ()=>{
    let loaded = false

    const script = document.createElement('script')
    script.addEventListener("load", function(){ loaded=true })
    for(k in scriptOptions){
      if(k=='src') continue             // Always last...
      script[k] = scriptOptions[k]
    }
    script.src = scriptOptions.src      // Always last...

    document.body.appendChild(script)

    while(!loaded) await sleep(40)
