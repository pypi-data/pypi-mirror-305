...
def get_dbType(dbTypeN=None):
    return f"_{dbType}" if dbType else ''
def create_insert_list_file(dbName,dbType=None):
    return "{dbName}{get_dbType{dbType}}_table_config.json"
class envManager(metaclass=SingletonMeta):
    def __init__(self,dbName=None,dbType=None,env_path=None,tables_dir=None):
    ...
    def get_db_values(self,dbName,dbType=None,env_path=None):
        ...
        for key,value in keys.items():
            keys[key]=get_pure_env_value(key=get_db_key(dbName,dbType,key),path=get_env_path())
        return keys
