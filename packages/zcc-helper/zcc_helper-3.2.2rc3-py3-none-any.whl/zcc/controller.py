...
class ControlPoint:
    ...
    def __init__(
        ...
        self.device_mac = format(uuid.uuid4().node, "=012x")
    ...
    async def __get_devices(self):
        ...
        try:
            ...
            self.logger.debug("Failed to get properties - will retry")

        self.logger.debug("Getting initial device properties - second attempt")
        await self.socket.sendall(
            ControlPointProtocol.get("properties"), response_expected=False
        )
