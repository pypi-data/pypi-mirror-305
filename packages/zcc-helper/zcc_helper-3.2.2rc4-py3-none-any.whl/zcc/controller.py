...
class ControlPoint:
    ...
    async def __get_devices(self):
        ...
        while retries < ControlPoint.MAXIMUM_RETRIES:
            try:
                await asyncio.wait_for(
                ...
                self.logger.debug(
                    "ZCC connection failed - didn't receive any properties - will retry"
                )
                self.logger.debug("Getting initial device properties - retry")
                await self.socket.sendall(
                    ControlPointProtocol.get("properties"), response_expected=False
                )
                retries += 1
