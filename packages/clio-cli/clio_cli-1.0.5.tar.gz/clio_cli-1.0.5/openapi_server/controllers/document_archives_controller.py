from typing import List, Dict
from aiohttp import web

from openapi_server.models.document_archive_create_request import DocumentArchiveCreateRequest
from openapi_server.models.document_archive_show import DocumentArchiveShow
from openapi_server.models.error import Error
from openapi_server import util


async def document_archive_create(request: web.Request, x_api_version=None, fields=None, body=None) -> web.Response:
    """Create a new DocumentArchive

    Outlines the parameters and data fields used when creating a new DocumentArchive

    :param x_api_version: The [API minor version](#section/Minor-Versions). Default: latest version.
    :type x_api_version: str
    :param fields: The fields to be returned. See response samples for what fields are available. For more information see the [fields section](#section/Fields).
    :type fields: str
    :param body: Request Body for Document Archives
    :type body: dict | bytes

    """
    body = DocumentArchiveCreateRequest.from_dict(body)
    return web.Response(status=200)


async def document_archive_download(request: web.Request, id) -> web.Response:
    """Download the DocumentArchive

    Download the DocumentArchive

    :param id: The unique identifier for the DocumentArchive.
    :type id: int

    """
    return web.Response(status=200)


async def document_archive_show(request: web.Request, id, if_modified_since=None, if_none_match=None, x_api_version=None, fields=None) -> web.Response:
    """Return the data for a single DocumentArchive

    Outlines the parameters, optional and required, used when requesting the data for a single DocumentArchive

    :param id: The unique identifier for the DocumentArchive.
    :type id: int
    :param if_modified_since: The server will send the requested resource with a 200 status, but only if it has been modified after the given date. (Expects an RFC 2822 timestamp).
    :type if_modified_since: str
    :param if_none_match: The server will send the requested resource with a 200 status, but only if the existing resource&#39;s [ETag](#section/ETags) doesn&#39;t match any of the values listed.
    :type if_none_match: str
    :param x_api_version: The [API minor version](#section/Minor-Versions). Default: latest version.
    :type x_api_version: str
    :param fields: The fields to be returned. See response samples for what fields are available. For more information see the [fields section](#section/Fields).
    :type fields: str

    """
    if_modified_since = util.deserialize_date(if_modified_since)
    return web.Response(status=200)
