...
async def test_group_create(client):
    ...
    Create a new Group
    """
    body = openapi_server.GroupCreateRequest()
...
async def test_group_destroy(client):
    ...
    response = await client.request(
        method='DELETE',
        path='/api/v4/groups/{id_jso}'.format(id=56),
        headers=headers,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
...
async def test_group_index(client):
...
async def test_group_show(client):
