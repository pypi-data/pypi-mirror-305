...
class BillUpdateRequestDataDiscount(BaseModel):
    """
    BillUpdateRequestDataDiscount
    """
    rate: Optional[Union[StrictFloat, StrictInt]] = Field(default=None, description="Discount amount for the Bill. This can either be a percentage or monetary value, this is determined by the `discount[type]`.")
    type: Optional[StrictStr] = Field(default=None, description="The type of discount you are applying to your Bill with the `discount[rate]`.")
    note: Optional[StrictStr] = Field(default=None, description="A note for your Bill's discount.")
    __properties = ["rate", "type", "note"]
    ...
    def type_validate_enum(cls, value):
        ...
        if value not in ('percentage', 'money'):
            raise ValueError("must be one of enum values ('percentage', 'money')")
        return value
    ...
    @classmethod
    def from_json(cls, json_str: str) -> BillUpdateRequestDataDiscount:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> BillUpdateRequestDataDiscount:
        """Create an instance of BillUpdateRequestDataDiscount from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return BillUpdateRequestDataDiscount.parse_obj(obj)

        _obj = BillUpdateRequestDataDiscount.parse_obj({
            "rate": obj.get("rate"),
            "type": obj.get("type"),
            "note": obj.get("note")
        })
