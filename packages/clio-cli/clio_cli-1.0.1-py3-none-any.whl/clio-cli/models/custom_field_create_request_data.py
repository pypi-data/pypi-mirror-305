...
class CustomFieldCreateRequestData(BaseModel):
    ...
    displayed: Optional[StrictBool] = Field(default=True, description="Whether or not the CustomField should be displayed by default.")
    field_type: StrictStr = Field(default=..., description="Field type of the CustomField.")
    name: StrictStr = Field(default=..., description="CustomField name.")
    parent_type: StrictStr = Field(default=..., description="Type of object the CustomField is for.")
    picklist_options: Optional[conlist(CustomFieldCreateRequestDataPicklistOptionsInner)] = None
    required: Optional[StrictBool] = Field(default=False, description="Whether or not the CustomField should require a value.")
    __properties = ["display_order", "displayed", "field_type", "name", "parent_type", "picklist_options", "required"]
    ...
    def parent_type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in ('Contact', 'Matter'):
            raise ValueError("must be one of enum values ('Contact', 'Matter')")
        return value
    ...
    @classmethod
    def from_json(cls, json_str: str) -> CustomFieldCreateRequestData:
    ...
    def to_dict(self):
        ...
        _dict = self.dict(by_alias=True,
                          ...
                          exclude_none=True)
        # override the default output from pydantic by calling `to_dict()` of each item in picklist_options (list)
        _items = []
        if self.picklist_options:
            for _item in self.picklist_options:
                if _item:
                    _items.append(_item.to_dict())
            _dict['picklist_options'] = _items
        return _dict

    @classmethod
    def from_dict(cls, obj: dict) -> CustomFieldCreateRequestData:
        """Create an instance of CustomFieldCreateRequestData from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return CustomFieldCreateRequestData.parse_obj(obj)

        _obj = CustomFieldCreateRequestData.parse_obj({
            "display_order": obj.get("display_order"),
            "displayed": obj.get("displayed") if obj.get("displayed") is not None else True,
            "field_type": obj.get("field_type"),
            "name": obj.get("name"),
            "parent_type": obj.get("parent_type"),
            "picklist_options": [CustomFieldCreateRequestDataPicklistOptionsInner.from_dict(_item) for _item in obj.get("picklist_options")] if obj.get("picklist_options") is not None else None,
