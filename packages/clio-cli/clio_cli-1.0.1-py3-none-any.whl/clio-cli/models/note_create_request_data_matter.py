...
class NoteCreateRequestDataMatter(BaseModel):
    """
    NoteCreateRequestDataMatter
    """
    id: StrictInt = Field(default=..., description="The unique identifier for a single Matter associated with the Note. The keyword null is not valid for this field. Required only if the Note type is `\"Matter\"`.")
    __properties = ["id"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> NoteCreateRequestDataMatter:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> NoteCreateRequestDataMatter:
        """Create an instance of NoteCreateRequestDataMatter from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return NoteCreateRequestDataMatter.parse_obj(obj)

        _obj = NoteCreateRequestDataMatter.parse_obj({
            "id": obj.get("id")
        })
        return _obj
