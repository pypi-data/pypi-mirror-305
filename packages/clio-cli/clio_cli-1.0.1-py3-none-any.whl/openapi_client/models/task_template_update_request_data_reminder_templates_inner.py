...
class TaskTemplateUpdateRequestDataReminderTemplatesInner(BaseModel):
    ...
    notification_type: Optional[StrictStr] = Field(default=None, description="Notification method.")
    id: Optional[StrictInt] = Field(default=None, description="The unique identifier for a single ReminderTemplate associated with the TaskTemplate. The keyword `null` is not valid for this field.")
    destroy: Optional[StrictBool] = Field(default=None, alias="_destroy", description="The destroy flag. If the flag is set to `true` and the unique identifier of the associated ReminderTemplate is present, the ReminderTemplate is deleted from the TaskTemplate.")
    __properties = ["duration_value", "duration_unit", "notification_type", "id", "_destroy"]
    ...
    def notification_type_validate_enum(cls, value):
        ...
        if value not in ('Email', 'Popup'):
            raise ValueError("must be one of enum values ('Email', 'Popup')")
        return value
    ...
    @classmethod
    def from_json(cls, json_str: str) -> TaskTemplateUpdateRequestDataReminderTemplatesInner:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> TaskTemplateUpdateRequestDataReminderTemplatesInner:
        """Create an instance of TaskTemplateUpdateRequestDataReminderTemplatesInner from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return TaskTemplateUpdateRequestDataReminderTemplatesInner.parse_obj(obj)

        _obj = TaskTemplateUpdateRequestDataReminderTemplatesInner.parse_obj({
