...
class MyEventUpdateRequestData(BaseModel):
    """
    MyEventUpdateRequestData
    """
    marked_as_read: Optional[StrictBool] = Field(default=None, description="Indicates whether the event notification should be marked as read by the current user. If `false` the event status is reset to unread.")
    __properties = ["marked_as_read"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> MyEventUpdateRequestData:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> MyEventUpdateRequestData:
        """Create an instance of MyEventUpdateRequestData from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return MyEventUpdateRequestData.parse_obj(obj)

        _obj = MyEventUpdateRequestData.parse_obj({
            "marked_as_read": obj.get("marked_as_read")
        })
        return _obj
