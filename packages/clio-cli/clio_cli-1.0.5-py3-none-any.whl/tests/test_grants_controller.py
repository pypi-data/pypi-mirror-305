...
async def test_grant_create(client):
    """Test case for grant_create

    Create a new grant
    """
    body = openapi_server.GrantCreateRequest()
...
async def test_grant_destroy(client):
    ...
    response = await client.request(
        method='DELETE',
        path='/api/v4/grants/{id_jso}'.format(id=56),
        headers=headers,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
...
async def test_grant_index(client):
...
async def test_grant_show(client):
...
@pytest.mark.skip("Connexion does not support multiple consumes. See https://github.com/zalando/connexion/pull/760")
async def test_grant_update(client):
