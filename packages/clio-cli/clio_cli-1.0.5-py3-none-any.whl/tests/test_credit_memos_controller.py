...
async def test_credit_memo_index(client):
...
async def test_credit_memo_show(client):
    ...
    response = await client.request(
        method='GET',
        path='/api/v4/credit_memos/{id_jso}'.format(id=56),
        headers=headers,
        params=params,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
