...
class ClioPaymentsLinkCreateRequestDataSubject(BaseModel):
    """
    ClioPaymentsLinkCreateRequestDataSubject
    """
    id: StrictInt = Field(default=..., description="The ID of the record associated with the payment link.")
    type: StrictStr = Field(default=..., description="The type of the record associated with the payment link. The type of record determines the behavior of the payment link.  Type of subject object: * 'BankAccount' results in a payment link for a direct payment. * 'Bill' results in a payment link for paying an existing invoice or trust request. * 'Contact' results in a payment link for paying the outstanding invoices of a contact. ")
    __properties = ["id", "type"]

    @validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in ('BankAccount', 'Bill', 'Contact'):
            raise ValueError("must be one of enum values ('BankAccount', 'Bill', 'Contact')")
        return value
    ...
    @classmethod
    def from_json(cls, json_str: str) -> ClioPaymentsLinkCreateRequestDataSubject:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> ClioPaymentsLinkCreateRequestDataSubject:
        """Create an instance of ClioPaymentsLinkCreateRequestDataSubject from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return ClioPaymentsLinkCreateRequestDataSubject.parse_obj(obj)

        _obj = ClioPaymentsLinkCreateRequestDataSubject.parse_obj({
            "id": obj.get("id"),
            "type": obj.get("type")
        })
        return _obj
