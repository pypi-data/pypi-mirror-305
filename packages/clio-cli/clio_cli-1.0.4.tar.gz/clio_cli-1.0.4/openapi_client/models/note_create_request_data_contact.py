...
class NoteCreateRequestDataContact(BaseModel):
    """
    NoteCreateRequestDataContact
    """
    id: StrictInt = Field(default=..., description="The unique identifier for a single Contact associated with the Note. The keyword null is not valid for this field. Required only if the Note type is `\"Contact\"`.")
    __properties = ["id"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> NoteCreateRequestDataContact:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> NoteCreateRequestDataContact:
        """Create an instance of NoteCreateRequestDataContact from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return NoteCreateRequestDataContact.parse_obj(obj)

        _obj = NoteCreateRequestDataContact.parse_obj({
            "id": obj.get("id")
        })
        return _obj
