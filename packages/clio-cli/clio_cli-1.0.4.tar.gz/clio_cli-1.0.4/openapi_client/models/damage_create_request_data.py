...
class DamageCreateRequestData(BaseModel):
    """
    DamageCreateRequestData
    """
    amount: Union[StrictFloat, StrictInt] = Field(default=..., description="Amount")
    damage_type: StrictStr = Field(default=..., description="Damage type")
    description: StrictStr = Field(default=..., description="Description")
    matter_id: StrictInt = Field(default=..., description="The unique identifier of the Matter to which the Damage is associated.")
    __properties = ["amount", "damage_type", "description", "matter_id"]

    @validator('damage_type')
    def damage_type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in ('special', 'general', 'other'):
            raise ValueError("must be one of enum values ('special', 'general', 'other')")
        return value
    ...
    @classmethod
    def from_json(cls, json_str: str) -> DamageCreateRequestData:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> DamageCreateRequestData:
        """Create an instance of DamageCreateRequestData from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return DamageCreateRequestData.parse_obj(obj)

        _obj = DamageCreateRequestData.parse_obj({
