...
class WebhookCreateRequestData(BaseModel):
    ...
    """
    events: Optional[conlist(StrictStr)] = Field(default=None, description="The events your webhook is subscribed to.")
    ...
    fields: StrictStr = Field(default=..., description="Fields to be included in the Webhook request.")
    model: StrictStr = Field(default=..., description="What model the Webhook is for. This field accepts either [the string identifier of the model or its ID](#section/Supported-Models)")
    url: StrictStr = Field(default=..., description="The URL of where to POST the Webhook. Note that only URLs using the `https` protocol will be accepted.")
    __properties = ["events", "expires_at", "fields", "model", "url"]
    ...
    def events_validate_enum(cls, value):
        ...
        for i in value:
            if i not in ('created', 'updated', 'deleted', 'matter_opened', 'matter_pended', 'matter_closed'):
                raise ValueError("each list item must be one of ('created', 'updated', 'deleted', 'matter_opened', 'matter_pended', 'matter_closed')")
        return value

    @validator('model')
    def model_validate_enum(cls, value):
        """Validates the enum"""
        if value not in ('activity', 'bill', 'calendar_entry', 'clio_payments_payment', 'communication', 'contact', 'document', 'folder', 'matter', 'task'):
            raise ValueError("must be one of enum values ('activity', 'bill', 'calendar_entry', 'clio_payments_payment', 'communication', 'contact', 'document', 'folder', 'matter', 'task')")
        return value
    ...
    @classmethod
    def from_json(cls, json_str: str) -> WebhookCreateRequestData:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> WebhookCreateRequestData:
        """Create an instance of WebhookCreateRequestData from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return WebhookCreateRequestData.parse_obj(obj)

        _obj = WebhookCreateRequestData.parse_obj({
