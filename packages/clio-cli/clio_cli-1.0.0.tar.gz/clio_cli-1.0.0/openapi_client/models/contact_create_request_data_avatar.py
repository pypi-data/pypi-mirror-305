...
class ContactCreateRequestDataAvatar(BaseModel):
    """
    ContactCreateRequestDataAvatar
    """
    image: Optional[StrictStr] = Field(default=None, description="The avatar image for the *Contact*, base64-encoded. Must be a valid GIF, JPG, or PNG image of less than 2 megabytes in size.")
    destroy: Optional[StrictBool] = Field(default=None, alias="_destroy", description="The destroy flag. If the flag is set to `true` and the unique identifier of the associated Avatar is present, the Avatar is deleted from the Contact.")
    __properties = ["image", "_destroy"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> ContactCreateRequestDataAvatar:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> ContactCreateRequestDataAvatar:
        """Create an instance of ContactCreateRequestDataAvatar from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return ContactCreateRequestDataAvatar.parse_obj(obj)

        _obj = ContactCreateRequestDataAvatar.parse_obj({
            "image": obj.get("image"),
