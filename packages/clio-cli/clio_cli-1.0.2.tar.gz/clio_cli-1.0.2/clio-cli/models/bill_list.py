...
class BillList(BaseModel):
    ...
    BillList
    """
    data: conlist(Bill) = Field(default=..., description="Bill List Response")
    __properties = ["data"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> BillList:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> BillList:
        """Create an instance of BillList from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return BillList.parse_obj(obj)

        _obj = BillList.parse_obj({
            "data": [Bill.from_dict(_item) for _item in obj.get("data")] if obj.get("data") is not None else None
        })
        return _obj
