...
class DocumentAutomationCreateRequestData(BaseModel):
    ...
    """
    document_template: DocumentAutomationCreateRequestDataDocumentTemplate = Field(...)
    filename: StrictStr = Field(default=..., description="The filename the generated document should have.")
    formats: conlist(StrictStr) = Field(default=..., description="The formats the document should be generated as. It can either be generated as a PDF and/or in whatever type the document template is.")
    matter: DocumentAutomationCreateRequestDataMatter = Field(...)
    __properties = ["document_template", "filename", "formats", "matter"]

    @validator('formats')
    def formats_validate_enum(cls, value):
        """Validates the enum"""
        for i in value:
            if i not in ('pdf', 'original'):
                raise ValueError("each list item must be one of ('pdf', 'original')")
        return value
    ...
    @classmethod
    def from_json(cls, json_str: str) -> DocumentAutomationCreateRequestData:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> DocumentAutomationCreateRequestData:
        """Create an instance of DocumentAutomationCreateRequestData from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return DocumentAutomationCreateRequestData.parse_obj(obj)

        _obj = DocumentAutomationCreateRequestData.parse_obj({
            "document_template": DocumentAutomationCreateRequestDataDocumentTemplate.from_dict(obj.get("document_template")) if obj.get("document_template") is not None else None,
            "filename": obj.get("filename"),
            "formats": obj.get("formats"),
            "matter": DocumentAutomationCreateRequestDataMatter.from_dict(obj.get("matter")) if obj.get("matter") is not None else None
        })
        return _obj
