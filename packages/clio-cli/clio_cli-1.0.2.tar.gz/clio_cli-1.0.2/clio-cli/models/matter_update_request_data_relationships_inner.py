...
class MatterUpdateRequestDataRelationshipsInner(BaseModel):
    """
    MatterUpdateRequestDataRelationshipsInner
    """
    description: Optional[StrictStr] = Field(default=None, description="Describe the relationship between a Contact and a Matter.")
    contact: Optional[MatterUpdateRequestDataRelationshipsInnerContact] = None
    id: Optional[StrictInt] = Field(default=None, description="The unique identifier for a single Relationship associated with the Matter. The keyword `null` is not valid for this field.")
    destroy: Optional[StrictBool] = Field(default=None, alias="_destroy", description="The destroy flag. If the flag is set to `true` and the unique identifier of the associated Relationship is present, the Relationship is deleted from the Matter.")
    __properties = ["description", "contact", "id", "_destroy"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> MatterUpdateRequestDataRelationshipsInner:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> MatterUpdateRequestDataRelationshipsInner:
        """Create an instance of MatterUpdateRequestDataRelationshipsInner from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return MatterUpdateRequestDataRelationshipsInner.parse_obj(obj)

        _obj = MatterUpdateRequestDataRelationshipsInner.parse_obj({
            "description": obj.get("description"),
            "contact": MatterUpdateRequestDataRelationshipsInnerContact.from_dict(obj.get("contact")) if obj.get("contact") is not None else None,
            "id": obj.get("id"),
