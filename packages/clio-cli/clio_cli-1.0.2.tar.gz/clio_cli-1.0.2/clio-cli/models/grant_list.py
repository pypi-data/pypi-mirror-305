...
class GrantList(BaseModel):
    ...
    GrantList
    """
    data: conlist(Grant) = Field(default=..., description="Grant List Response")
    __properties = ["data"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> GrantList:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> GrantList:
        """Create an instance of GrantList from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return GrantList.parse_obj(obj)

        _obj = GrantList.parse_obj({
            "data": [Grant.from_dict(_item) for _item in obj.get("data")] if obj.get("data") is not None else None
        })
        return _obj
