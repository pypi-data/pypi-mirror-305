...
class MatterList(BaseModel):
    ...
    MatterList
    """
    data: conlist(Matter) = Field(default=..., description="Matter List Response")
    __properties = ["data"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> MatterList:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> MatterList:
        """Create an instance of MatterList from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return MatterList.parse_obj(obj)

        _obj = MatterList.parse_obj({
            "data": [Matter.from_dict(_item) for _item in obj.get("data")] if obj.get("data") is not None else None
        })
        return _obj
