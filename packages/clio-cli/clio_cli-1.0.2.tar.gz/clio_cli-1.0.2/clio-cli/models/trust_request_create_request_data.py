...
class TrustRequestCreateRequestData(BaseModel):
    ...
    """
    approved: StrictBool = Field(default=..., description="Whether or not the TrustRequest should be automatically approved.")
    client_id: StrictInt = Field(default=..., description="The client_id associated to the TrustRequest")
    due_date: date = Field(default=..., description="The date the TrustRequest is due (Expects an ISO-8601 date).")
    issue_date: date = Field(default=..., description="The date the TrustRequest is issued (Expects an ISO-8601 date).")
    matter: Optional[conlist(TrustRequestCreateRequestDataMatterInner)] = None
    note: Optional[StrictStr] = Field(default=None, description="The client level TrustRequest note")
    trust_amount: Union[StrictFloat, StrictInt] = Field(default=..., description="The TrustRequest's amount")
    trust_type: StrictStr = Field(default=..., description="The type of TrustRequest")
    __properties = ["approved", "client_id", "due_date", "issue_date", "matter", "note", "trust_amount", "trust_type"]

    @validator('trust_type')
    ...
    @classmethod
    def from_json(cls, json_str: str) -> TrustRequestCreateRequestData:
    ...
    def to_dict(self):
        ...
        _dict = self.dict(by_alias=True,
                          ...
                          exclude_none=True)
        # override the default output from pydantic by calling `to_dict()` of each item in matter (list)
        _items = []
        if self.matter:
            for _item in self.matter:
                if _item:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> TrustRequestCreateRequestData:
        """Create an instance of TrustRequestCreateRequestData from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return TrustRequestCreateRequestData.parse_obj(obj)

        _obj = TrustRequestCreateRequestData.parse_obj({
            "approved": obj.get("approved"),
            "client_id": obj.get("client_id"),
            "due_date": obj.get("due_date"),
            "issue_date": obj.get("issue_date"),
            "matter": [TrustRequestCreateRequestDataMatterInner.from_dict(_item) for _item in obj.get("matter")] if obj.get("matter") is not None else None,
            "note": obj.get("note"),
            "trust_amount": obj.get("trust_amount"),
            "trust_type": obj.get("trust_type")
        })
        return _obj
