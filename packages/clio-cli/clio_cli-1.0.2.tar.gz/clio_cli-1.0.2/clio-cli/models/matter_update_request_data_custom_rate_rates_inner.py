...
class MatterUpdateRequestDataCustomRateRatesInner(BaseModel):
    """
    MatterUpdateRequestDataCustomRateRatesInner
    """
    user: Optional[MatterUpdateRequestDataCustomRateRatesInnerUser] = None
    ...
    var_date: Optional[date] = Field(default=None, alias="date", description="The date the rate is for. Only valid for ContingencyFee. (Expects an ISO-8601 date).")
    rate: Optional[Union[StrictFloat, StrictInt]] = Field(default=None, description="If `type` is `HourlyRate`, it is the dollar amount of the custom rate of the User or Group for the Matter.  If `type` is `FlatRate`, it is the dollar amount of the custom flat rate for the Matter.  If `type` is `ContingencyFee`, it is the percentage of the contingency fee awarded to the user for the Matter. ")
    id: Optional[StrictInt] = Field(default=None, description="The unique identifier for a single Rate associated with the Matter. The keyword `null` is not valid for this field.")
    destroy: Optional[StrictBool] = Field(default=None, alias="_destroy", description="The destroy flag. If the flag is set to `true` and the unique identifier of the associated Rate is present, the Rate is deleted from the Matter.")
    ...
    __properties = ["user", "award", "note", "date", "rate", "id", "_destroy", "activity_description", "group"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> MatterUpdateRequestDataCustomRateRatesInner:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> MatterUpdateRequestDataCustomRateRatesInner:
        """Create an instance of MatterUpdateRequestDataCustomRateRatesInner from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return MatterUpdateRequestDataCustomRateRatesInner.parse_obj(obj)

        _obj = MatterUpdateRequestDataCustomRateRatesInner.parse_obj({
            "user": MatterUpdateRequestDataCustomRateRatesInnerUser.from_dict(obj.get("user")) if obj.get("user") is not None else None,
            "award": obj.get("award"),
            "note": obj.get("note"),
            "var_date": obj.get("date"),
            "rate": obj.get("rate"),
            "id": obj.get("id"),
            "destroy": obj.get("_destroy"),
            "activity_description": MatterCreateRequestDataCustomRateRatesInnerActivityDescription.from_dict(obj.get("activity_description")) if obj.get("activity_description") is not None else None,
            "group": MatterCreateRequestDataCustomRateRatesInnerGroup.from_dict(obj.get("group")) if obj.get("group") is not None else None
        })
        return _obj
