...
class DocumentAutomation(BaseModel):
    ...
    __properties = ["id", "etag", "state", "export_formats", "filename", "created_at", "updated_at", "matter", "document_template", "documents"]
    ...
    def state_validate_enum(cls, value):
        ...
        if value not in ('not_started', 'queued', 'in_progress', 'empty', 'failed', 'completed'):
            raise ValueError("must be one of enum values ('not_started', 'queued', 'in_progress', 'empty', 'failed', 'completed')")
        return value

    @validator('export_formats')
    def export_formats_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value

        if value not in ('pdf', 'original'):
            raise ValueError("must be one of enum values ('pdf', 'original')")
        return value
    ...
    @classmethod
    def from_json(cls, json_str: str) -> DocumentAutomation:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> DocumentAutomation:
        """Create an instance of DocumentAutomation from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return DocumentAutomation.parse_obj(obj)

        _obj = DocumentAutomation.parse_obj({
            "id": obj.get("id"),
            "etag": obj.get("etag"),
            "state": obj.get("state"),
            ...
            "document_template": DocumentTemplateBase.from_dict(obj.get("document_template")) if obj.get("document_template") is not None else None,
