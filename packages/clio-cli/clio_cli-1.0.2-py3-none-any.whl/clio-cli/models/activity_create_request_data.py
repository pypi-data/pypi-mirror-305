...
class ActivityCreateRequestData(BaseModel):
    ...
    var_date: date = Field(default=..., alias="date", description="The date the Activity was performed. (Expects an ISO-8601 date).")
    ...
    type: StrictStr = Field(default=..., description="The type of the Activity.")
    ...
    __properties = ["activity_description", "calendar_entry", "client_portal", "communication", "contact_note", "date", "expense_category", "matter", "matter_note", "no_charge", "non_billable", "note", "price", "quantity", "reference", "start_timer", "task", "tax_setting", "text_message_conversation", "type", "user", "utbms_expense", "vendor"]

    @validator('tax_setting')
    def tax_setting_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value

        if value not in ('no_tax', 'tax_1_only', 'tax_2_only', 'tax_1_and_tax_2'):
    ...
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in ('TimeEntry', 'ExpenseEntry', 'HardCostEntry', 'SoftCostEntry'):
    ...
    @classmethod
    def from_json(cls, json_str: str) -> ActivityCreateRequestData:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> ActivityCreateRequestData:
        """Create an instance of ActivityCreateRequestData from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return ActivityCreateRequestData.parse_obj(obj)

        _obj = ActivityCreateRequestData.parse_obj({
            "activity_description": ActivityCreateRequestDataActivityDescription.from_dict(obj.get("activity_description")) if obj.get("activity_description") is not None else None,
            "calendar_entry": ActivityCreateRequestDataCalendarEntry.from_dict(obj.get("calendar_entry")) if obj.get("calendar_entry") is not None else None,
            "client_portal": ActivityCreateRequestDataClientPortal.from_dict(obj.get("client_portal")) if obj.get("client_portal") is not None else None,
            "communication": ActivityCreateRequestDataCommunication.from_dict(obj.get("communication")) if obj.get("communication") is not None else None,
            "contact_note": ActivityCreateRequestDataContactNote.from_dict(obj.get("contact_note")) if obj.get("contact_note") is not None else None,
            "var_date": obj.get("date"),
            "expense_category": ActivityCreateRequestDataExpenseCategory.from_dict(obj.get("expense_category")) if obj.get("expense_category") is not None else None,
            "matter": ActivityCreateRequestDataMatter.from_dict(obj.get("matter")) if obj.get("matter") is not None else None,
            "matter_note": ActivityCreateRequestDataContactNote.from_dict(obj.get("matter_note")) if obj.get("matter_note") is not None else None,
            ...
            "task": ActivityCreateRequestDataTask.from_dict(obj.get("task")) if obj.get("task") is not None else None,
            "tax_setting": obj.get("tax_setting"),
            "text_message_conversation": ActivityCreateRequestDataTextMessageConversation.from_dict(obj.get("text_message_conversation")) if obj.get("text_message_conversation") is not None else None,
            "type": obj.get("type"),
            "user": ActivityCreateRequestDataUser.from_dict(obj.get("user")) if obj.get("user") is not None else None,
            "utbms_expense": ActivityCreateRequestDataUtbmsExpense.from_dict(obj.get("utbms_expense")) if obj.get("utbms_expense") is not None else None,
            "vendor": ActivityCreateRequestDataVendor.from_dict(obj.get("vendor")) if obj.get("vendor") is not None else None
        })
        return _obj
