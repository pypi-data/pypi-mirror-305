...
class MatterUpdateRequestDataStatuteOfLimitations(BaseModel):
    """
    MatterUpdateRequestDataStatuteOfLimitations
    """
    status: Optional[StrictStr] = Field(default=None, description="The task status of Statue of Limitations. Users without advanced tasks are allowed to select `Complete' or `Pending` only.")
    due_at: Optional[date] = Field(default=None, description="The due date of Statute of Limitations. (Expects an ISO-8601 date).")
    reminders: Optional[conlist(MatterUpdateRequestDataStatuteOfLimitationsRemindersInner)] = None
    __properties = ["status", "due_at", "reminders"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> MatterUpdateRequestDataStatuteOfLimitations:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> MatterUpdateRequestDataStatuteOfLimitations:
        """Create an instance of MatterUpdateRequestDataStatuteOfLimitations from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return MatterUpdateRequestDataStatuteOfLimitations.parse_obj(obj)

        _obj = MatterUpdateRequestDataStatuteOfLimitations.parse_obj({
            "status": obj.get("status"),
            "due_at": obj.get("due_at"),
            "reminders": [MatterUpdateRequestDataStatuteOfLimitationsRemindersInner.from_dict(_item) for _item in obj.get("reminders")] if obj.get("reminders") is not None else None
        })
        return _obj
