...
class ConversationMessageCreateRequestDataReceiversInner(BaseModel):
    """
    ConversationMessageCreateRequestDataReceiversInner
    """
    id: StrictInt = Field(default=..., description="The unique identifier for a single receiver for this ConversationMessage.")
    type: StrictStr = Field(default=..., description="The type for a single receiver for this ConversationMessage, could be `Contact` or `User`.")
    __properties = ["id", "type"]

    @validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in ('Contact', 'User'):
            raise ValueError("must be one of enum values ('Contact', 'User')")
        return value
    ...
    @classmethod
    def from_json(cls, json_str: str) -> ConversationMessageCreateRequestDataReceiversInner:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> ConversationMessageCreateRequestDataReceiversInner:
        """Create an instance of ConversationMessageCreateRequestDataReceiversInner from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return ConversationMessageCreateRequestDataReceiversInner.parse_obj(obj)

        _obj = ConversationMessageCreateRequestDataReceiversInner.parse_obj({
            "id": obj.get("id"),
            "type": obj.get("type")
        })
        return _obj
