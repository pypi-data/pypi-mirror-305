...
class MatterUpdateRequestDataCustomRate(BaseModel):
    """
    MatterUpdateRequestDataCustomRate
    """
    type: Optional[StrictStr] = Field(default=None, description="The type of custom rate for the Matter.")
    rates: Optional[conlist(MatterUpdateRequestDataCustomRateRatesInner)] = None
    __properties = ["type", "rates"]
    ...
    def type_validate_enum(cls, value):
        ...
        if value not in ('FlatRate', 'HourlyRate', 'ContingencyFee'):
            raise ValueError("must be one of enum values ('FlatRate', 'HourlyRate', 'ContingencyFee')")
        return value
    ...
    @classmethod
    def from_json(cls, json_str: str) -> MatterUpdateRequestDataCustomRate:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> MatterUpdateRequestDataCustomRate:
        """Create an instance of MatterUpdateRequestDataCustomRate from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return MatterUpdateRequestDataCustomRate.parse_obj(obj)

        _obj = MatterUpdateRequestDataCustomRate.parse_obj({
            "type": obj.get("type"),
            "rates": [MatterUpdateRequestDataCustomRateRatesInner.from_dict(_item) for _item in obj.get("rates")] if obj.get("rates") is not None else None
        })
        return _obj
