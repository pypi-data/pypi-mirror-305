...
class CommunicationUpdateRequestDataExternalPropertiesInner(BaseModel):
    """
    CommunicationUpdateRequestDataExternalPropertiesInner
    """
    id: Optional[StrictInt] = Field(default=None, description="The unique identifier for a single ExternalProperty associated with the Communication. The keyword `null` is not valid for this field.")
    name: Optional[StrictStr] = Field(default=None, description="The ExternalProperty name. Note: **there is a limit of 5 external_properties per Communication**")
    value: Optional[StrictStr] = Field(default=None, description="The ExternalProperty value.")
    destroy: Optional[StrictBool] = Field(default=None, alias="_destroy", description="The destroy flag. If the flag is set to `true` and the unique identifier of the associated ExternalProperty is present, the ExternalProperty is deleted from the Communication.")
    __properties = ["id", "name", "value", "_destroy"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> CommunicationUpdateRequestDataExternalPropertiesInner:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> CommunicationUpdateRequestDataExternalPropertiesInner:
        """Create an instance of CommunicationUpdateRequestDataExternalPropertiesInner from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return CommunicationUpdateRequestDataExternalPropertiesInner.parse_obj(obj)

        _obj = CommunicationUpdateRequestDataExternalPropertiesInner.parse_obj({
            "id": obj.get("id"),
            "name": obj.get("name"),
            "value": obj.get("value"),
