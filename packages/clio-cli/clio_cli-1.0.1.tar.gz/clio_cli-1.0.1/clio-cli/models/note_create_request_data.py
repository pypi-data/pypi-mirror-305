...
class NoteCreateRequestData(BaseModel):
    ...
    """
    contact: NoteCreateRequestDataContact = Field(...)
    var_date: Optional[date] = Field(default=None, alias="date", description="Date for the Note. (Expects an ISO-8601 date).")
    detail: Optional[StrictStr] = Field(default=None, description="Note body.")
    matter: NoteCreateRequestDataMatter = Field(...)
    notification_event_subscribers: Optional[conlist(NoteCreateRequestDataNotificationEventSubscribersInner)] = None
    subject: Optional[StrictStr] = Field(default=None, description="Note subject.")
    type: StrictStr = Field(default=..., description="Note type.")
    __properties = ["contact", "date", "detail", "matter", "notification_event_subscribers", "subject", "type"]

    @validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in ('Matter', 'Contact'):
            raise ValueError("must be one of enum values ('Matter', 'Contact')")
        return value
    ...
    @classmethod
    def from_json(cls, json_str: str) -> NoteCreateRequestData:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> NoteCreateRequestData:
        """Create an instance of NoteCreateRequestData from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return NoteCreateRequestData.parse_obj(obj)

        _obj = NoteCreateRequestData.parse_obj({
            "contact": NoteCreateRequestDataContact.from_dict(obj.get("contact")) if obj.get("contact") is not None else None,
            "var_date": obj.get("date"),
            "detail": obj.get("detail"),
            "matter": NoteCreateRequestDataMatter.from_dict(obj.get("matter")) if obj.get("matter") is not None else None,
            "notification_event_subscribers": [NoteCreateRequestDataNotificationEventSubscribersInner.from_dict(_item) for _item in obj.get("notification_event_subscribers")] if obj.get("notification_event_subscribers") is not None else None,
