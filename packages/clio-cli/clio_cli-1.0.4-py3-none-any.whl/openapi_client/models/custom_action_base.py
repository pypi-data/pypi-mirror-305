...
class CustomActionBase(BaseModel):
    ...
    CustomActionBase
    """
    id: Optional[StrictInt] = Field(default=None, description="Unique identifier for the *CustomAction*")
    etag: Optional[StrictStr] = Field(default=None, description="ETag for the *CustomAction*")
    created_at: Optional[datetime] = Field(default=None, description="The time the *CustomAction* was created (as a ISO-8601 timestamp)")
    updated_at: Optional[datetime] = Field(default=None, description="The time the *CustomAction* was last updated (as a ISO-8601 timestamp)")
    label: Optional[StrictStr] = Field(default=None, description="Text label to be displayed on the custom link.")
    target_url: Optional[StrictStr] = Field(default=None, description="Target URL which will be opened in a new tab when the user clicks the custom link.")
    ui_reference: Optional[StrictStr] = Field(default=None, description="UI reference location within Clio where the link will be displayed.")
    __properties = ["id", "etag", "created_at", "updated_at", "label", "target_url", "ui_reference"]

    @validator('ui_reference')
    def ui_reference_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value

        if value not in ('activities/show', 'documents/show', 'contacts/show', 'matters/show', 'folders/show'):
            raise ValueError("must be one of enum values ('activities/show', 'documents/show', 'contacts/show', 'matters/show', 'folders/show')")
        return value
    ...
    @classmethod
    def from_json(cls, json_str: str) -> CustomActionBase:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> CustomActionBase:
        """Create an instance of CustomActionBase from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return CustomActionBase.parse_obj(obj)

        _obj = CustomActionBase.parse_obj({
            "id": obj.get("id"),
            "etag": obj.get("etag"),
            ...
            "ui_reference": obj.get("ui_reference")
        })
        return _obj
