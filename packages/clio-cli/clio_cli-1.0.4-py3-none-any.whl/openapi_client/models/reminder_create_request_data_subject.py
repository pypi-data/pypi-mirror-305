...
class ReminderCreateRequestDataSubject(BaseModel):
    """
    ReminderCreateRequestDataSubject
    """
    id: StrictInt = Field(default=..., description="The unique identifier for a single CalendarEntry and Task associated with the Reminder. The keyword `null` is not valid for this field.")
    type: StrictStr = Field(default=..., description="Model type of the Subject.")
    __properties = ["id", "type"]

    @validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in ('CalendarEntry', 'Task'):
            raise ValueError("must be one of enum values ('CalendarEntry', 'Task')")
        return value
    ...
    @classmethod
    def from_json(cls, json_str: str) -> ReminderCreateRequestDataSubject:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> ReminderCreateRequestDataSubject:
        """Create an instance of ReminderCreateRequestDataSubject from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return ReminderCreateRequestDataSubject.parse_obj(obj)

        _obj = ReminderCreateRequestDataSubject.parse_obj({
            "id": obj.get("id"),
            "type": obj.get("type")
        })
        return _obj
