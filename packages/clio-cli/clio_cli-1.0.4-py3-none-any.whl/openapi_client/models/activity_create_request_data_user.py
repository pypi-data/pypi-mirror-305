...
class ActivityCreateRequestDataUser(BaseModel):
    """
    ActivityCreateRequestDataUser
    """
    id: Optional[StrictInt] = Field(default=None, description="The unique identifier for a single User associated with the Activity. Use the keyword `null` to specify no association. On creation, if no user is specified, it will default to the current user.  If a TimeEntry is created by a Clio Connect user, the field is not editable. ")
    __properties = ["id"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> ActivityCreateRequestDataUser:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> ActivityCreateRequestDataUser:
        """Create an instance of ActivityCreateRequestDataUser from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return ActivityCreateRequestDataUser.parse_obj(obj)

        _obj = ActivityCreateRequestDataUser.parse_obj({
            "id": obj.get("id")
        })
        return _obj
