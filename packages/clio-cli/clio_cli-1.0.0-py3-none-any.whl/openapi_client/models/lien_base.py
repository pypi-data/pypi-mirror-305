...
class LienBase(BaseModel):
    ...
    LienBase
    """
    id: Optional[StrictInt] = Field(default=None, description="Unique identifier for the *Lien*")
    amount: Optional[Union[StrictFloat, StrictInt]] = Field(default=None, description="The amount for Lien")
    description: Optional[StrictStr] = Field(default=None, description="Lien description")
    etag: Optional[StrictStr] = Field(default=None, description="ETag for the *Lien*")
    lien_type: Optional[StrictStr] = Field(default=None, description="Lien type")
    mark_as_lien: Optional[StrictBool] = Field(default=None, description="Mark item as Lien")
    created_at: Optional[datetime] = Field(default=None, description="The time the *Lien* was created (as a ISO-8601 timestamp)")
    updated_at: Optional[datetime] = Field(default=None, description="The time the *Lien* was last updated (as a ISO-8601 timestamp)")
    __properties = ["id", "amount", "description", "etag", "lien_type", "mark_as_lien", "created_at", "updated_at"]

    @validator('lien_type')
    def lien_type_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value

        if value not in ('general', 'medical_payer', 'medical_provider'):
            raise ValueError("must be one of enum values ('general', 'medical_payer', 'medical_provider')")
        return value
    ...
    @classmethod
    def from_json(cls, json_str: str) -> LienBase:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> LienBase:
        """Create an instance of LienBase from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return LienBase.parse_obj(obj)

        _obj = LienBase.parse_obj({
            "id": obj.get("id"),
            "amount": obj.get("amount"),
            "description": obj.get("description"),
            "etag": obj.get("etag"),
            "lien_type": obj.get("lien_type"),
            "mark_as_lien": obj.get("mark_as_lien"),
