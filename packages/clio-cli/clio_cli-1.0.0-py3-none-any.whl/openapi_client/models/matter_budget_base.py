...
class MatterBudgetBase(BaseModel):
    ...
    MatterBudgetBase
    """
    id: Optional[StrictInt] = Field(default=None, description="Unique identifier for the *MatterBudget*")
    etag: Optional[StrictStr] = Field(default=None, description="ETag for the *MatterBudget*")
    budget: Optional[Union[StrictFloat, StrictInt]] = Field(default=None, description="The amount allocated for the matter.")
    include_expenses: Optional[StrictBool] = Field(default=None, description="Whether the budget includes expenses.")
    notification_threshold: Optional[StrictInt] = Field(default=None, description="Percentage of the budget when it starts notifying users. The number must be between 0 and 100.")
    notify_users: Optional[StrictBool] = Field(default=None, description="Whether to notify users when the matter reaches the notification threshold.")
    created_at: Optional[datetime] = Field(default=None, description="The time the *MatterBudget* was created (as a ISO-8601 timestamp)")
    updated_at: Optional[datetime] = Field(default=None, description="The time the *MatterBudget* was last updated (as a ISO-8601 timestamp)")
    __properties = ["id", "etag", "budget", "include_expenses", "notification_threshold", "notify_users", "created_at", "updated_at"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> MatterBudgetBase:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> MatterBudgetBase:
        """Create an instance of MatterBudgetBase from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return MatterBudgetBase.parse_obj(obj)

        _obj = MatterBudgetBase.parse_obj({
            "id": obj.get("id"),
            "etag": obj.get("etag"),
