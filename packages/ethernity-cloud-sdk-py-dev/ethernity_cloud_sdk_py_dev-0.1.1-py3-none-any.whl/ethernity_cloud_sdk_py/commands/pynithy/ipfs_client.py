...
def main():
    ...
    initialize(args.host, args.protocol, args.port, args.token)

    if args.action == "upload":
        if args.filePath:
            hhash = upload_file_to_ipfs(args.filePath)
            print(f"{hhash}")
            ...
            while (not hhash or hhash == "Upload failed.") and retry_count < 3:
                try:
                    hhash = upload_folder_to_ipfs(args.folderPath)
                    print(f"{hhash}")
                except Exception as e:
        ...
        if args.filePath:
            print(f"Downloading file from IPFS: {args.hhash}")
            get_from_ipfs(args.hhash, args.filePath)
            print(f"File downloaded. {args.hhash}")
        elif args.folderPath:
            print(f"Downloading folder from IPFS: {args.hhash}")
            download_folder_from_ipfs(args.hhash, args.folderPath)
            print(f"Folder downloaded. {args.hhash}")
        else:
