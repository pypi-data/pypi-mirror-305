...
class Cache(Modulation, Generic[M]):
    ...
    def with_fir(self, iterable: Iterable[float]) -> Fir[Cache[M]]: ...
    def with_radiation_pressure(self, ) -> RadiationPressure[Cache[M]]: ...
    def with_timeout(self, timeout: timedelta | None) -> DatagramWithTimeout[Cache[M]]: ...
    def with_parallel_threshold(self, threshold: int | None) -> DatagramWithParallelThreshold[Cache[M]]: ...
    def with_segment(self, segment: Segment, transition_mode: TransitionModeWrap | None) -> DatagramWithSegment[Cache[M]]: ...
