...
import aiohttp # type: ignore # aiohttp
import asyncio
import threading
import logging
import time
...
import websocket  # type: ignore # Synchronous WebSocket client
import websockets  # type: ignore # Asynchronous WebSocket client
from fiscus.fiscus_token import generate_token
from fiscus.response import FiscusResponse, FiscusError, FiscusErrorCode
from fiscus.callbacks import FiscusCallback, FiscusOnResponse
from fiscus.enums import FiscusActionType, FiscusLogLevel, FiscusActionTypeEncoder, FiscusRestType
...
class _ConnectionManager:
	...
	def __init__(self, api_key: str):
		...
		"""
		self.logger = logging.getLogger('fiscus.connection')
		...
		self.websocket_endpoint = 'wss://6qnsbv19b9.execute-api.us-east-1.amazonaws.com/prod/'  # WebSocket API endpoint dev
	...
	def start_websocket_connection_sync(self, user_id: str) -> None:
		...
		def on_error(ws, error):
			self.logger.error(f"Synchronous WebSocket error: {error}")
			self.logger.critical("Critical error encountered in synchronous WebSocket connection.")

			# Check if on_error callback is defined before calling
			if self.on_error:
				self.on_error({"error": error})
			else:
				self.logger.debug("No on_error callback defined; error not passed to callback.")
	...
	def send_websocket_message_sync(
		...
		if not self.websocket_sync_connected or not self.websocket_sync:
			self.logger.error("Cannot send message: Synchronous WebSocket is not connected.")
		...
		try:
			...
			while True:
				...
				if remaining_time <= 0:
					self.logger.error("Synchronous WebSocket response timeout.")
					break
				try:
			...
			if responses:
				...
				if first_response.get('success'):
					...
					self.logger.error(f"WebSocket operation failed with error: {error_message}")

					# Return the FiscusResponse with the constructed error (no direct callback handling here)
				...
				self.logger.error(f"No responses received for message_id: {message_id}.")
			...
			self.logger.error(f"Exception while sending synchronous WebSocket message: {e}")
	...
	async def start_websocket_connection(self, user_id: str) -> None:
		...
		try:
			...
			self.websocket = await websockets.connect(
				self.websocket_endpoint,
				extra_headers={f"{k}: {v}" for k, v in headers.items()},
				ping_interval=60,
				ping_timeout=30,
			)
			...
			self.logger.error(f"Failed to establish asynchronous WebSocket connection: {e}")
			...
			async for message in self.websocket:
				...
				try:
					...
					if message_id and message_id in self.pending_responses_async:
						...
						# Example: self.general_callback({'response': message_data})
				except json.JSONDecodeError as e:
					self.logger.error(f"JSON decoding error in asynchronous message: {e}")
		except Exception as e:
			self.logger.error(f"Error in asynchronous WebSocket receive loop: {e}")
	...
	async def send_websocket_message(
		...
		if not self.websocket_connected or not self.websocket:
			self.logger.error("Cannot send message: Asynchronous WebSocket is not connected.")
		...
		try:
			...
			try:
				...
				if response_data.get('success'):
					...
					self.logger.error(f"Asynchronous WebSocket operation failed with error: {error_msg}")
					return FiscusResponse(
						success=False,
						error=fiscus_error,
						message_id=message_id
				...
				self.logger.error("Asynchronous WebSocket response timed out.")
			...
			self.logger.error(f"Exception while sending asynchronous WebSocket message: {e}")
			...
			if message_id in self.pending_responses_async:
				self.pending_responses_async.pop(message_id)
			if message_id in self.callback_async:
	...
	async def stop_websocket_connection(self) -> None:
		...
		if self.websocket_connected and self.websocket:
			...
			try:
				...
				self.logger.error(f"Error closing asynchronous WebSocket: {e}")
			self.websocket_connected = False
	...
	async def send_rest_request(
		...
		try:
			...
			self.logger.error(f"Error sending REST request: {e}")
			fiscus_error = FiscusError(
	...
	def _handle_response(self, response, response_data, action, start_time) -> FiscusResponse:
		...
		# General error handling for non-200 statuses
			...
			return FiscusResponse(
				success=False,
				error=FiscusError(
					code=FiscusErrorCode.UNKNOWN_ERROR,
					message=error_message
				),
				message_id=response_data.get("message_id", str(uuid.uuid4()))
			)
		...
		# Success handling for USER action type
			...
			if response_data.get("success", False):
				...
				self.logger.error(f"REST USER operation failed with error: {error_message}")
				return FiscusResponse(
					success=False,
					error=FiscusError(
						code=FiscusErrorCode.UNKNOWN_ERROR,
		...
		# General success handling for other action types
			...
			self.logger.error(f"REST operation failed with error: {error_message}")
			return FiscusResponse(
				success=False,
				error=FiscusError(
					code=FiscusErrorCode.UNKNOWN_ERROR,
	...
	def _get_rest_endpoint(self, action: FiscusActionType) -> str:
		...
		if action not in self.rest_endpoint_map:
			raise ValueError(f"Endpoint for action '{action}' not found in the configuration")
