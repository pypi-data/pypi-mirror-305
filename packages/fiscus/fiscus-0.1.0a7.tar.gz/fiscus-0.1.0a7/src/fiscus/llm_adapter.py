...
from fiscus.enums import FiscusLLMType, FiscusResponseType
...
class _LLMAdapter:
        ...
        **kwargs
		...
		try:
			if use_structured_output:
				...
				parsed_result = self._parse_response_with_fallback(content, response_key)
	...
	def _execute_anthropic(
		...
		**kwargs
		...
		try:
			...
			return self._parse_response_with_fallback(response.content[0].text if response.content else None, response_key)
		except Exception as e:
	...
	def _parse_response_with_fallback(self, response_content: Any, response_key: str) -> Any:
		"""
