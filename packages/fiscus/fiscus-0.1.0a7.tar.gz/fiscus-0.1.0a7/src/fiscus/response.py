...
from fiscus.enums import FiscusCallbackType
...
class FiscusError:
	...
	def __init__(self, code: FiscusErrorCode, message: str, details: Optional[Any] = None):
		"""
		Initialize a FiscusError.

		:param code: A FiscusErrorCode enumeration value.
...
class FiscusResponse:
	...
	def __init__(
		...
		"""
		self.logger = logging.getLogger(f'fiscus.response.{self.__class__.__name__}')
		...
		if isinstance(error, FiscusError):
			...
			self.error = FiscusError(
				code=error.get('code', FiscusErrorCode.UNKNOWN_ERROR),
