...
from fiscus.enums import FiscusActionType, FiscusLogLevel, FiscusRestType
...
class FiscusUser:
	...
	def __init__(self, user_id: str, client: Optional['FiscusClient'] = None):
		...
		:param client: Reference to the parent FiscusClient instance.
		"""
		...
		self.logger = logging.getLogger(f'fiscus.user.{self.user_id}')
	...
	def _send_user_data(
		...
		"""
		Synchronous method to send user-related data using either WebSocket or REST, with operation passed as query parameter.
		"""
		...
		if self.client.connection_manager.websocket_sync_connected:
			...
			self.logger.warning("No live WebSocket connection, falling back to REST for sending user data.")
	...
	async def _send_user_data_async(
		...
		"""
		Asynchronous method to send user-related data using either WebSocket or REST, with operation passed as query parameter.
		"""
		...
		if self.client.connection_manager.websocket_connected:
			...
			return await self.client.orchestrator.send_user_data_via_websocket_async(
				user_id=self.user_id, 
				data=data, 
				callbacks=callbacks
			)
		else:
			self.logger.warning("No live WebSocket connection, falling back to REST for sending user data asynchronously.")
	...
	def add_connector(self, connector: str) -> FiscusResponse:
		...
		data = {
			'connector': connector,
			'data': connector_instance.configuration or {}
		}
		...
		if response.success:
			...
			if response.is_nested:
				self.logger.error(f"Failed to add connector '{connector}': {response.nested_message}")
				return FiscusResponse(success=False, error_message=response.nested_message)
			else:
				self.logger.error(f"Failed to add connector '{connector}': {response.error}")
				return FiscusResponse(success=False, error_message=response.error)
	...
	async def add_connector_async(self, connector: str) -> FiscusResponse:
		...
		if response.success:
			...
			if response.is_nested:
				self.logger.error(f"Failed to add connector '{connector}' asynchronously: {response.nested_message}")
				return FiscusResponse(success=False, error_message=response.nested_message)
			else:
				self.logger.error(f"Failed to add connector '{connector}' asynchronously: {response.error}")
				return FiscusResponse(success=False, error_message=response.error)
	...
	def authenticate_connector(self, connector: str, auth_params: Optional[Dict[str, Any]] = None) -> FiscusResponse:
		"""
		Synchronously authenticate with a connector.
		"""
		...
		# Check for successful authentication and extract only the `authorization_url`
		if response.success:
		...
		self.logger.error(f"Failed to authenticate connector '{connector}': {response.error}")
		return FiscusResponse(success=False, error_message=response.error)

	async def authenticate_connector_async(self, connector: str, auth_params: Optional[Dict[str, Any]] = None) -> FiscusResponse:
		"""
		Asynchronously authenticate with a connector.
		"""
		...
		# Extract only the authorization URL from the response if successful
		if response.success:
			authorization_url = response.to_json().get("result", {}).get("data")
		...
		# If unsuccessful, handle error as before
		self.logger.error(f"Failed to authenticate connector '{connector}' asynchronously: {response.error}")
		return FiscusResponse(success=False, error_message=response.error)

	def deauthenticate_connector(self, connector: str) -> FiscusResponse:
		"""
		Synchronously deauthenticate from a connector.
		"""
		...
		if connector_instance:
			...
			self.logger.info(f"Connector '{connector}' deauthenticated successfully.")
			...
			if response.success:
				...
				self.logger.error(f"Failed to deauthenticate connector '{connector}': {response.error}")
				return FiscusResponse(success=False, error_message=response.error)
		else:
			self.logger.warning(f"FiscusConnector '{connector}' not found for user '{self.user_id}'.")

			# Optionally, return a response indicating the connector was not found
			return FiscusResponse(success=False, error_message=f"Connector '{connector}' not found.")
		
		return FiscusResponse(success=True, result=response)

	async def deauthenticate_connector_async(self, connector: str) -> FiscusResponse:
		"""
		Asynchronously deauthenticate from a connector.
		"""
		...
		if connector_instance:
			connector_instance.authenticated = False
			connector_instance.credentials = None
			self.logger.info(f"Connector '{connector}' deauthenticated successfully (async).")
			...
			if response.success:
				...
				self.logger.error(f"Failed to deauthenticate connector '{connector}' asynchronously: {response.error}")
				return FiscusResponse(success=False, error_message=response.error)
		else:
			self.logger.warning(f"FiscusConnector '{connector}' not found for user '{self.user_id}' (async).")
			# Optionally, return a response indicating the connector was not found
			return FiscusResponse(success=False, error_message=f"Connector '{connector}' not found (async).")
		
		return FiscusResponse(success=True, result=response)
	
	def list_connectors(self) -> FiscusResponse:
		"""
		List all connectors and context associated by fetching from the backend.

		:return: List of connector data.
		"""
		...
		if not response_categories.success:
			self.logger.error(f"Error fetching connected categories: {response_categories.error}")
		...
		if not response_connectors.success:
			self.logger.error(f"Error fetching connectors for categories {categories}: {response_connectors.error}")
	...
	async def list_connectors_async(self) -> FiscusResponse:
		"""
		Asynchronously list all connected connectors by fetching from the backend.
		...
		if not response_categories.success:
			self.logger.error(f"Error fetching connected categories asynchronously: {response_categories.error}")
		...
		if not response_connectors.success:
			self.logger.error(f"Error fetching connectors for categories {categories} asynchronously: {response_connectors.error}")
	...
	def list_connected_connectors(self) -> FiscusResponse:
		"""
		List all connected connectors by fetching from the backend.
	...
	def _get_connected_categories(self) -> List[str]:
		...
		if response.success:
			...
			self.logger.error(f"Failed to retrieve connected categories: {response.error}")
	...
	async def _get_connected_categories_async(self) -> List[str]:
		...
		if response.success:
			...
			self.logger.error(f"Failed to retrieve connected categories asynchronously: {response.error}")
	...
	def _get_connected_connectors(self, categories: List[str]) -> List[str]:
		...
		if response.success:
			...
			self.logger.error(f"Failed to retrieve connectors for categories {categories}: {response.error}")
	...
	async def _get_connected_connectors_async(self, categories: List[str]) -> List[str]:
		...
		if response.success:
			...
			self.logger.error(f"Failed to retrieve connectors for categories {categories} asynchronously: {response.error}")
	...
	def set_context(self, key: str, value: Any, ttl_seconds: Optional[int] = None) -> FiscusResponse:
		"""
		Set a context variable synchronously with optional TTL.
		
		:param key: The context key (e.g., 'language', 'theme').
		:param value: The value to set for the context.
		:param ttl_seconds: Time-to-live in seconds for this context value. Default is 24 hours.
		"""
		...
		if response.success:
			...
			self.logger.error(f"Failed to update context '{key}': {response.error}")
	...
	async def set_context_async(self, key: str, value: Any, ttl_seconds: Optional[int] = None) -> FiscusResponse:
		"""
		Set a context variable asynchronously with optional TTL.
		...
		if response.success:
			...
			self.logger.error(f"Failed to update context '{key}' asynchronously: {response.error}")
			return FiscusResponse(success=False, error=response.error)
	...
	def get_context(self, key: Optional[str] = None) -> FiscusResponse:
		"""
		Retrieve a specific context variable or all context if no key is provided.

		:param key: The context key (e.g., 'language', 'theme'). If None, retrieves all context.
		:return: FiscusResponse containing the context value(s), or an error if retrieval fails.
		"""
	...
	async def get_context_async(self, key: Optional[str] = None) -> FiscusResponse:
		"""
		Asynchronously retrieve a specific context variable or all context if no key is provided.
	...
	def assign_role(self, role: str) -> FiscusResponse:
		...
		if role not in self.roles:
			...
			if response.success:
				...
				self.logger.error(f"Failed to assign role '{role}': {response.error}")
				return FiscusResponse(success=False, error=response.error)
		else:
	...
	async def assign_role_async(self, role: str) -> FiscusResponse:
		...
		if role not in self.roles:
			...
			if response.success:
				...
				self.logger.error(f"Failed to assign role '{role}' asynchronously: {response.error}")
				return FiscusResponse(success=False, error=response.error)
		else:
	...
	def add_policy(self, policy: Dict[str, Any]) -> FiscusResponse:
		"""
		Synchronously add a policy to the user.
		"""
		...
		if response.success:
			...
			self.logger.error(f"Failed to add policy: {response.error}")
	...
	async def add_policy_async(self, policy: Dict[str, Any]) -> FiscusResponse:
		"""
		Asynchronously add a policy to the user.
		"""
		...
		if response.success:
			...
			self.logger.error(f"Failed to add policy asynchronously: {response.error}")
			self.audit_trail.record('add_policy_async', {'error': response.error})
			return FiscusResponse(success=False, error=response.error)
	...
	def set_user_id(self, new_user_id: str) -> FiscusResponse:
		...
		if response.success:
			...
			self.logger.error(f"Failed to change user ID: {response.error}")
	...
	async def set_user_id_async(self, new_user_id: str) -> FiscusResponse:
		...
		if response.success:
			...
			self.logger.error(f"Failed to change user ID asynchronously: {response.error}")
			self.audit_trail.record('set_user_id_async', {'error': response.error})
			return FiscusResponse(success=False, error=response.error)
	...
	def get_connectors_info(self) -> Dict[str, Any]:
		...
		if response.success:
			...
			self.logger.error(f"Failed to retrieve connectors info: {response.error}")
	...
	async def get_connectors_info_async(self) -> Dict[str, Any]:
		...
		if response.success:
			...
			self.logger.error(f"Failed to retrieve connectors info asynchronously: {response.error}")
			self.audit_trail.record('get_connectors_info_async', {'error': response.error})
			return FiscusResponse(success=False, error=response.error)
	...
	def set_dynamic_preferences(self, preferences: Dict[str, str]) -> FiscusResponse:
		"""
		Synchronously set dynamic preferences for categories and connectors.
		"""
		...
		if response.success:
			...
			self.logger.error(f"Failed to set dynamic preferences: {response.error}")
			return FiscusResponse(success=False, error=response.error)
	...
	async def set_dynamic_preferences_async(self, preferences: Dict[str, str]) -> FiscusResponse:
		"""
		Asynchronously set dynamic preferences for categories and connectors.
		"""
		...
		if response.success:
			...
			self.logger.error(f"Failed to set dynamic preferences asynchronously: {response.error}")
			return FiscusResponse(success=False, error=response.error)
