...
class SequenceTools(object):
    ...
    @staticmethod
    def insert_xor_sequence(
            sequences: list,
            intro_xor: int = 2,
            need_group: bool = False,
            group_members: int | None = None,
    ) -> list:
        """
        Introduction
        ------------
    ...
    @staticmethod
    def delete_xor_sequence(
            sequences: list,
            intro_xor: int = 2,
            need_ungroup: bool = False
    ) -> list:
        """
        Introduction
        ------------
...
class ConsensusTools(object):
    ...
    @staticmethod
    def del_ont_adaptor(ont_read: str, head: str = "CAGCACCT", tail: str = "GGTGCTGT", threshold: int = 150) -> str:
    ...
    @staticmethod
    def del_assembly_adaptor(ont_read: str, assembly_adaptors: list, threshold: int = 20) -> str:
        adapter_used = 0
        disassembly_seqs = []
        ...
        for i in range(len(assembly_adaptors)):
            ...
            position = ont_read.find(adaptor)

            if position != -1 and position in range(80, 120):
                disassembly_seqs.append(ont_read[: position])
                ont_read = ont_read[position + 8:]
                adapter_used += 1

            else:
                scores = []
                start = 100 - threshold
                end = 100 + threshold if len(ont_read) >= 100 + threshold else len(ont_read)

                if start >= end:
                    disassembly_seqs.append(ont_read)
                    break

                else:
                    for j in range(start, end):
                        alignment = aligner.align(adaptor, ont_read[j: j + 8])[0]
                        scores.append(alignment.score)

                    max_score = max(scores)
                    if max_score < 6 and end == len(ont_read):
                        disassembly_seqs.append(ont_read)
                        break
                    ...
                    if len(max_score_indices) == 1:
                        ...
                        hamming_distances = [
                            HammingDistanceTools.hamming_distance_in_seqs(
                                ref_seq=adaptor,
                                qry_seq=ont_read[max_score_indices[x] + start: max_score_indices[x] + start + 8]
                            )
                    ...
                    disassembly_seqs.append(ont_read[: target_index])
                    ont_read = ont_read[target_index + 8:]
        ...
        if len(assembly_adaptors) == 11 and ont_read != "":
            disassembly_seqs.append(ont_read[: 115])
