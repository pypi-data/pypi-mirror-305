...
class B2CHelper:
    ...
    def filter_feat_branch(self, users: list, branch_id: int) -> list:
        ...
        logging.info(f'branch_id: {branch_id}')

        if self.token_request_data['b2c_tenant_id'] == 'e3ac84d9-d336-4184-b7ef-5b9c56d2e201':
            filtered_users = []
            for user in users:
                ...
                logging.info(branch_id_extract)
                # logging.info('\n\n\n')
    ...
    def get_users(self, company_id: str = None, role_type: str = None, branch_id: int = None) -> list:
        ...
        logging.info(f'ROLE_TYPE: {role_type}')
        logging.info(f'BRANCH_ID: {branch_id}')
