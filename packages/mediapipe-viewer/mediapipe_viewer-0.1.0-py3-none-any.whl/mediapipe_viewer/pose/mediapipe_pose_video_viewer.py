...
# 显示
def show(mv1: MediapipePoseVideo, mv2: MediapipePoseVideo):
    # 初始化 Rerun SDK
    ...
    while True:
        ...
        if ret1:
            if mv1.get_video_frame_image() is not None:
                rr.log("/video1/video2d/frame", rr.Image(mv1.get_video_frame_image()).compress(jpeg_quality=10))
        ...
        if ret2:
            if mv2.get_video_frame_image() is not None:
                rr.log("/video2/video2d/frame", rr.Image(mv2.get_video_frame_image()).compress(jpeg_quality=10))
