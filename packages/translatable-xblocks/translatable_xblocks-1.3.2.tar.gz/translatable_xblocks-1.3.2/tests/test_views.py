...
class TestConfigApiGet(APITestCase):
    ...
    def test_get_config(
        ...
        with patch.object(IsAuthenticated, "has_permission", return_value=True):
            ...
            mock_get_course_translation_languages.return_value = mock_get_translations_language
...
class TestConfigApiSet(APITestCase):
    ...
    def set_config_request(self, course_id, feature_enabled, available_translation_languages):
        """Wraps the client call for ease of use."""
        request_data = {
            "course_id": course_id,
    ...
    @patch("translatable_xblocks.api.views.Config")
    def test_set_config(
        self, mock_config, mock_set_feature_enabled, mock_get_user_role, mock_post_course_translation_languages
    ):
        with patch.object(IsAuthenticated, "has_permission", return_value=True):
            # Given a valid course & settings
            course_id = self.valid_course_key
            ...
            mock_post_course_translation_languages.return_value = mock_post_translations_language

            # When I, as staff, try to set the config
            mock_get_user_role.return_value = "staff"
            new_setting_value = "baz"

            response = self.set_config_request(course_id, new_setting_value, self.configured_languages)
            ...
            mock_post_course_translation_languages.assert_called_with(mock_post_translations_language)
    ...
    def test_get_config_missing_available_translation_languages(
        self, mock_config, mock_set_feature_enabled, mock_get_user_role, mock_get_course_translation_languages
    ):
        course_id = self.valid_course_key
        ...
        mock_get_course_translation_languages.return_value = mock_post_translations_language
        ...
        with patch.object(IsAuthenticated, "has_permission", return_value=True):
            ...
            response = self.client.post(
                reverse("translatable-xblocks-config"), {"course_id": course_id, "feature_enabled": new_setting_value}
            )
