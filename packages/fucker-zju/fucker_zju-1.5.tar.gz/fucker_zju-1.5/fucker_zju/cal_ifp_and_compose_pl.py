...
def interaction(interaction_file_path):
    lines = list(csv.reader(open(interaction_file_path, 'r')))
    if len(lines) > 1:
        dic = {
            'Hbond': [],
            'pipi': [],
        ...
        for line in lines[1:]:
            if line[2] == 'hbond':
                dic['Hbond'].append(
                    {'x1': float(line[9]), 'y1': float(line[10]), 'z1': float(line[11]),
                    'x2': float(line[13]), 'y2': float(line[14]), 'z2': float(line[15])}
                )
                dic2['Hbond'].append(
                    {
                        'Resi_name': line[3], 'Resi_idx': line[5], 'Resi_chain': line[4], 'Resi_atom': line[7],
                        'isacceptor': line[8],
                        'Ligand_atom': line[12]
                    }
                )
                ...
                dic2['pipi'].append(
                    {
                        'Resi_name': line[3], 'Resi_idx': line[5], 'Resi_chain': line[4],
                        'Parallel': line[16], 'Perpendicular': line[17]
                    }
                )
            elif line[2] == 'pi_cation':
                if line[7] == '!':
                    # ���ڵ�����
                    dic['pi_cation'].append(
                        {'x1': float(line[9]), 'y1': float(line[10]), 'z1': float(line[11]),
                        'x2': float(line[13]), 'y2': float(line[14]), 'z2': float(line[15])}
                    )
                    dic2['pi_cation'].append(
                        {
                            'Resi_name': line[3], 'Resi_idx': line[5], 'Resi_chain': line[4],
                            'Resi_atom': 'Pi-Ring',
                            'Ligand_atom': line[12]
                    ...
                    # ����������
                    dic['pi_cation'].append(
                        {'x1': float(line[13]), 'y1': float(line[14]), 'z1': float(line[15]),
                        'x2': float(line[9]), 'y2': float(line[10]), 'z2': float(line[11])}
                    )
                    dic2['pi_cation'].append(
                        {
                            'Resi_name': line[3], 'Resi_idx': line[5], 'Resi_chain': line[4], 'Resi_atom': line[7],
                ...
                dic['salt_bridge'].append(
                    {'x1': float(line[9]), 'y1': float(line[10]), 'z1': float(line[11]),
                    'x2': float(line[13]), 'y2': float(line[14]), 'z2': float(line[15])}
                )

                dic2['salt_bridge'].append(
                    {
                        'Resi_name': line[3], 'Resi_idx': line[5], 'Resi_chain': line[4], 'Resi_atom': line[7],
                        'isacceptor': line[8],
                        'Ligand_atom': line[12]
