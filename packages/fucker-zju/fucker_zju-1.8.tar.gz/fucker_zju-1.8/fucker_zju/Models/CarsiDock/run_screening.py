...
def main(args):
    ...
    print(f"use device {os.environ['CUDA_VISIBLE_DEVICES']}")
    # args.cuda_device_index = int(os.environ.get('CUDA_VISIBLE_DEVICES'))
    ...
    for item in tqdm(data):
        ...
        num_mols = outputs['mol_list']
        if len(num_mols)>10:
            generate_mols = outputs['mol_list'][:10]
        else:
            generate_mols = outputs['mol_list']
        for i in range(len(generate_mols)):
            docked_mol.append(generate_mols[i])
