...
def get_trade_days_range(
    start_date: typing.Union[str, pd.Timestamp],
    end_date: typing.Union[str, pd.Timestamp],
) -> typing.List[datetime.date]:
    """
    获取两个日期之间的交易日列表。
    ...
    返回:
        List[datetime.date]: 在[start_date, end_date]范围内的交易日列表。
    ...
    # 筛选出在指定日期范围内的交易日
    filtered_dates = dt_series[
        (dt_series >= start_date_converted) & (dt_series <= end_date_converted)
    ]

    # 转换为datetime.date类型并返回
    return filtered_dates.dt.date.to_list()
