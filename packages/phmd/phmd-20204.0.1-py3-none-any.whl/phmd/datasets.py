...
import numpy as np
from phmd import *
from phmd.features import extract_features
...
def __update_meta(meta: dict, dataset_name: str):
    meta_file = os.path.dirname(os.path.realpath(__file__)) + "/datasets/%s.json" % dataset_name

    json.dump(meta, open(meta_file, "w"), indent=4)


def __get_sets(dataset_name: str):
    meta = read_meta(dataset_name)

    return [f['type'] for f in meta['files']]
    ...
    if (len(task['identifier']) > 0) or (
        ...
        if 'split_by' in task:
            ...
            if ('classification' in task['type']):
                logging.info(f"Is possible stratified split: {possible_target_balancing}")
...
def load_cv_sets_features(dataset_name: str, task_name, ts_len: int, fold: int, num_folds: int = 5,
                          preprocess=None, return_test=False, normalize_output=False, filters=None, random_state=666,
                          test_pct=0.3, n_jobs=1, backup=False):
    """
       Loads and preprocesses train, validation, and optional test datasets for a specified task.
       Extracts features from the time-series data, reshaping it for model input, and handles caching
       to avoid repeated computations.

       Parameters:
       -----------
       dataset_name : str
           Name of the dataset to load metadata and data.
       task_name : str
           Name of the task within the dataset, used to retrieve task-specific features and target.
       ts_len : int
           Length of each time-series segment for feature extraction.
       fold : int
           The current fold index for cross-validation.
       num_folds : int, optional
           Total number of folds for cross-validation (default is 5).
       preprocess : transformer or None, optional
           Transformer for data preprocessing; if provided, it will be fit on training data and
           applied to all sets (default is `None`).
       return_test : bool, optional
           If `True`, returns a test set along with train and validation sets (default is `False`).
       normalize_output : bool, optional
           If `True`, normalizes the output data based on predefined parameters (default is `False`).
       filters : dict or None, optional
           Dictionary of filters to apply for data selection (default is `None`).
       random_state : int, optional
           Random seed for data splitting (default is 666).
       test_pct : float, optional
           Percentage of data to use for the test set if `return_test` is `True` (default is 0.3).
       n_jobs : int, optional
           Number of processes for parallel feature extraction (default is 1).
       backup : bool, optional
           If `True`, saves the processed feature data to avoid reprocessing (default is `False`).

       Returns:
       --------
       dict
           A dictionary containing the training and validation (and optionally test) sets with extracted features.
           Each set is a DataFrame where the columns represent the extracted features and the target variable.

       Example:
       --------
       To load and extract features for a specific dataset task:
       ```python
       sets = load_train_features('CWRU', 'fault_diameter', ts_len=100, fold=1, num_folds=3)
       ```

       Notes:
       ------
       - If a precomputed backup of the feature data exists, the function loads it to save time.
       - Data is reshaped into time-series segments with length `ts_len`, allowing efficient feature extraction.
       - If `preprocess` is provided, the features are normalized or transformed before returning.
       - Feature extraction can be parallelized using `n_jobs` for faster processing on large datasets.
   ...
   """
    ...
    file_hash = get_hash(dataset_name=dataset_name,
                         task_name=task_name,
                         ts_len=ts_len,
                         fold=fold,
                         num_folds=num_folds,
    ...
    features_backup_file = f"{dataset_name}_{task_name}_{file_hash}.pk"
    data_dir = get_storage_dir()
    features_backup_file = os.path.join(data_dir, features_backup_file)

    if os.path.exists(features_backup_file):
        _sets = pk.load(open(features_backup_file, 'rb'))

    else:
        # Load the training, validation, and test sets
        _sets = load_cv_sets(dataset_name, task_name, fold, num_folds, None, return_test,
                             normalize_output, filters, random_state, test_pct)

        # Divide full time-series in ts_len length time-series
        for set_name, X in _sets.items():
            X = X.reset_index()
            if len(task['identifier']) == 0:
                task['identifier'] = ['id']
                X['id'] = 0
            ...
            X['_index'] = X.index
            X['start_index'] = X.groupby(task['identifier'])._index.transform('min')
            X['ts_index'] = (X.index - X.start_index) // ts_len
            X['time'] = (X.index - X.start_index - (ts_len * X.ts_index))
            del X['start_index']
            del X['_index']

            index_cols = task['identifier'] + ['ts_index']
            T = X.pivot_table(index=index_cols, columns='time', values=task['target'])
            T = T.loc[~T.isnull().any(axis=1)]
            T = T.loc[:, [ts_len - 1]]
            T.columns = [task['target']]
            T = T.reset_index()
            T.loc[:, task['target']] = T.loc[:, task['target']].astype('float32')

            for raw_feature in task['features']:
                start_time = time.time()
                S = X.pivot_table(index=index_cols, columns='time', values=raw_feature)
                S = S.loc[~S.isnull().any(axis=1)]

                feat_type = meta["features"][raw_feature]["type"]
                frec_feat = feat_type in ['acoustic', 'vibration']

                end_time = time.time()
                logging.info(f"Splitted time series in {end_time - start_time} seconds")

                Xfeatures = extract_features(S, S.columns, n_jobs, prefix=raw_feature,
                                             frec_features=frec_feat)

                start_time = time.time()

                T = pd.concat((T, Xfeatures), axis=1)

                end_time = time.time()
                logging.info(f"Features concatenation in {end_time - start_time} seconds")
        ...
        if backup:
            pk.dump(_sets, open(features_backup_file, 'wb'))


    if preprocess is not None:
        X = _sets['train']
        features = [x for x in X.columns if x not in task['identifier'] + [task['target']]]
        preprocess.fit(X[features])

        for name, X in _sets.items():
            X.loc[:, features] = preprocess.transform(X.loc[:, features])
            _sets[name] = X
...
class Task:
    def __init__(self, dataset: Dataset, task_name: str, default_method: str = "sets"):
        ...
        self.default_method = default_method  # Set default loading method
        self.preprocess = None
        self.normalize_output = None
        self.test_pct = 0.3
        self.return_test = True
        self.random_state = 666
        self.ts_len = self.meta.get("min_ts_len", 10000) // 10
        self.n_jobs = 4
    ...
    def load_fold(self, fold: int):
        ...
        if self.method == "sets":
            ...
            logging.info(f"Splitting time series into segments of {self.ts_len} data points to extract features.")
            return load_cv_sets_features(
                dataset_name=self.dataset.dataset_name,
                task_name=self.name,
                ts_len=self.ts_len,
                fold=fold,
            ...
            raise ValueError("Invalid method. Use 'sets' or 'features'.")
