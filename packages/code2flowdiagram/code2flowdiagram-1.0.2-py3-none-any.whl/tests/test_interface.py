...
from code2flowdiagram.engine import code2flowdiagram, main, _generate_graphviz, SubsetParams
from code2flowdiagram import model

IMG_PATH = '/tmp/code2flowdiagram/output.png'
if os.path.exists("/tmp/code2flowdiagram"):
    try:
        shutil.rmtree('/tmp/code2flowdiagram')
    except FileNotFoundError:
        os.remove('/tmp/code2flowdiagram')
os.mkdir('/tmp/code2flowdiagram')
...
def test_generate_image():
    if os.path.exists(IMG_PATH):
        os.remove(IMG_PATH)
    code2flowdiagram(os.path.abspath(__file__),
...
def test_no_files():
    with pytest.raises(AssertionError):
        code2flowdiagram(os.path.abspath(__file__) + "fakefile",
                  output_file=IMG_PATH)


def test_graphviz_error(caplog):
    caplog.set_level(logging.DEBUG)
    _generate_graphviz("/tmp/code2flowdiagram/nothing", "/tmp/code2flowdiagram/nothing",
                       "/tmp/code2flowdiagram/nothing")
    assert "non-zero exit" in caplog.text


def test_no_files_2():
    if not os.path.exists('/tmp/code2flowdiagram/no_source_dir'):
        os.mkdir('/tmp/code2flowdiagram/no_source_dir')
    if not os.path.exists('/tmp/code2flowdiagram/no_source_dir/fakefile'):
        with open('/tmp/code2flowdiagram/no_source_dir/fakefile', 'w') as f:
            f.write("hello world")

    with pytest.raises(AssertionError):
        code2flowdiagram('/tmp/code2flowdiagram/no_source_dir',
                  output_file=IMG_PATH)
...
def test_json():
    code2flowdiagram('test_code/py/simple_b',
              output_file='/tmp/code2flowdiagram/out.json',
              hide_legend=False)
    with open('/tmp/code2flowdiagram/out.json') as f:
...
def test_weird_encoding():
    """
    To address https://github.com/scottrogowski/code2flowdiagram/issues/28
    ...
    code2flowdiagram('test_code/py/weird_encoding',
              output_file='/tmp/code2flowdiagram/out.json',
              hide_legend=False)
    with open('/tmp/code2flowdiagram/out.json') as f:
        jobj = json.loads(f.read())
...
def test_bad_acorn(mocker, caplog):
    caplog.set_level(logging.DEBUG)
    mocker.patch('code2flowdiagram.javascript.get_acorn_version', return_value='7.6.9')
    code2flowdiagram("test_code/js/simple_a_js", "/tmp/code2flowdiagram/out.json")
...
def test_bad_ruby_parse(mocker):
    ...
    with pytest.raises(AssertionError) as ex:
        code2flowdiagram("test_code/rb/simple_b", "/tmp/code2flowdiagram/out.json")
        assert "ruby-parse" in ex and "syntax" in ex


def test_bad_php_parse_a():
    with pytest.raises(AssertionError) as ex:
        code2flowdiagram("test_code/php/bad_php/bad_php_a.php", "/tmp/code2flowdiagram/out.json")
        assert "parse" in ex and "syntax" in ex


def test_bad_php_parse_b():
    with pytest.raises(AssertionError) as ex:
        code2flowdiagram("test_code/php/bad_php/bad_php_b.php", "/tmp/code2flowdiagram/out.json")
        assert "parse" in ex and "php" in ex.lower()


def test_no_source_type():
    with pytest.raises(AssertionError):
        code2flowdiagram('test_code/js/exclude_modules_es6',
                  output_file='/tmp/code2flowdiagram/out.json',
                  hide_legend=False)
...
def test_cli_log_default(mocker):
    ...
    logging.basicConfig.assert_called_once_with(format="code2flowdiagram: %(message)s",
                                                level=logging.INFO)
