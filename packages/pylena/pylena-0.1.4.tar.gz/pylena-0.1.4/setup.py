...
class ConanCMakeBuildExtension(build_ext):
    ...
    def _build(self, ext : ConanCMakeExtension):
        ...
        if not "lrde-public" in (x.name for x in remotes):
            remotes.insert(0, Remote("lrde-public", "https://artifactory.lrde.epita.fr/artifactory/api/conan/lrde-public"))
        ...
        default_profile.options.skip_pybind11 = True
        ...
        api.config.global_conf.update("core.sources:download_urls", ["origin", "https://c3i.jfrog.io/artifactory/conan-center-backup-sources/"])
        ...
        deps_graph = api.graph.load_graph_consumer(conanfile_path, None, None, None, None,
        ...
        force = int(os.environ.get("PYLENA_FORCE_BUILD_FROM_SOURCES", 0)) # Hack to force rebuild from sources (setup.py build --force)
        ...
        output_folder = cwd
        api.install.install_consumer(deps_graph=deps_graph,
                                       generators=None,
        ...
        subprocess.run(["cmake", "-S", ".", "-B", "build",    
                        "--preset", f"conan-{ext.build_type.lower()}"]
                        + options, cwd=cwd, check=True)
