...
class FBGcom:
    ...
    def auto_setting(self):
        ...
        _peaks, _ = scipy.signal.find_peaks(_spectrum_proc, height=10, distance=50, prominence=100)
        _peaks = _peaks[1:]
        if self.FBG_num is None:
            ...
            while not len(_peaks) == self.FBG_num:
                _peaks, _ = scipy.signal.find_peaks(_spectrum_proc, height=height_th, distance=50, prominence=100)
                _peaks = _peaks[1:]
                height_th += 100
                if height_th > 10000:
