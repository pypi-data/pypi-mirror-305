...
class DefaultShrinkModel(MCAPSetOpModelModule):
    ...
    def eval_output_function(self, output, metrics):
        if len(output) < 2:
            ...
            for metric in metrics:
                output[metric] = None
            return output
        model_names = output[1].run_service("GET", "VERTEX_MODEL_NAMES")
        if len(model_names) < 1:
            output = dict()
            for metric in metrics:
                output[metric] = None
        ...
        model = output[1].run_service("GET", "VERTEX_MODEL", model_names[0])
        return model.run_service("EVAL", "VERTEX", output[1], metrics)
