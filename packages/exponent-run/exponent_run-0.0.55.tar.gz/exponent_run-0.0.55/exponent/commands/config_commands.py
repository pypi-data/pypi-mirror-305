...
def upgrade_user_to_paid(
    ...
    """Upgrade a user to paid plan. Only available to @exponent.run admins."""
    if not settings.api_key:
        redirect_to_login(settings)
        return

    run_until_complete(
        upgrade_user_to_paid_task(
            api_key=settings.api_key,
            base_api_url=settings.base_api_url,
            email=email,
        )
...
async def upgrade_user_to_paid_task(
    api_key: str,
    base_api_url: str,
    email: str,
) -> None:
    ...
    result = await graphql_client.query(UPGRADE_USER_TO_PAID_MUTATION, variables)
    if "__typename" in result["upgradeUserToPaid"]:
        if result["upgradeUserToPaid"]["__typename"] == "Error":
            click.secho(f"Error: {result['upgradeUserToPaid']['message']}", fg="red")
        else:
            click.secho(
                f"Successfully upgraded user {result['upgradeUserToPaid']['email']} to {result['upgradeUserToPaid']['plan']} plan",
                fg="green",
            )
    else:
        click.echo(result)
