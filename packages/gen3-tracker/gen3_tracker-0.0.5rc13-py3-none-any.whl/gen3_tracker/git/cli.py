...
@click.option('--legacy-bucket', show_default=True, default=False, is_flag=True, help='Specify this flag to add files to a project on a bucket that was created with a legacy g3t client version.')
@click.option('--debug', is_flag=True)
@click.option('--skip_validate', is_flag=True, help='Skip validation of the metadata')
@click.pass_context
def push(ctx, step: str, transfer_method: str, overwrite: bool, re_run: bool, wait: bool, dry_run: bool, fhir_server: bool, debug: bool, legacy_bucket: bool):
    ...
    try:
        ...
        if fhir_server or step in ['fhir']:
            ...
            with Halo(text='Sending to FHIR Server', spinner='line', placement='right', color='white'):
                click.secho(f'Contacting {auth.endpoint}/Bundle')
            ...
            with open("logs/publish.log", 'a') as f:
                log_msg = {'timestamp': datetime.now(pytz.UTC).isoformat(), "result": f"{result}"}
                #log_msg = {'timestamp': datetime.now(pytz.UTC).isoformat(), "msg": f"{res}"}
                click.secho('Published project. See logs/publish.log', fg=SUCCESS_COLOR, file=sys.stderr)
                f.write(json.dumps(log_msg, separators=(',', ':')))
                f.write('\n')
            return
        ...
        if step in ['publish', 'all'] and not fhir_server:
            ...
            if transfer_method == 'gen3':
                with Halo(text='Publishing', spinner='line', placement='right', color='white') as spinner:
                    ...
                    _ = publish_commits(config, wait=wait, auth=auth, bucket_name=bucket_name, spinner=spinner, legacy_bucket=legacy_bucket)
                click.secho('Published project. See logs/publish.log', fg=SUCCESS_COLOR, file=sys.stderr)
                with open("logs/publish.log", 'a') as f:
