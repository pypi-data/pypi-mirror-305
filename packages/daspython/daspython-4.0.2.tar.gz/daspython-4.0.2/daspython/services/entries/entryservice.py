...
class EntryService(ApiMethods):
    ...
    def __convertToInputRequest(self, body: InsertRequest) -> InsertRequest:
        ...
        entry_fields = entryFieldService.get_all(
            body.attributeId, DisplayType.FORM)
        ...
        for key in body.entry:
            field = next(filter(lambda x: x.get('displayName')
                         == key, entry_fields.items), None)
            if (field is None):
                continue
            request.entry[field.get(
                'column')] = body.entry[field.get('displayName')]

        grouped_field = next(filter(lambda x: x.get(
            'inputType') == InputType.GroupedField.value, entry_fields.items), None)
        ...
        if not grouped_field is None:
            ...
            if not custom_data is None:
                for key in body.entry:
                    field = next(filter(lambda x: x.get('displayName')
                                 == key, custom_data.get('fields')), None)
                    if (field is None):
                        continue
                    request.entry[field.get(
                        'column')] = body.entry[field.get('displayName')]
