...
class RouterManager:
    ...
    async def get_graph(self, chat_id: str, llm, router_id: int, condition_cls: Condition = Condition,
        ...
        logger.debug(f'meta_template_id: {router.meta_template_id}')
        ...
        if router.meta_template_id != None:
            ...
            logger.debug(f'basic info: {basic_info_template.content}') # debug
            for tmp in templates:
                tmp.content = f'\n{basic_info_template.content}\n{tmp.content}'
                logger.debug(f'template info: {tmp.content}') # debug
