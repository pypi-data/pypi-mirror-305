...
class Particle(Tube):
    ...
    def _wip_feed(self, buffer, context):
        ...
        """
        for wave__, uid in self._ordered_streaming(buffer):
            stream = buffer[uid]
            data = stream.pop(0)
...
class XInterpolate(Particle):
    ...
    def _wip_analyze_data(self, data):
        keys = set([_ for _ in data if not re.match(self.EXCLUDE, _)])
        data_keys = set(
            [_ for _ in keys if isinstance(data[_], (int, float))]
        )
        ...
        for k in self.DATETIME:
            ...
            log.error(
                "can't find a datetime key from %s candidates", self.DATETIME
            )
                ...
                for key, known_idts in info.items():
                    ...
                    for parameter, data_points in data.items():
                        ...
                        if received:  # use the last found
                            ...
                            new["datetime"] = dt
                            new[MONOTONIC_KEY] = int(
                                dt.timestamp() * WAVE_FACTOR
                            )
    ...
    def _plot(self, curve, data_points, value, label):
        ...
        data_points_x, data_points_y = zip(
            *[(pt[0], pt[1]) for pt in data_points]
        )
    ...
    def _interpolate(self, idt0, idt1, key):
        pass
    ...
    async def _compute(self, edge, ekeys):
        ...
        for uid, uid_holder in edge.items():
            if re.match(REG_PRIVATE_KEY, uid):
                continue
            aggregated = result.setdefault(uid, {})
