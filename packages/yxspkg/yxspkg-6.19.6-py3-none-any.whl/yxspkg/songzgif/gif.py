...
class VideoReadThread(QThread):   
    ...
    def run(self):
        try:
            num_frames = self.videoreader.n_frames
            it = self.videoreader
            it.seek(0)
            img0 = it.copy()
            it.seek(1)
            img1 = it.copy()
            if img0.mode != img1.mode:
                img0 = img0.convert(img1.mode)
            img0 = np.array(img0)
            self.trigger.emit((img0,))
...
class GifMaker(QWidget):
    ...
    def editGif(self,filename):
        print('tttttt')
        videoreader=Image.open(filename)
        # videoreader=imageio.get_reader(filename,'ffmpeg')
        duration = videoreader.info['duration']
        fps=1000/duration
