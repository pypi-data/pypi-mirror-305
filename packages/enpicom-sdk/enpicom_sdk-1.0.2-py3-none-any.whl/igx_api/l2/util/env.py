...
from igx_api.l2.types.log import LogLevel, LogLevelModel
...
def get_api_host() -> str:
    return os.environ.get("IGX_API_HOST", "https://api.igx.bio/v1")


def get_api_key() -> str | None:
    return os.environ.get("IGX_API_KEY")


def get_api_key_or_error() -> str:
    return env_or_raise("IGX_API_KEY")


def get_event_host() -> str:
    return os.environ.get("IGX_EVENT_HOST", "igx.bio")


def get_event_port() -> int:
    return int(os.environ.get("IGX_EVENT_PORT", "443"))


def get_event_root_topic() -> str:
    # Allow to be overwritten for local development
    return os.environ.get("IGX_EVENT_ROOT_TOPIC", get_event_host())
