import os
import sys
from types import TracebackType
from typing import Self, Type

from loguru import logger

from igx_api.l1 import openapi_client
from igx_api.l2.client.api.basket_api import BasketApi
from igx_api.l2.client.api.cluster_api import ClusterApi
from igx_api.l2.client.api.collection_api import CollectionApi
from igx_api.l2.client.api.file_api import FileApi
from igx_api.l2.client.api.filter_api import FilterApi
from igx_api.l2.client.api.job_api import JobApi
from igx_api.l2.client.api.liabilities_api import LiabilitiesApi
from igx_api.l2.client.api.profile_api import ProfileApi
from igx_api.l2.client.api.reference_database_api import ReferenceDatabaseApi
from igx_api.l2.client.api.tag_api import TagApi
from igx_api.l2.client.api.track_api import TrackApi
from igx_api.l2.client.api.tree_api import TreeApi
from igx_api.l2.client.api.whoami_api import WhoamiApi
from igx_api.l2.types.log import LogLevel
from igx_api.l2.util.env import get_api_host, get_api_key, get_log_level


class ApiHostNotSet(Exception):
    """Raised when `IGX_API_HOST` is not set as an environment variable."""

    def __init__(self) -> None:
        """@private"""
        super().__init__("Must set either IGX_API_HOST environment variable.")


class ApiKeyNotSet(Exception):
    """Raised when `IGX_API_KEY` is not as an environment variable or passed as an argument to the client."""

    def __init__(self) -> None:
        """@private"""
        super().__init__("Must set either IGX_API_KEY environment variable or pass it as an argument.")


def configure_logger(level: LogLevel) -> None:
    """@private"""
    logger.remove()

    logger.add(
        sys.stdout,
        colorize=True,
        level=level,
    )


class IgxApiClient:
    """The higher level API client to comfortably interact with the IGX API.

    This class is a context manager, so it should be used with the `with` statement.

    By default, arguments are taken from the environment variables.

    Key is a required argument or the environment variable `IGX_API_KEY` must be set.

    """

    _configuration: openapi_client.Configuration
    _log_level: LogLevel
    _extra_headers: dict[str, str] | None = None
    _inner_api_client: openapi_client.ApiClient

    basket_api: BasketApi
    cluster_api: ClusterApi
    collection_api: CollectionApi
    file_api: FileApi
    filter_api: FilterApi
    job_api: JobApi
    liabilities_api: LiabilitiesApi
    profile_api: ProfileApi
    reference_database_api: ReferenceDatabaseApi
    tag_api: TagApi
    track_api: TrackApi
    tree_api: TreeApi
    whoami_api: WhoamiApi

    def __init__(self, api_key: str | None = None, log_level: LogLevel = get_log_level()):
        """Initialize the API client.

        Args:
            api_key (str | None): The API key to use. If left blank the API key will be retrieved from the
                `IGX_API_KEY` environment variable.
            log_level (LogLevel): The log level to use. Defaults to `DEBUG`.

        Raises:
            igx_api.l2.client.igx_api_client.ApiHostNotSet: When `IGX_API_HOST` env variable is not set.
            igx_api.l2.client.igx_api_client.ApiKeyNotSet: When `IGX_API_KEY` env variable is not set or passed as an argument.

        Example:

            ```python
            from igx_api.l2.client.igx_api_client import IgxApiClient

            with IgxApiClient() as igx_client:
                # Use the client here, for example, access the collection api
                collection_api = igx_client.collection_api
            ```
        """

        host = get_api_host()
        if host is None:
            raise ApiHostNotSet()

        if api_key is None:
            api_key = get_api_key()
            if api_key is None:
                raise ApiKeyNotSet()

        configure_logger(log_level)
        self._log_level = log_level
        self._configuration = openapi_client.Configuration(host=host)
        self._configuration.api_key["igxApiKey"] = api_key

    def _add_extra_headers(self, headers: dict[str, str]) -> Self:
        self._extra_headers = headers if self._extra_headers is None else {**self._extra_headers, **headers}
        return self

    def __enter__(self) -> Self:
        self._inner_api_client = openapi_client.ApiClient(self._configuration)

        if os.environ.get("USER_ID") and os.environ.get("USER_ORG_ID") and os.environ.get("USER_DEFAULT_SPACE_ID"):
            self._add_extra_headers(
                {
                    "igx-user-id": str(os.environ.get("USER_ID")),
                    "igx-user-org-id": str(os.environ.get("USER_ORG_ID")),
                    "igx-user-default-space-id": str(os.environ.get("USER_DEFAULT_SPACE_ID")),
                }
            )

        if self._extra_headers:
            for header, value in self._extra_headers.items():
                logger.trace(f"Setting header {header} to {value}")
                self._inner_api_client.set_default_header(header, value)

        if "localhost" in self._configuration.host:
            kubernetes_namespace = os.environ.get("KUBERNETES_NAMESPACE", "igx-platform")
            self._inner_api_client.set_default_header("cookie", f"igx-namespace-id={kubernetes_namespace}")

        self.basket_api = BasketApi(self._inner_api_client, self._log_level)
        self.cluster_api = ClusterApi(self._inner_api_client, self._log_level)
        self.collection_api = CollectionApi(self._inner_api_client, self._log_level)
        self.file_api = FileApi(self._inner_api_client, self._log_level)
        self.filter_api = FilterApi(self._inner_api_client, self._log_level)
        self.job_api = JobApi(self._inner_api_client, self._log_level)
        self.liabilities_api = LiabilitiesApi(self._inner_api_client, self._log_level)
        self.profile_api = ProfileApi(self._inner_api_client, self._log_level)
        self.reference_database_api = ReferenceDatabaseApi(self._inner_api_client, self._log_level)
        self.tag_api = TagApi(self._inner_api_client, self._log_level)
        self.track_api = TrackApi(self._inner_api_client, self._log_level)
        self.tree_api = TreeApi(self._inner_api_client, self._log_level)
        self.whoami_api = WhoamiApi(self._inner_api_client, self._log_level)

        return self

    def __exit__(self, exc_type: Type[BaseException] | None, exc: BaseException | None, traceback: TracebackType | None) -> None:
        self._inner_api_client.__exit__(exc_type, exc, traceback)
