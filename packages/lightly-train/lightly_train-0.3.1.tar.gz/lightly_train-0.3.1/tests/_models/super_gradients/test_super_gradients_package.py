...
if importlib.util.find_spec("super_gradients") is None:
...
class TestSuperGradientsPackage:
    ...
    def test_get_feature_extractor_cls(
        self, model_name: str, expected_cls: type
    ) -> None:
        model = models.get(model_name, num_classes=2)
        fe_cls = SuperGradientsPackage.get_feature_extractor_cls(model)
        assert fe_cls is expected_cls
