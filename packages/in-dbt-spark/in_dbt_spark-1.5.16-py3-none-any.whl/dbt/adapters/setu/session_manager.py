    ...
    def create_session(
        ...
        with cls.session_lock:
            session_initialization_config = SessionInitializationConfig(
                ...
                jars=get_jars(jars, spark_conf.pop("spark.jars.packages", None)),  # type: ignore
                ...
                spark_conf=set_spark_conf_with_defaults(spark_conf, kafka_topic, kafka_broker_url),
            ...
            with SetuClient(url=url, auth=auth, verify=verify) as client:
                ...
                return SetuSession(
                    url, session.session_id, auth, verify, kafka_topic, kafka_broker_url
                )
