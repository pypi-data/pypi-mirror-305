...
class BacktestResult:
    ...
    def plot_single(self, single_asset):
        ...
        assert single_asset in self.simulator.position.columns, f"{single_asset} should be in position"
        assert self.simulator.valuation.size != 0, "Valuation can not empty"

        non_zero_position = self.simulator.position.loc[:, self.simulator.position.sum() != 0]
        if self.simulator.position.shape == self.simulator.valuation.shape:
            ...
            try:
                ...
                non_zero_valuation = self.simulator.valuation[:, np.nansum(self.simulator.valuation, axis=0) != 0]
        ...
        add = (position_single > position_previous) & (position_single != 0) & ~entry & ~short & ~position_end
        reduce = (position_single < position_previous) & (position_single != 0) & ~entry & ~short & ~position_end

        signal = np.where(entry, 2, 
                        np.where(add, 1,
                                np.where(reduce, -1,
                                            np.where(short, -2, 
                                                    np.where(position_end, 3, 0)))))

        pnl_df = pd.DataFrame(valuation_single)
        pnl_df['signal_start'] = signal
        pnl_df['pnl_percent_start'] = (pnl_df[signal != 0][single_asset].pct_change() * 100).shift(-1)
        ...
        pnl_df['pnl_start'] = np.where(pnl_df['pnl_percent_start'].abs() == 100, 0, pnl_df['pnl_start'])
        pnl_df['pnl_start'] = np.where(pnl_df[single_asset] < 0, -pnl_df['pnl_start'], pnl_df['pnl_start'])
        pnl_df['pnl_start'] = np.where((pnl_df['signal_start'] == 3), np.nan, pnl_df['pnl_start'])
        pnl_df['pnl_percent_start'] = np.where(pnl_df['pnl_percent_start'].abs() == 100, 0, pnl_df['pnl_percent_start'])
        pnl_df['pnl_percent_end'] = np.where(pnl_df['pnl_percent_end'].abs() == 100, 0, pnl_df['pnl_percent_end'])

        pnl_df['ffill_signal'] = pnl_df.signal_start.replace(0, np.nan).fillna(method='ffill')
