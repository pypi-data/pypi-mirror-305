...
ITEM = namedtuple("freq_validation",
...
class ValidationHelper(IValidation):
    ...
    def __init__(self, model_path=None, model_info=None):
        ...
        self.end = outsample_end_date(datetime.now(),
                                      n_week=FREQ_TO_VALUE[self.model_freq].validation_end[DAYS])
    ...
    def validate_combine_getter(self, end):
        try:
            ...
            validation_start = self.times_before(end, model_range, self.exchange, self.model_freq)
            print("start: %s" % validation_start)
            start_time = datetime.now()

            start_date = self.times_before(end, model_range, self.exchange, self.model_freq)
            end_date = self.times_before(end, timedelta(days=1), self.exchange, self.model_freq)
            ...
            if test_idx[0] < expected_idx[0] or test_idx[-1] > expected_idx[-1]:
                raise ModelValidationError(
                    ModelValidationErrorMessage.index_out_of_range_error(
                        start_date, end_date, test_idx, expected_idx
                    )
                )
    ...
    def validate_start_dependency(self):
        def _getter(_end: datetime, _bef: int) -> pd.DataFrame:
            return self.__MODEL.get(
                self.times_before(_end, timedelta(days=_bef), self.exchange, self.model_freq),
    ...
    def validate_end_dependency(self):
        def _getter(_end: datetime, _bef: int) -> pd.DataFrame:
            return self.__MODEL.get(
                self.times_before(_end, timedelta(days=100), self.exchange, self.model_freq),
                self.times_before(_end, timedelta(days=_bef), self.exchange, self.model_freq),
            )
    ...
    def times_before(end, times: timedelta, exchange="krx", freq='1d'):
        def last_trading_times(end, exchange="krx"):
            ...
            return int(
                list(iter_trading_times(start, end, exchange, freq))[-1].strftime(date_format)
            )
