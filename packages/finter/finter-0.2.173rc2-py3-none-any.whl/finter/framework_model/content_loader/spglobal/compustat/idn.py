...
class CompustatLoader(Loader):
    ...
    def get_df(
        ...
        if adj:
            adj_cm_name = f"content.spglobal.compustat.cax.id-{self.__INST}-adjust_factor.1d"

            BaseAlpha.get_cm(adj_cm_name)
            adj_factor = self._load_cache(adj_cm_name, **base_args).reindex(columns=raw_col).fillna(1)

            if adj_div:
                BaseAlpha.get_cm(f"content.spglobal.compustat.cax.id-{self.__INST}-total_return_factor.1d")
