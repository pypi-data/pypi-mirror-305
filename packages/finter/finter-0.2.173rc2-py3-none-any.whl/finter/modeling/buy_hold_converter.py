...
class BuyHoldConverter:
    ...
    def __init__(self):
        self.universe_price_keys = {
            "kr_stock": "price_close",
            "us_etf": "price_close",
            "us_stock": "price_close",
            "vn_stock": "price_close",
        }
    ...
    def _get_price_data(self, universe, start, end, columns):
        ...
        """
        price_key = self.universe_price_keys.get(universe)
        if not price_key:
            raise ValueError(f"Unsupported universe: {universe}")

        cf = ContentFactory(universe, start, end)
        return cf.get_df(price_key)[columns]
