...
from typing import Optional, Type, Set, Union, Iterator, Callable, List, Tuple
...
class BaseMetaModel(metaclass=ModelClassMeta):
    ...
    def simulate(
        ...
        sim = Simulator(
            position,
            price.reindex(position.index),
            initial_cash=np.float64(initial_cash),
            buy_fee_tax=np.float64(buy_fee_tax),
            sell_fee_tax=np.float64(sell_fee_tax),
            slippage=np.float64(slippage),
        )
        sim.run()
