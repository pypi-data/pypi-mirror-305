...
class ApolloClient(object):
    ...
    def get_json_from_net(self, namespace='application'):
        url = f'{self.config_url}/configs/{self.app_id}/{self.cluster}/{namespace}?releaseKey={self.secret}&ip={self.ip}'
        try:
    ...
    def get_value(self, key, default_val=None, namespace='application'):
        try:
            ...
            apollo_logger.error(
                f"get_value has error, [key is {key}], [namespace is {namespace}], [error is {e}], ")
            return default_val
    ...
    def _update_cache_and_file(self, namespace_data, namespace='application'):
        ...
        if self._hash.get(namespace) == new_hash:
            ...
            with open(self._cache_file_path + '%s_configuration_%s.txt' % (self.app_id, namespace),
                      'w') as f:
                f.write(new_string)
        ...
        try:
            ...
            if http_code == 200:
                ...
                for entry in data:
                    ...
                    apollo_logger.info("%s has changes: notificationId=%d", namespace, n_id)
                    self._get_net_and_set_local(namespace, n_id, call_change=True)
                    return
            else:
                apollo_logger.warning('Sleep...')
        except Exception as e:
            apollo_logger.warning(str(e))
    ...
    def _listener(self):
        apollo_logger.info('start long_poll')
        while not self._stopping:
            self._long_poll()
            time.sleep(self._cycle_time)
        apollo_logger.info("stopped, long_poll")
