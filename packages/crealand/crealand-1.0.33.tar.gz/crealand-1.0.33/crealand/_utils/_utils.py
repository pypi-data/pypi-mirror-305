from typing import Any, Union, Callable, List, Tuple, Set, Dict, get_origin, get_args, Generic, TypeVar, _GenericAlias
...
def check_type(func):
            ...
            if key in parameters:
                ...
                if param.default == inspect.Parameter.empty and key in annotations:
                    ...
                    if not is_instance_of(value, annotation):
                        raise TypeError(f"关键字参数 {key} 类型不匹配，期望的类型是 {annotation}, 得到的是 {type(value)}")
    ...
    def is_instance_of(value, annotation):
        ...
        elif get_origin(annotation) in (List, Tuple, Set, Dict):
