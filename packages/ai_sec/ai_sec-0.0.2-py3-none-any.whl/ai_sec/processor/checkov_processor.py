...
from ai_sec.processor.ai_processor import \
...
class CheckovProcessor(LinterProcessor):
    ...
    def __init__(self, framework: str):
        ...
        self.ai_processor = AIProcessor(framework=self.framework) if os.getenv("OPENAI_API_KEY") else None
    ...
    def process_data(self, linter_results: Dict) -> List[Dict]:
        ...
        for issue in linter_results.get('failed_checks', []):
            ...
            issue_data = {
                ...
                'Links': ', '.join(checkov_issue.guideline if isinstance(checkov_issue.guideline, list) else [checkov_issue.guideline] if checkov_issue.guideline else ""),
                'Additional Context': checkov_issue.resource if checkov_issue.resource else ""  # Add resource under additional context
            }
