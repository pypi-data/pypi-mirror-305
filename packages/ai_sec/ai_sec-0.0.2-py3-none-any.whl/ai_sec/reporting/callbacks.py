...
def register_callbacks(app, linter_data):
        ...
        try:
            ...
            if active_cell:
                ...
                if row_index < len(table_data):
                    ...
                    context = linter_entry.get('Context', 'No additional context available.')
                    resource = linter_entry.get('Additional Context', None)  # Set to None if unavailable
                    ...
                    for link in links:
                        if link:
                            link_elements.append(
                                html.Div([
                                    html.I(className="fas fa-link", style={"marginRight": "5px"}),
                                    html.A(href=link, children=link, target="_blank", style={
                                        "color": "#3399f3",
                                        "wordWrap": "break-word"
                                    })
                    ...
                    modal_content = html.Div(
                        [
                            ...
                            html.P(f"Resource: {resource}", className="modal-resource") if resource else None,
    ...
    @app.callback(
        Output('navbar-title', 'children'),  # Assuming the Navbar title component has id 'navbar-title'
        [Input('url', 'pathname')]
    )
...
def create_sidebar(linter_data, current_path):
    ...
    links = [
        dbc.NavLink(
            linter,
            href=f"/{linter.lower()}",
            active="exact" if f"/{linter.lower()}" == current_path else "",  # Mark active based on URL path
            style={
                "fontSize": "16px",
                "padding": "12px 20px",
                "borderRadius": "6px",
                "textAlign": "center",
                "color": "#ffffff" if f"/{linter.lower()}" == current_path else "#b0bec5",  # Active link color
                "backgroundColor": "#0056b3" if f"/{linter.lower()}" == current_path else "transparent",  # Active link background color
        ...
        [
            ...
            html.Div(
                [
                    ...
                    html.H2("AI_SEC", className="sidebar-title", style={
                        "fontSize": "22px",
                        "fontWeight": "bold",
            ...
            html.Div(
                dbc.NavLink(
                    html.Img(src="/assets/home-solid.svg", style={"width": "30px", "height": "auto"}),
                    href="/",
                    active="exact" if current_path == "/" else "",  # Home link active check
                    className="home-link",
                    style={"textAlign": "center", "marginBottom": "20px"}
                )
            ),
            html.Hr(style={"border": "1px solid #E0E0E0", "width": "80%", "margin": "auto"}),  # Divider

            # Navigation links for Linters
            dbc.Nav(links, vertical=True, pills=True, style={
                "paddingTop": "10px",
                "paddingLeft": "10px",
                "paddingRight": "10px",
                "textAlign": "center"
