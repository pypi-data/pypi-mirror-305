...
def adjust_paths_generic(result_dict, base_directory):
    ...
    for result in result_dict.get('linters', {}).get('tfsec', {}).get('results', []):
        if 'location' in result and 'filename' in result['location']:
            result['location']['filename'] = normalize_path(result['location']['filename'])
...
def get_relative_path(full_path, base_directory):
    ...
    try:
        ...
        if relative_path.startswith(".."):
            ...
            logger.warning(f"Path {relative_path} goes outside the base directory, returning original path.")
            return full_path

        logger.info(f"Relative path: {relative_path}")
        return relative_path
    except Exception as e:
        logger.error(f"Error getting relative path for '{full_path}' with base directory '{base_directory}': {e}")
