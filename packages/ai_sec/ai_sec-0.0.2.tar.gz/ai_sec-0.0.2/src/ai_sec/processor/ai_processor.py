...
class AIProcessor:
    ...
    def process_linter_issue(self, issue_data: Dict) -> Dict:
        ...
        if severity == "UNKNOWN" and self.ai_handler:
            inferred_severity = self.ai_handler.infer_severity(issue_data["Description"], self.framework)
        ...
        if issue_data["Severity"] in ["CRITICAL", "HIGH", "MEDIUM"] and self.ai_handler:
            issue_data["Context"] = self.ai_handler.generate_resolution_context(issue_data["Description"], self.framework)
