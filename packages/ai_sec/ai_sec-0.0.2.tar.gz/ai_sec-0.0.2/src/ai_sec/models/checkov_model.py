...
class CheckovResult(BaseModel):
    ...
    def _parse_checks(checks: List[dict], result_type: str) -> List[CheckovIssue]:
        ...
        for check in checks:
            ...
            line_number_start, line_number_end = file_line_range if len(file_line_range) == 2 else (None, None)

            parsed_checks.append(CheckovIssue(
                check_id=check.get('check_id', ""),  # Default to empty string if None
                file_path=check.get('file_path', ""),  # Default to empty string if None
