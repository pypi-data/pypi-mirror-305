...
def run_linters_and_generate_report(config: Dict, directory: str) -> str:
    ...
    if not framework:
        ...
        raise ValueError("Framework not detected. Ensure that the infrastructure type is detected and set correctly.")
    ...
    for linter_name, LinterClass, ResultModel, ProcessorClass in linters:
        ...
        if parsed_json_result and 'error' not in parsed_json_result:
            try:
                ...
                parsed_result = ResultModel.from_raw_json(json.dumps(parsed_json_result))
            ...
            logger.error(f"{linter_name} returned an error: {parsed_json_result.get('error') if parsed_json_result else 'No valid result'}")
            results['linters'][linter_name] = {'error': parsed_json_result.get('error') if parsed_json_result else 'No valid result'}
