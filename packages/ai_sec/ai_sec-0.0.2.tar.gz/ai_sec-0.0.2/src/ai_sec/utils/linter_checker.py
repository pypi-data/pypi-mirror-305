...
def check_linter_installed(linter_name, version_command=['--version']):
    ...
    if linter_path is None:
        logger.error(f"{linter_name} is not installed. Please install it to use this tool.")
        return False

    try:
        # Run the version command safely without shell=True
        subprocess.run([linter_name] + version_command, capture_output=True, check=True)  # nosec B603
        ...
        logger.error(f"{linter_name} not found. Please ensure it is properly installed.")
        return False
