...
SUPPORTED_CLOUDFORMATION_FILES = ('.json', '.yaml', '.yml')  # CloudFormation can be YAML or JSON
SUPPORTED_KUBERNETES_FILES = ('.yaml', '.yml')  # Kubernetes manifests are typically YAML
        ...
        for file in files:
            ...
            try:
                ...
                if file.endswith(SUPPORTED_TERRAFORM_FILES):
                    if any(keyword in content for keyword in ['"terraform"', 'resource', 'provider']):
