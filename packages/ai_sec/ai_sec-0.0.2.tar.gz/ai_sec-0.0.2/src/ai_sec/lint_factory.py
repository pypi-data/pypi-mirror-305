...
class LinterFactory:
    ...
    SUPPORTED_FRAMEWORKS = {
        ...
        'tfsec': ['terraform', 'cloudformation'],  # TFSec supports Terraform and CloudFormation
        'checkov': ['terraform', 'cloudformation', 'kubernetes'],  # Checkov supports all three
    }
    ...
    def get_enabled_linters(config: Dict) -> list:
        ...
        for linter_name, (LinterClass, ResultModel, ProcessorClass) in LinterFactory.LINTER_MAP.items():
            ...
            if config['linters'].get(linter_name, {}).get('enabled', False):
                if infra_type in LinterFactory.SUPPORTED_FRAMEWORKS.get(linter_name, []):
                    enabled_linters.append((linter_name, LinterClass, ResultModel, ProcessorClass))
                else:
                    logger.debug(f"Linter {linter_name} does not support framework {infra_type}. Skipping...")
