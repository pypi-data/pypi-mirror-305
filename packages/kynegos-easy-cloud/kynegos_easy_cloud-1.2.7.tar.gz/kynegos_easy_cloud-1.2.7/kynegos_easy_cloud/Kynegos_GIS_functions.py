...
def process_gdf_for_bigquery(gdf):
    ...
    gdf = gdf.to_crs("EPSG:4326")
    gdf = gdf[~gdf['geometry'].is_empty & gdf['geometry'].notnull() & gdf['geometry'].apply(lambda x: x is not None)]
    gdf['geometry'] = gdf['geometry'].apply(lambda geom: orient(geom, sign=1.0))
    gdf['geometry'] = gdf['geometry'].apply(make_valid)

    return gdf

def descargar_wfs(filename_temp, wfs_url, request_type, version, output_format, swap_xy = False):
    """
