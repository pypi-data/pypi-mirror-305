...
async def invoke_function(function, tool, openai_function):
    try:
        ...
        print(f"invoke_function Error invoking function {tool.function.name}: {e}")
...
async def process_agent_request(row_batch: List[Dict], workflow: Dict, custom_instructions: str) -> List[Dict]:
    ...
    try:
        ...
        print(f"process_agent_request process_agent_request An error occurred: {e}")
                        ...
                        with open(local_path, 'w') as file:
                            if output.get("format", "") == 'list':
                                ...
                                if data_list:
                                    ...
                                    print(f"Deduped and removed count: {len(data_list)}")
...
def remove_empty_property_rows(data_list, properties):
    """
    Remove rows with empty values for the specified properties.
    """
    filtered_list = []
    for item in data_list:
        empty = False
        for property_name in properties:
            value = item.get(property_name, "").strip()
            if value == "":
                empty = True
                break
        if not empty:
...
def deduplicate_list_by_properties(data_list, properties):
    ...
    Only deduplicate if the property value is not empty, strip spaces, and compare in lowercase.
    """
    for property_name in properties:
        seen = set()
        deduplicated_list = []
        for item in data_list:
            value = item.get(property_name, "").strip().lower()
            if value == "":
