...
def minimize_window():
    ...
    window = gw.getWindowsWithTitle("Command Prompt")  # Adjust if your terminal has a different title
    if window:
        window[0].minimize()
    ...
    while True:
        ...
        if user_input.lower() == "screenshot":
            try:
                # Minimize the console window
                minimize_window()
                time.sleep(1)  # Allow time for the window to minimize
            ...
            # Generate response from Gemini for other inputs
