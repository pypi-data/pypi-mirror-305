...
from ...utils import SKIP_ENV_GRAPH_MODE, set_mode_by_env_config, SKIP_ENV_CPU
set_mode_by_env_config()

def _test_Transformer_cell_smoke(batch_first, src_size, tgt_size):
    d_model = 64
    bsz = 8
    nhead = 16
...
# TODO: use parameter names to identify keyword args and delete all None args
# after graph mode bug fixed
def test_Transformer_cell1():
    ...
    # seq_length = 25
    # tgt_length = 15
    _test_Transformer_cell_smoke(True, (8, 25, 64), (8, 15, 64))

def test_Transformer_cell2():
    _test_Transformer_cell_smoke(False, (25, 8, 64), (15, 8, 64))

def _get_model_for_transformerdecoderlayer(batch_first):
    d_model = 4
    nhead = 2
    dim_feedforward = 16
    dropout = 0.0
    ...
    for idx, p in enumerate(model.parameters()):
        ...
        p.data = p.data.copy_adapter(x)
    return model

def _test_transformerdecoderlayer(batch_first):
    model = _get_model_for_transformerdecoderlayer(batch_first)
    ...
    # deterministic input
    decoder_input = _perm_fn(ms_torch.tensor([[[9., 10., 11., 12.]],
                                            [[11., 12., 13., 14.]]]), batch_first)
    memory_input = ms_torch.tensor([[[1., 2., 3., 4.]]])
    result = model(decoder_input, memory_input)
    result = result.detach().numpy()
    ref_output = _perm_fn(ms_torch.tensor([[[2.422245, 0.051716, -0.606338, -0.024756]],
                                        [[2.422245, 0.051716, -0.606338, -0.024756]]]), batch_first)
    ref_output = ref_output.detach().numpy()
    assert tuple(result.shape) == tuple(ref_output.shape)
    np.testing.assert_allclose(result, ref_output, atol=1e-5)

    # deterministic input
    decoder_input = _perm_fn(ms_torch.tensor([[[1., 2., 3., 4.]],
                                            [[5., 6., 7., 8.]]]), batch_first)
    memory_input = _perm_fn(ms_torch.tensor([[[9., 10., 11., 12.]],
                                            [[11., 12., 13., 14.]]]), batch_first)
    result = model(decoder_input, memory_input)
    ref_output = _perm_fn(ms_torch.tensor([[[2.343536, 0.085561, -0.654954, 0.074991]],
                                        [[2.343536, 0.085561, -0.654954, 0.074991]]]), batch_first)
    result = result.detach().numpy()
    ref_output = ref_output.detach().numpy()
    assert tuple(result.shape) == tuple(ref_output.shape)
    np.testing.assert_allclose(result, ref_output, atol=1e-5)

def _get_decoder_memory_input_for_transformerdecoderlayer(batch_first):
    decoder_input = _perm_fn(ms_torch.tensor([[[0.4517, 0.6793, 0.5313, 0.0034],
                                            [0.2678, 0.3677, 0.4459, 0.7166]],
                                            [[0.8100, 0.3716, 0.4096, 0.1976],
                                            [0.6958, 0.8844, 0.6081, 0.8315]],
                                            [[0.0494, 0.9343, 0.5955, 0.3830],
                                            [0.5404, 0.3464, 0.9378, 0.6200]]]), batch_first)
    memory_input = _perm_fn(ms_torch.tensor([[[0.7462, 0.6653, 0.5679, 0.4891],
                                            ...
                                            [0.3718, 0.4945, 0.9511, 0.0864]]]), batch_first)
    return (decoder_input, memory_input)

def _test_transformerdecoderlayer_with_keypaddingmask(batch_first):
    model = _get_model_for_transformerdecoderlayer(batch_first)
    decoder_input, memory_input = _get_decoder_memory_input_for_transformerdecoderlayer(batch_first)
    # deterministic input
    result = model(decoder_input, memory_input)
    ref_output = _perm_fn(ms_torch.tensor([[[2.430065, 0.027862, -0.601136, -0.073096],
                                        ...
                                        [2.432306, 0.028858, -0.599542, -0.072846]]]), batch_first)
    result = result.detach().numpy()
    ref_output = ref_output.detach().numpy()
    assert tuple(result.shape) == tuple(ref_output.shape)
    np.testing.assert_allclose(result, ref_output, atol=1e-5)
    ...
    result = model(decoder_input, memory_input, None, None, key_padding_mask)
    ref_output = _perm_fn(ms_torch.tensor([[[2.430065, 0.027862, -0.601136, -0.073096],
                                        [2.431935, 0.028907, -0.599809, -0.072488]],
                                        [[2.428457, 0.027053, -0.602275, -0.073462],
                                        [2.431970, 0.029387, -0.599789, -0.071621]],
    ...
    result = model(decoder_input, memory_input, None, None, key_padding_mask)
    ref_output = _perm_fn(ms_torch.tensor([[[2.430025, 0.027643, -0.601164, -0.073476],
                                        ...
                                        [2.432659, 0.029244, -0.599294, -0.072382]]]), batch_first)
    result = result.detach().numpy()
    ref_output = ref_output.detach().numpy()
    assert tuple(result.shape) == tuple(ref_output.shape)
...
def _test_transformerdecoderlayer_with_memorykeypaddingmask(batch_first):
    model = _get_model_for_transformerdecoderlayer(batch_first)
    decoder_input, memory_input = _get_decoder_memory_input_for_transformerdecoderlayer(batch_first)
    # memory_key_padding_mask
    key_padding_mask = ms_torch.zeros(2, 5) == 1
    result = model(decoder_input, memory_input, None, None, None, key_padding_mask)
    ref_output = _perm_fn(ms_torch.tensor([[[2.430065, 0.027862, -0.601136, -0.073096],
                                        [2.431935, 0.028907, -0.599809, -0.072488]],
                                        [[2.428457, 0.027053, -0.602275, -0.073462],
                                        [2.431970, 0.029387, -0.599789, -0.071621]],
    ...
    result = model(decoder_input, memory_input, None, None, None, key_padding_mask)
    ref_output = _perm_fn(ms_torch.tensor([[[2.429757, 0.027358, -0.601351, -0.073816],
                                        ...
                                        [2.433075, 0.028543, -0.598987, -0.073985]]]), batch_first)
    result = result.detach().numpy()
    ref_output = ref_output.detach().numpy()
    assert tuple(result.shape) == tuple(ref_output.shape)
    # TODO: check with lower tolerance
...
def test_transformerdecoderlayer_with_keypaddingmask_batchfirst():
    _test_transformerdecoderlayer_with_keypaddingmask(True)

def test_transformerdecoderlayer_with_keypaddingmask():
    _test_transformerdecoderlayer_with_keypaddingmask(False)

def test_transformerdecoderlayer_with_memorykeypaddingmask_batchfirst():
    _test_transformerdecoderlayer_with_memorykeypaddingmask(True)

def test_transformerdecoderlayer_with_memorykeypaddingmask():
    _test_transformerdecoderlayer_with_memorykeypaddingmask(False)

def test_transformerdecoderlayer_batchfirst():
    _test_transformerdecoderlayer(True)

def test_transformerdecoderlayer():
    _test_transformerdecoderlayer(False)

def _test_transformerdecoderlayer_gelu(activation, batch_first):
    d_model = 4
    nhead = 2
    dim_feedforward = 16
    dropout = 0.0
    ...
    decoder_input = _perm_fn(ms_torch.tensor([[[9., 10., 11., 12.]],
                                            [[11., 12., 13., 14.]]]), batch_first)
    memory_input = _perm_fn(ms_torch.tensor([[[1., 2., 3., 4.]]]), batch_first)
    result = model(decoder_input, memory_input)
    ref_output = _perm_fn(ms_torch.tensor([[[2.415448, 0.054389, -0.610932, -0.0156613]],
                                        [[2.415448, 0.054389, -0.610932, -0.0156613]]]), batch_first)
    # TODO: check with lower tolerance
    # np.testing.assert_allclose(result.numpy(), ref_output.numpy(), rtol=1e-5, atol=0)
    np.testing.assert_allclose(result.numpy(), ref_output.numpy(), rtol=1e-3)
    ...
    memory_input = _perm_fn(ms_torch.tensor([[[9., 10., 11., 12.]],
                                            [[11., 12., 13., 14.]]]), batch_first)
    result = model(decoder_input, memory_input)
    ref_output = _perm_fn(ms_torch.tensor([[[2.338531, 0.087709, -0.65776, 0.080646]],
                                        [[2.338531, 0.087709, -0.65776, 0.080646]]]), batch_first)
    # TODO: check with lower tolerance
    # np.testing.assert_allclose(result.numpy(), ref_output.numpy(), rtol=1e-5, atol=0)
    np.testing.assert_allclose(result.numpy(), ref_output.numpy(), rtol=1e-3)
    ...
    memory_input = _perm_fn(ms_torch.tensor([[[0.7462, 0.6653, 0.5679, 0.4891],
                                            ...
                                            [0.3718, 0.4945, 0.9511, 0.0864]]]), batch_first)
    result = model(decoder_input, memory_input)
    ref_output = _perm_fn(ms_torch.tensor([[[2.42049104, 0.03443088, -0.60793706, -0.05436271],
                                        ...
                                        [2.42240309, 0.0354595, -0.60659063, -0.05378816]]]), batch_first)
    # TODO: check with lower tolerance
    # np.testing.assert_allclose(result.numpy(), ref_output.numpy(), rtol=1e-5, atol=0)
    np.testing.assert_allclose(result.numpy(), ref_output.numpy(), rtol=1e-3)

def test_transformerdecoderlayer_gelu_batchfirst():
    _test_transformerdecoderlayer_gelu('gelu', True)

def test_transformerdecoderlayer_gelu():
    _test_transformerdecoderlayer_gelu(nn.GELU(), False)

def _get_a_test_layer_for_transformerencoder(use_cuda, activation, batch_first=False):
    d_model = 4
    nhead = 2
    dim_feedforward = 16
    dropout = 0.0
...
def _test_transformerencoder(batch_first, training):
    activation = F.relu
    use_cuda = ms_torch.cuda.is_available()
    device = ms_torch.device("cuda" if use_cuda else "cpu")

    encoder_layer = _get_a_test_layer_for_transformerencoder(use_cuda=use_cuda, activation=activation,
                                        batch_first=batch_first)
    ...
    # deterministic input
    encoder_input = _perm_fn(ms_torch.tensor([[[0.7462, 0.6653, 0.5679, 0.4891],
                                            [0.5387, 0.1655, 0.3565, 0.0471]],
                                            [[0.8335, 0.2799, 0.5031, 0.2947],
                                            [0.1402, 0.0318, 0.7636, 0.1346]],
                                            [[0.6333, 0.9344, 0.1376, 0.9938],
                                            ...
                                            ), batch_first).to(device)
    result = model(encoder_input)
    ref_output = _perm_fn(ms_torch.tensor([[[2.428589, 0.020835, -0.602055, -0.085249],
    ...
    result = model(encoder_input, None, mask)
    assert tuple(result.shape) == tuple(ref_output.shape)
    np.testing.assert_allclose(result.numpy(), ref_output.numpy(), rtol=1e-7, atol=1e-5)
    mask[0, 1] = 1
    mask[1, 3] = 1
    ...
    result = model(encoder_input, None, mask)
    ref_output = _perm_fn(ms_torch.tensor([[[2.429026, 0.020793, -0.601741, -0.085642],
    ...
    if not training:
        model = model.eval()
    result = model(encoder_input, None, mask)
    ref_output = _perm_fn(ms_torch.tensor([[[2.419051, 0.017446, -0.608738, -0.085003],
    ...
    model = nn.TransformerEncoder(encoder_layer, 4, enable_nested_tensor=False).to(device)
    if not training:
        model = model.eval()
    result = model(encoder_input, None, mask)
    ref_output = _perm_fn(ms_torch.tensor([[[2.419101, 0.017453, -0.608703, -0.085025],
    ...
    if not training:
        model = model.eval()
    result = model(encoder_input, None, mask)
    ref_output = _perm_fn(ms_torch.tensor([[[1.695949, -0.357635, -0.893077, -0.445238],
    ...
    model = nn.TransformerEncoder(encoder_layer, 4, norm=norm, enable_nested_tensor=False).to(device)
    if not training:
        model = model.eval()
    result = model(encoder_input, None, mask)
    ref_output = _perm_fn(ms_torch.tensor([[[1.695955, -0.357639, -0.893051, -0.445265],
...
def test_transformerencoder_batchfirst_training():
    _test_transformerencoder(True, True)

def test_transformerencoder_batchfirst():
    _test_transformerencoder(True, False)

def test_transformerencoder_training():
    _test_transformerencoder(False, True)

def test_transformerencoder():
    _test_transformerencoder(False, False)

def _get_a_test_layer_for_transformerdecoder(use_cuda, activation, batch_first=False):
    d_model = 4
    nhead = 2
    dim_feedforward = 16
    dropout = 0.0
...
def _test_transformerdecoder(batch_first):
    activation = F.relu
    use_cuda = ms_torch.cuda.is_available()
    device = ms_torch.device("cuda" if use_cuda else "cpu")

    decoder_layer = _get_a_test_layer_for_transformerdecoder(use_cuda=use_cuda, activation=activation,
                                        batch_first=batch_first)
    ...
    # deterministic input
    decoder_input = _perm_fn(ms_torch.tensor([[[9., 10., 11., 12.]],
                                            [[11., 12., 13., 14.]]]), batch_first).to(device)
    memory_input = _perm_fn(ms_torch.tensor([[[1., 2., 3., 4.]]]), batch_first).to(device)
    result = model(decoder_input, memory_input)
    ref_output = _perm_fn(ms_torch.tensor([[[2.422245, 0.051716, -0.606338, -0.024756]],
...
def test_transformerdecoder_batchfirst():
    _test_transformerdecoder(True)

def test_transformerdecoder():
    _test_transformerdecoder(False)

def _get_model_and_device_for_transformerdecoder(batch_first):
    activation = F.relu
    use_cuda = ms_torch.cuda.is_available()
    device = ms_torch.device("cuda" if use_cuda else "cpu")
...
def _get_decode_and_memory_input_for_transformerdecoder(batch_first, device):
    # deterministic input
    decoder_input = _perm_fn(ms_torch.tensor([[[0.4517, 0.6793, 0.5313, 0.0034],
                                            [0.2678, 0.3677, 0.4459, 0.7166]],
                                            [[0.8100, 0.3716, 0.4096, 0.1976],
...
def _perm_fn(x, batch_first):
    return x.transpose(1, 0) if batch_first else x

def _test_transformerdecoder_with_keypaddingmask(batch_first):

    model, device = _get_model_and_device_for_transformerdecoder(batch_first)
    decoder_input, memory_input = _get_decode_and_memory_input_for_transformerdecoder(batch_first, device)
    ...
    result = model(decoder_input, memory_input, None, None,
                    key_padding_mask)
    ref_output = _perm_fn(ms_torch.tensor([[[2.430065, 0.027862, -0.601136, -0.073096],
                                        [2.431935, 0.028907, -0.599809, -0.072488]],
                                        [[2.428457, 0.027053, -0.602275, -0.073462],
...
def _test_transformerdecoder_with_memorykeypaddingmask(batch_first):
    model, device = _get_model_and_device_for_transformerdecoder(batch_first)
    decoder_input, memory_input = _get_decode_and_memory_input_for_transformerdecoder(batch_first, device)

    # memory_key_padding_mask
    key_padding_mask = ms_torch.zeros(2, 5).to(device) == 1
    result = model(decoder_input, memory_input, None, None, None,
                    key_padding_mask)
    ref_output = _perm_fn(ms_torch.tensor([[[2.430065, 0.027862, -0.601136, -0.073096],
                                        [2.431935, 0.028907, -0.599809, -0.072488]],
                                        [[2.428457, 0.027053, -0.602275, -0.073462],
...
def test_transformerdecoder_with_keypaddingmask_batchfirst():
    _test_transformerdecoder_with_keypaddingmask(True)

def test_transformerdecoder_with_keypaddingmask():
    _test_transformerdecoder_with_keypaddingmask(False)

def test_transformerdecoder_with_memorykeypaddingmask_batchfirst():
    _test_transformerdecoder_with_memorykeypaddingmask(True)

def test_transformerdecoder_with_memorykeypaddingmask():
    _test_transformerdecoder_with_memorykeypaddingmask(False)

def _test_transformerdecoder_multiple_layers(batch_first):
    activation = F.relu
    use_cuda = ms_torch.cuda.is_available()
    device = ms_torch.device("cuda" if use_cuda else "cpu")
    ...
    # multiple layers no norm
    model = nn.TransformerDecoder(decoder_layer, 3).to(device)
    ...
    result = model(decoder_input, memory_input)
    ref_output = _perm_fn(ms_torch.tensor([[[2.42794, 0.026164, -0.60263, -0.0747591],
...
def _test_transformerdecoder_multiple_layers_with_norm(batch_first):
    activation = F.relu
    use_cuda = ms_torch.cuda.is_available()
    device = ms_torch.device("cuda" if use_cuda else "cpu")
    ...
    model = nn.TransformerDecoder(decoder_layer, 3, norm=norm).to(device)
    ...
    result = model(decoder_input, memory_input)
    ref_output = _perm_fn(ms_torch.tensor([[[1.69559, -0.357291, -0.894741, -0.443553],
...
def test_transformerdecoder_multiple_layers_batchfirst():
    _test_transformerdecoder_multiple_layers(True)

def test_transformerdecoder_multiple_layers_with_norm_batchfirst():
    _test_transformerdecoder_multiple_layers_with_norm(True)

def test_transformerdecoder_multiple_layers():
    _test_transformerdecoder_multiple_layers(False)

def test_transformerdecoder_multiple_layers_with_norm():
    _test_transformerdecoder_multiple_layers_with_norm(False)

def _test_transformerdecoder_gelu(batch_first):
    activation = "gelu"
    use_cuda = ms_torch.cuda.is_available()
    device = ms_torch.device("cuda" if use_cuda else "cpu")
    ...
    result = model(decoder_input, memory_input)
    ref_output = _perm_fn(ms_torch.tensor([[[2.415448, 0.054389, -0.610932, -0.0156613]],
                                        [[2.415448, 0.054389, -0.610932, -0.0156613]]]), batch_first).to(device)
    assert tuple(result.shape) == tuple(ref_output.shape)
    np.testing.assert_allclose(result.numpy(), ref_output.numpy(), rtol=1e-7, atol=1e-4)
    ...
    result = model(decoder_input, memory_input)
    ref_output = _perm_fn(ms_torch.tensor([[[2.338531, 0.087709, -0.65776, 0.080646]],
                                        [[2.338531, 0.087709, -0.65776, 0.080646]]]), batch_first).to(device)
    assert tuple(result.shape) == tuple(ref_output.shape)
    np.testing.assert_allclose(result.numpy(), ref_output.numpy(), rtol=1e-7, atol=1e-4)
...
def test_transformerdecoder_gelu_batchfirst():
    _test_transformerdecoder_gelu(True)

def test_transformerdecoder_gelu():
    _test_transformerdecoder_gelu(False)

# TODO: forward func in TransformerEncoderLayer can't take keyword arguments in GRAPH MODE
# need mindspore to fix this bug
...
def _test_transformerencoderlayer_gelu(activation, batch_first, training):
    d_model = 4
    nhead = 2
    dim_feedforward = 16
    dropout = 0.0
    ...
    # deterministic input
    encoder_input = _perm_fn(ms_torch.tensor([[[1., 2., 3., 4.]],
                                            [[5., 6., 7., 8.]]], device='cpu', dtype=ms_torch.float), batch_first)
    result = model(encoder_input)
    ref_output = _perm_fn(ms_torch.tensor([[[2.264103, 0.121417, -0.696012, 0.159724]],
                                        [[2.264103, 0.121417, -0.696012, 0.159724]]],
                                          device='cpu', dtype=ms_torch.float), batch_first)
    # TODO: check with lower tolerance
    # np.testing.assert_allclose(result.numpy(), ref_output.numpy(), rtol=rtol, atol=atol)
    np.testing.assert_allclose(result.numpy(), ref_output.numpy(), atol=1e-3)
    ...
    encoder_input = _perm_fn(ms_torch.tensor([[[0.7462, 0.6653, 0.5679, 0.4891],
                             ...
                             batch_first)
    result = model(encoder_input)
    ref_output = _perm_fn(ms_torch.tensor([[[2.42163188, 0.03227153, -0.60714219, -0.05908082],
...
# deterministic tests for TransformerEncoderLayer with gelu activation
# @dtypesIfCUDA(torch.half, torch.float)
def test_transformerencoderlayer_gelu_batchfirst_training():
    _test_transformerencoderlayer_gelu(activation='gelu', batch_first=True, training=True)

def test_transformerencoderlayer_gelu_batchfirst():
    _test_transformerencoderlayer_gelu(activation=F.gelu, batch_first=True, training=False)

def test_transformerencoderlayer_gelu_training():
    _test_transformerencoderlayer_gelu(activation=nn.GELU(), batch_first=False, training=True)

def test_transformerencoderlayer_gelu():
    _test_transformerencoderlayer_gelu(activation=nn.GELU(), batch_first=False, training=False)
...
if __name__ == '__main__':
    ...
    test_transformerdecoderlayer_with_keypaddingmask_batchfirst()
    test_transformerdecoderlayer_with_keypaddingmask()
    test_transformerdecoderlayer_batchfirst()
    test_transformerdecoderlayer()

    test_transformerdecoderlayer_gelu_batchfirst()
    test_transformerdecoderlayer_gelu()

    test_transformerencoder_batchfirst_training()
    test_transformerencoder_batchfirst()
    test_transformerencoder_training()
    test_transformerencoder()

    test_transformerdecoder_batchfirst()
    test_transformerdecoder()
    test_transformerdecoder_with_keypaddingmask_batchfirst()
    test_transformerdecoder_with_keypaddingmask()
    test_transformerdecoder_multiple_layers_batchfirst()
    test_transformerdecoder_multiple_layers()
    test_transformerdecoder_gelu_batchfirst()
    test_transformerdecoder_gelu()
    ...
    test_transformerencoderlayer_gelu_batchfirst_training()
    test_transformerencoderlayer_gelu_batchfirst()
    test_transformerencoderlayer_gelu_training()
