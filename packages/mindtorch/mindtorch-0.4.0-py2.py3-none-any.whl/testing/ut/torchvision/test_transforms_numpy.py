...
import mindtorch.torchvision.transforms._pil_constants as _pil_constants
import mindtorch.torchvision.transforms.functional as F
import mindtorch.torchvision.transforms.functional_tensor as F_t
...
class TestConvertImageDtype:
    ...
    def test_float_to_int(self, input_dtype, output_dtype):
        #TODO: currently has problem handling boundary values
        if input_dtype == torch.float64 and output_dtype == torch.int32:
            pytest.skip(reason="currently has problem handling boundary values")
        input_image = torch.tensor((0.0, 1.0), dtype=input_dtype)
        transform = transforms.ConvertImageDtype(output_dtype)
        # transform_script = torch.jit.script(F.convert_image_dtype)
    ...
    @pytest.mark.skip(reason="currently devide_floor has problem handling boundary values")
    ...
    def test_int_to_int_consistency(self, input_dtype, output_dtype):
        #TODO: currently has problem handling boundary values
        if input_dtype == torch.int32 and output_dtype == torch.int64:
            pytest.skip(reason="currently has problem handling boundary values")
        input_max = torch.iinfo(input_dtype).max
        input_image = torch.tensor((0, input_max), dtype=input_dtype)
...
class TestToTensorV2:
    @pytest.mark.parametrize("channels", [1, 3, 4])
    def test_to_tensor(self, channels):
        height, width = 4, 4
        trans = transforms.ToTensorV2()
        ...
        assert np.allclose(output, input_data.numpy())
        ...
        assert np.allclose(input_data.numpy(), output)
    ...
    def test_pil_to_tensor(self, channels):
        height, width = 4, 4
        trans = transforms.PILToTensorV2()
        np_rng = np.random.RandomState(0)
        ...
        output = trans(img)  # HWC -> HWC
        expected_output = (input_data * 255).permute(1, 2, 0).byte()
        # torch.testing.assert_close(output, expected_output)
        assert np.allclose(output, expected_output.numpy())
        ...
        input_data = torch.ByteTensor(1, height, width)
        img = transforms.ToPILImage()(input_data.mul(255)).convert("1")
        output = torch.from_numpy(trans(img)).to(torch.uint8).bool().to(torch.uint8)
        # torch.testing.assert_close(input_data, output)
...
class TestPad:
    ...
    def test_pad_with_non_constant_padding_modes(self):
        ...
        assert torch.from_numpy(transforms.PILToTensorV2()(edge_padded_img)).size() == (35, 35, 3)
        ...
        assert torch.from_numpy(transforms.PILToTensorV2()(reflect_padded_img)).size() == (33, 35, 3)
        ...
        assert torch.from_numpy(transforms.PILToTensorV2()(symmetric_padded_img)).size() == (32, 34, 3)
        ...
        assert torch.from_numpy(transforms.PILToTensorV2()(symmetric_padded_img_neg)).size() == (28, 31, 3)
...
class TestToPil:
    ...
    def test_1_channel_tensor_to_pil_image(self, with_mode, img_data, expected_output, expected_mode):
        ...
        to_tensor = transforms.ToTensorV2()

        img = transform(img_data)
        assert img.mode == expected_mode
        # torch.testing.assert_close(expected_output, to_tensor(img).numpy())
        assert np.allclose(expected_output, torch.from_numpy(to_tensor(img)).numpy())

    def test_1_channel_float_tensor_to_pil_image(self):
        img_data = torch.Tensor(1, 4, 4).uniform_adapter()
    ...
    @pytest.mark.parametrize(
        ...
        [
            (torch.Tensor(4, 4, 1).uniform_adapter().numpy(), "F"),
    ...
    def test_2_channel_tensor_to_pil_image(self, expected_mode):
        img_data = torch.Tensor(2, 4, 4).uniform_adapter()
    ...
    def test_2d_tensor_to_pil_image(self, with_mode, img_data, expected_output, expected_mode):
        ...
        assert np.allclose(expected_output, torch.from_numpy(to_tensor(img)).permute(2,0,1).numpy()[0])

    @pytest.mark.parametrize("with_mode", [False, True])
    @pytest.mark.parametrize(
        "img_data, expected_mode",
        [
            (torch.Tensor(4, 4).uniform_adapter().numpy(), "F"),
    ...
    def test_3_channel_tensor_to_pil_image(self, expected_mode):
        img_data = torch.Tensor(3, 4, 4).uniform_adapter()
        expected_output = img_data.mul(255).int().float().div(255)
    ...
    def test_3_channel_tensor_to_pil_image_error(self):
        ...
        with pytest.raises(ValueError, match=r"pic should be 2/3 dimensional. Got \d+ dimensions."):
            transforms.ToPILImage()(torch.Tensor(1, 3, 4, 4).uniform_adapter())
    ...
    def test_4_channel_tensor_to_pil_image(self, expected_mode):
        img_data = torch.Tensor(4, 4, 4).uniform_adapter()
        expected_output = img_data.mul(255).int().float().div(255)
...
def test_ten_crop(should_vflip, single_dim):
    ...
    img = to_pil_image(torch.FloatTensor(3, h, w).uniform_adapter())
...
def test_five_crop(single_dim):
    ...
    img = torch.FloatTensor(3, h, w).uniform_adapter()
...
def test_random_crop():
    ...
    result = transforms.Compose(
        [transforms.ToPILImage(), transforms.RandomCrop((height, width)), transforms.PILToTensorV2()]
    )(img)
    result = torch.from_numpy(result)
...
def test_center_crop_2(odd_image_size, delta, delta_width, delta_height):
    ...
    output_pil = transforms.Compose(
        [transforms.ToPILImage(), transforms.CenterCrop(crop_size), transforms.PILToTensorV2()],
    )(img)
    output_pil = torch.from_numpy(output_pil)
    ...
    def _test_transformation(self, angle, translate, scale, shear, pil_image, input_img, center=None):
        ...
        for y in range(true_result.shape[0]):
            for x in range(true_result.shape[1]):
                ...
                res = np.floor(np.dot(inv_true_matrix, input_pt)).astype(np.int64)
                _x, _y = res[:2]
