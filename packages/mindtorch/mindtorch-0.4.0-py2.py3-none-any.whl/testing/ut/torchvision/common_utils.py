...
def _create_data(height=3, width=3, channels=3, device="cpu"):
    ...
    # tensor = torch.randint(0, 256, (channels, height, width), dtype=torch.float32, device=device)
...
def get_list_of_videos(tmpdir, num_videos=5, sizes=None, fps=None):
    ...
    for i in range(num_videos):
        ...
        if fps is None:
            ...
            f = fps[i]
        data = torch.randint(0, 256, (size, 300, 400, 3), dtype=torch.int8).to(torch.uint8)
...
def _assert_equal_tensor_to_pil(tensor, pil_image, msg=None):
    ...
    assert np.allclose(tensor.numpy(), pil_tensor.numpy())
...
def _test_fn_on_batch(batch_tensors, fn, scripted_fn_atol=1e-8, **fn_kwargs):
    ...
    for i in range(len(batch_tensors)):
        img_tensor = cast_to_adapter_tensor(batch_tensors[i, ...])
        transformed_img = fn(img_tensor, **fn_kwargs)
        # torch.testing.assert_close(transformed_img, transformed_batch[i, ...], rtol=0, atol=1e-6)
        assert np.allclose(transformed_img.numpy(), transformed_batch.numpy()[i, ...], rtol=0, atol=1e-6)
    ...
    if scripted_fn_atol >= 0:
        ...
        assert np.allclose(transformed_batch.numpy(), s_transformed_batch.numpy(), rtol=1e-5, atol=scripted_fn_atol)
