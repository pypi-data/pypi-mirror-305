...
import mindtorch.torchvision.transforms as T
import mindtorch.torchvision.transforms.functional as F
import mindtorch.torchvision.transforms.functional_pil as F_pil
...
from mindtorch.torchvision.transforms import InterpolationMode
...
class TestRotate:
    ...
    ALL_DTYPES = [None, torch.float32, torch.float16]
    ...
    @pytest.mark.parametrize("fn", [F.rotate])
    def test_rotate(self, device, height, width, center, dt, angle, expand, fill, fn):
        ...
        out_pil_tensor = torch.tensor(np.array(out_pil_img).transpose((2, 0, 1)))
        ...
        num_diff_pixels = cast_to_adapter_tensor(out_tensor != out_pil_tensor).sum().item() / 3.0
    ...
    def test_rotate_deprecation_resample(self):
        ...
        with pytest.warns(
            ...
            assert np.allclose(res1.numpy(), res2.numpy())
...
class TestAffine:
    ...
    @pytest.mark.parametrize("fn", [F.affine])
    def test_square_rotations(self, device, height, width, dt, angle, config, fn):
        ...
        #     assert np.allclose(torch.rot90(tensor, **config).numpy(), out_tensor.numpy())
    ...
    def test_all_ops(self, device, height, width, dt, a, t, s, sh, f, fn):
        ...
        num_diff_pixels = cast_to_adapter_tensor(out_tensor != out_pil_tensor).sum().item() / 3.0
        ratio_diff_pixels = num_diff_pixels / out_tensor.shape[-1] / out_tensor.shape[-2]
        # Tolerance : accuracy lower than 0.05 in some combinations
        assert ratio_diff_pixels < 0.06 
    ...
    def test_warnings(self, device):
        ...
        with pytest.warns(
            ...
            assert np.allclose(np.asarray(res1), np.asarray(res2))
...
@pytest.mark.parametrize("dt", [None, torch.float32, torch.float16])
@pytest.mark.parametrize(
    "fill",
    (
        None,
...
@pytest.mark.parametrize("fn", [F.perspective])
...
def check_functional_vs_PIL_vs_scripted(
    ...
    #TODO: There is a probability of accuracy errors on the GPU. Set the seed to avoid the error
...
def test_resized_crop(device, mode):
    ...
    assert np.allclose(tensor.numpy(), out_tensor.numpy())
    ...
    # )
    assert np.allclose(expected_out_tensor.numpy(), out_tensor.numpy())
...
# @pytest.mark.parametrize("fn", [F.gaussian_blur])
...
#     if image_size == "small":
#         tensor = (
#             torch.tensor(np.arange(3 * 10 * 12, dtype="uint8").reshape((10, 12, 3))).permute(2, 0, 1)
#         )
#         pytorch_tensor = \
#             pytorch.tensor(np.arange(3 * 10 * 12, dtype="uint8").reshape((10, 12, 3))).permute(2, 0, 1)
#
#     else:
#         tensor = torch.tensor(np.arange(26 * 28, dtype="uint8").reshape((1, 26, 28)))
#         pytorch_tensor = pytorch.tensor(np.arange(26 * 28, dtype="uint8").reshape((1, 26, 28)))
#
#     if dt == torch.float16 and device == "cpu":
#         # skip float16 on CPU case
#         return
...
#     if dt is not None:
#         tensor = tensor.to(dtype=dt)
#         pytorch_tensor = pytorch_tensor.to(dtype=dt)
#
...
#     # )
#
#     torch_out = tv_F.gaussian_blur(pytorch_tensor, kernel_size=ksize, sigma=sigma).clone()
#     print("------------torch_out:", torch_out.shape)
#     out = fn(tensor, kernel_size=ksize, sigma=sigma).clone()
#     print("------------torch_out:", torch_out.shape)
#     print("------------out:", out.numpy().shape)
#
#     # torch.testing.assert_close(out, true_out, rtol=0.0, atol=1.0, msg=f"{ksize}, {sigma}")
#     assert np.allclose(out.numpy(), torch_out.numpy())
...
def test_hsv2rgb(device):
    ...
    for _ in range(10):
        ...
        assert np.allclose(ft_img.numpy(), colorsys_img.numpy(), rtol=0.0, atol=1e-5)
...
def test_rgb2hsv(device):
    ...
    for _ in range(10):
        ...
        max_diff_h = (colorsys_img_h - ft_hsv_img_h).abs().max()
...
def test_rgb_to_grayscale(device, num_output_channels):
    ...
    gray_pil_image = F.to_grayscale(pil_img, num_output_channels=num_output_channels)
...
def test_five_crop(device):
    ...
    for i in range(len(batch_tensors)):
        ...
        for j in range(len(tuple_transformed_imgs)):
            ...
            assert np.allclose(true_transformed_img.numpy(), transformed_img.numpy())
