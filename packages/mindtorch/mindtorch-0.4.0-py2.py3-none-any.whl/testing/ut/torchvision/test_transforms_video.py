...
from mindtorch.torchvision.transforms import Compose
from .common_utils import is_test_under_gpu_context
...
with warnings.catch_warnings(record=True):
    ...
    import mindtorch.torchvision.transforms._transforms_video as transforms
...
class TestVideoTransforms:
    ...
    def test_center_crop_video(self):
        ...
        clip[:, oh1 : oh1 + oheight, ow1 : ow1 + owidth, :] = 0.
    ...
    def test_normalize_video(self, channels):
        ...
        #TODO: transform a large tensor to a list may result in core dump on GPU
        if is_test_under_gpu_context():
            numFrames = random.randint(4, 10)
        ...
        clip = torch.Tensor(channels, numFrames, height, width).normal_adapter(mean, std)
    ...
    def test_random_horizontal_flip_video(self, p):
        ...
        if p == 0:
            ...
            assert np.allclose(out.numpy(), clip.numpy())
        elif p == 1:
            # torch.testing.assert_close(out, hclip)
            assert np.allclose(out.numpy(), hclip.numpy())
