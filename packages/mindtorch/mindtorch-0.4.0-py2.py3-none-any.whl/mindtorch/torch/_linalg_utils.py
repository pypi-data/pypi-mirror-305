...
from mindtorch.torch.common import float16, float32, float64
from mindtorch.torch.functional import matmul, orgqr, geqrf, flip
from mindtorch.torch.linalg import qr, eigh

def is_sparse(A):
    """Check if tensor A is a sparse tensor"""
    if isinstance(A, ms.SparseTensor):
        return True
    elif isinstance(A, ms.Tensor):
        return False
    else:
        error_str = "expected Tensor" + " but got {}".format(type(A))
...
def s_matmul(A: Optional[Tensor], B: Tensor) -> Tensor:
    ...
    if A is None:
        return B
    if is_sparse(A):
        raise NotImplementedError("Sparse matmul is not implemented.")
    return matmul(A, B)
...
def basis(A):
    ...
    if A.is_cuda:
        # orgqr is not available in CUDA
        Q = qr(A).Q
    else:
        Q = orgqr(*geqrf(A))
    return Q
...
def solve(input: Tensor, A: Tensor, *, out=None) -> Tuple[Tensor, Tensor]:
    raise RuntimeError(
        "This function was deprecated since version 1.9 and is now removed. "
        "Please use the `torch.linalg.solve` function instead.",
    )
...
def eig(
    ...
    raise RuntimeError(
        "This function was deprecated since version 1.9 and is now removed. "
        "Please use the `torch.linalg.eig` function instead.",
    )
