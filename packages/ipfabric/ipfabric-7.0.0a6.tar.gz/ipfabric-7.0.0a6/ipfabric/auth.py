...
class Setup(BaseSettings):
    ...
    def _verify_auth(self):
        if self.auth:
            ...
            self.client.auth = TokenAuth(self.token, api_version=self._os_api_version)
    ...
    def _get_auth_from_auth(self) -> Auth:
        ...
        if isinstance(self.auth, str):
            try:
                ...
                return TokenAuth(self.auth, api_version=self._os_api_version)
    ...
    def check_version(self) -> tuple:
        ...
        if os_api_version[0] != cfg_version[0]:
            raise RuntimeError(
                f"OS Major Version v{os_api_version[0]} does not match SDK Major Version v{cfg_version[0]}."
            )
        ...
        if cfg_version[0:2] > os_api_version:
            logger.warning(
