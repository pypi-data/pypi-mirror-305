# kpi_library/date/first_quartile.py
import json
import pandas as pd
...
class DateFirstQuartile(MetricModel):
    """
    This metric returns the first quartile (25th-percentile) of a time column.

    Example
    -------
    >>> dm = DateFirstQuartile()
    ...
    Timestamp('2022-03-21 06:00:00')
    >>> data = pd.Series(["2022-03-20", "2022-03-20", "2022-03-22", "2022-03-23", "2022-03-24", "2022-03-25"])
    >>> dm.run(data, date_format=r'%Y-%m-%d')
    Timestamp('2022-03-20 12:00:00')
    >>> dm.to_dqv(data, date_format=r'%Y-%m-%d')
    [{'dqv_isMeasurementOf': 'date.first_quartile', 'dqv_computedOn': '', 'rdf_datatype': 'DateTime', 'ddqv_hasParam\
eters': [{'parameter_name': 'date_format', 'value': '"%Y-%m-%d"'}], 'dqv_value': '2022-03-20 12:00:00'}]
    >>> dm.to_dqv(data, date_format="-1")
    [{'dqv_isMeasurementOf': 'date.first_quartile', 'dqv_computedOn': '', 'rdf_datatype': 'DateTime', 'ddqv_hasParam\
eters': [{'parameter_name': 'date_format', 'value': '"-1"'}], 'dqv_value': 'null'}]
    >>> data = pd.Series(["01:00:05", "01:00:05", "01:00:20", "01:00:20", "01:00:30", "01:00:40"], name='timestamp')
    >>> dm.run(data, date_format='%H:%M:%S')
    Timestamp('1900-01-01 01:00:08.750000128')
    >>> dm.run(pd.Series([None, None, None]))
    >>> dm.to_dqv(pd.Series([None, None, None]))
    [{'dqv_isMeasurementOf': 'date.first_quartile', 'dqv_computedOn': '', 'rdf_datatype': 'DateTime', 'ddqv_hasParam\
eters': [{'parameter_name': 'date_format', 'value': 'null'}], 'dqv_value': 'null'}]
    >>> dm.to_dqv(pd.Series(["None", "None", "None"]))
    [{'dqv_isMeasurementOf': 'date.first_quartile', 'dqv_computedOn': '', 'rdf_datatype': 'DateTime', 'ddqv_hasParam\
eters': [{'parameter_name': 'date_format', 'value': 'null'}], 'dqv_value': 'null'}]
    >>> dm.to_dqv(pd.Series([1, 1, 1]))
    [{'dqv_isMeasurementOf': 'date.first_quartile', 'dqv_computedOn': '', 'rdf_datatype': 'Error', 'ddqv_hasParamete\
rs': [{'parameter_name': 'date_format', 'value': 'null'}], 'dqv_value': 'null'}]
    """
    def __init__(self):
        super(DateFirstQuartile, self).__init__(
            identifier='date.first_quartile',
            keyword='DateFirstQuartile',
            title='First quartile',
            definition='First quartile (25th-percentile) of time data.',
    ...
    def run(self, data: pd.Series, **kwargs) -> Optional[pd.Timestamp]:
        """
        This method computes the first quartile (25th-percentile) of the data given as parameter.
