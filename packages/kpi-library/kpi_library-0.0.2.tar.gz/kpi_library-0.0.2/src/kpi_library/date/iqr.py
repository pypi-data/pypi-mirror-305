...
class DateIqr(MetricModel):
    """
    This metric returns the interquartile range between the third and first quartile of a time column.
    ...
    Timedelta('3 days 06:00:00')
    >>> dm.to_dqv(data, date_format=r'%Y-%m-%d')
    [{'dqv_isMeasurementOf': 'date.iqr', 'dqv_computedOn': '', 'rdf_datatype': 'Timedelta', 'ddqv_hasParameters': [{\
'parameter_name': 'date_format', 'value': '"%Y-%m-%d"'}], 'dqv_value': '3 days 06:00:00'}]
...
'parameter_name': 'date_format', 'value': '"-1"'}], 'dqv_value': 'null'}]
    ...
    Timedelta('0 days 00:00:18.749999872')
...
'parameter_name': 'date_format', 'value': 'null'}], 'dqv_value': 'null'}]
    ...
    [{'dqv_isMeasurementOf': 'date.iqr', 'dqv_computedOn': '', 'rdf_datatype': 'Error', 'ddqv_hasParameters': [{\
'parameter_name': 'date_format', 'value': 'null'}], 'dqv_value': 'null'}]
    """
    def __init__(self):
        super(DateIqr, self).__init__(
            identifier='date.iqr',
            keyword='DateIqr',
            title='Interquartile range',
            definition='Interquartile range between the third and first quartile of time data.',
    ...
    def run(self, data: pd.Series, **kwargs) -> Optional[pd.Timedelta]:
        """
        This method returns the interquartile range between the third and first quartile of the data given as parameter.
        ...
        _: :obj:`pandas.Timedelta`, optional.
            Interquartile range value of the data.
        """
        ...
        if srs.empty:
            return None
        # compute statistic
        quartiles = srs.quantile(q=[0.25, 0.75])
