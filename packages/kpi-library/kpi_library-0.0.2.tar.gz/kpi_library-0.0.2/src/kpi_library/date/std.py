...
class DateStd(MetricModel):
    """
    This metric computes the standard deviation value of a time column.
    ...
    Timedelta('1 days 20:53:59.599108634')
    >>> data = pd.Series(["2022-03-20", "2022-03-20", "2022-03-22", "2022-03-23", "2022-03-24", "2022-03-25"])
    >>> dm.run(data, date_format=r'%Y-%m-%d')
    Timedelta('2 days 01:34:27.072593237')
    >>> dm.to_dqv(data, date_format=r'%Y-%m-%d')
    [{'dqv_isMeasurementOf': 'date.std', 'dqv_computedOn': '', 'rdf_datatype': 'Timedelta', 'ddqv_hasParameters': [{\
'parameter_name': 'date_format', 'value': '"%Y-%m-%d"'}], 'dqv_value': '2 days 01:34:27.072593237'}]
...
'parameter_name': 'date_format', 'value': '"-1"'}], 'dqv_value': 'null'}]
    ...
    Timedelta('0 days 00:00:13.784048752')
...
'parameter_name': 'date_format', 'value': 'null'}], 'dqv_value': 'null'}]
    ...
    [{'dqv_isMeasurementOf': 'date.std', 'dqv_computedOn': '', 'rdf_datatype': 'Error', 'ddqv_hasParameters': [{\
'parameter_name': 'date_format', 'value': 'null'}], 'dqv_value': 'null'}]
    """
    def __init__(self):
        super(DateStd, self).__init__(
            identifier='date.std',
            keyword='DateStd',
            title='Standard deviation',
            definition='Standard deviation value of time data.',
    ...
    def run(self, data: pd.Series, **kwargs) -> Optional[pd.Timedelta]:
        """
        This method returns the standard deviation value of the data given as parameter.
