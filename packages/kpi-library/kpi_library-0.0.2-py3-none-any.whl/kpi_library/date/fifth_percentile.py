# kpi_library/date/fifth_percentile.py
import json
import pandas as pd
...
class DateFifthPercentile(MetricModel):
    """
    This metric returns the fifth percentile of a time column.

    Example
    -------
    >>> dm = DateFifthPercentile()
    ...
    [{'dqv_isMeasurementOf': 'date.fifth_percentile', 'dqv_computedOn': '', 'rdf_datatype': 'DateTime', 'ddqv_hasParam\
eters': [{'parameter_name': 'date_format', 'value': '"%Y-%m-%d"'}], 'dqv_value': '2022-03-20 00:00:00'}]
...
eters': [{'parameter_name': 'date_format', 'value': 'null'}], 'dqv_value': 'null'}]
    ...
    [{'dqv_isMeasurementOf': 'date.fifth_percentile', 'dqv_computedOn': '', 'rdf_datatype': 'Error', 'ddqv_hasParamete\
rs': [{'parameter_name': 'date_format', 'value': 'null'}], 'dqv_value': 'null'}]
    """
    def __init__(self):
        super(DateFifthPercentile, self).__init__(
            identifier='date.fifth_percentile',
            keyword='DateFifthPercentile',
            title='Fifth percentile',
            definition='Fifth percentile of time data.',
    ...
    def run(self, data: pd.Series, **kwargs) -> Optional[pd.Timestamp]:
        """
        This method computes the fifth percentile of the data given as parameter.
