...
class DateMean(MetricModel):
    """
    This metric computes the mean value of a time column.
...
'parameter_name': 'date_format', 'value': 'null'}], 'dqv_value': 'null'}]
    ...
    [{'dqv_isMeasurementOf': 'date.mean', 'dqv_computedOn': '', 'rdf_datatype': 'Error', 'ddqv_hasParameters': [{\
'parameter_name': 'date_format', 'value': 'null'}], 'dqv_value': 'null'}]
    """
    def __init__(self):
        super(DateMean, self).__init__(
            identifier='date.mean',
            title='Mean value',
            definition='Mean value of time data.',
            expected_data_type=str(ResultTypes.DATE.value),
            parameters=[ParameterModel(name='date_format', data_type=str(ResultTypes.STRING.value), default_value=None,
