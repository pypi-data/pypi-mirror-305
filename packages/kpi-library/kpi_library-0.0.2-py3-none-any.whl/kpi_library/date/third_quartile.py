# kpi_library/date/third_quartile.py
import json
import pandas as pd
...
class DateThirdQuartile(MetricModel):
    """
    This metric returns the third quartile (75th-percentile) of a time column.

    Example
    -------
    >>> dm = DateThirdQuartile()
    ...
    [{'dqv_isMeasurementOf': 'date.third_quartile', 'dqv_computedOn': '', 'rdf_datatype': 'DateTime', 'ddqv_hasParam\
eters': [{'parameter_name': 'date_format', 'value': '"%Y-%m-%d"'}], 'dqv_value': '2022-03-23 18:00:00'}]
...
eters': [{'parameter_name': 'date_format', 'value': '"-1"'}], 'dqv_value': 'null'}]
    ...
    Timestamp('1900-01-01 01:00:27.500000')
...
eters': [{'parameter_name': 'date_format', 'value': 'null'}], 'dqv_value': 'null'}]
    ...
    [{'dqv_isMeasurementOf': 'date.third_quartile', 'dqv_computedOn': '', 'rdf_datatype': 'Error', 'ddqv_hasParamete\
rs': [{'parameter_name': 'date_format', 'value': 'null'}], 'dqv_value': 'null'}]
    """
    def __init__(self):
        super(DateThirdQuartile, self).__init__(
            identifier='date.third_quartile',
            keyword='DateThirdQuartile',
            title='Third quartile',
            definition='Third quartile (75th-percentile) of time data.',
    ...
    def run(self, data: pd.Series, **kwargs) -> Optional[pd.Timestamp]:
        """
        This method computes the third quartile (75th-percentile) of the data given as parameter.
