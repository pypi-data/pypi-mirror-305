# kpi_library/date/ninety_fifth_percentile.py
import json
import pandas as pd
...
class DateNinetyFifthPercentile(MetricModel):
    """
    This metric returns the 95th-percentile of a time column.

    Example
    -------
    >>> dm = DateNinetyFifthPercentile()
    ...
    [{'dqv_isMeasurementOf': 'date.ninety_fifth_percentile', 'dqv_computedOn': '', 'rdf_datatype': 'DateTime', 'ddqv_h\
asParameters': [{'parameter_name': 'date_format', 'value': '"%Y-%m-%d"'}], 'dqv_value': '2022-03-24 18:00:00'}]
    >>> dm.to_dqv(data, date_format="-1")
    [{'dqv_isMeasurementOf': 'date.ninety_fifth_percentile', 'dqv_computedOn': '', 'rdf_datatype': 'DateTime', 'ddqv_h\
asParameters': [{'parameter_name': 'date_format', 'value': '"-1"'}], 'dqv_value': 'null'}]
    >>> data = pd.Series(["01:00:05", "01:00:05", "01:00:20", "01:00:20", "01:00:30", "01:00:40"], name='timestamp')
    >>> dm.run(data, date_format='%H:%M:%S')
    Timestamp('1900-01-01 01:00:37.500000')
    >>> dm.run(pd.Series([None, None, None]))
    >>> dm.to_dqv(pd.Series([None, None, None]))
    [{'dqv_isMeasurementOf': 'date.ninety_fifth_percentile', 'dqv_computedOn': '', 'rdf_datatype': 'DateTime', 'ddqv_h\
asParameters': [{'parameter_name': 'date_format', 'value': 'null'}], 'dqv_value': 'null'}]
    >>> dm.to_dqv(pd.Series(["None", "None", "None"]))
    [{'dqv_isMeasurementOf': 'date.ninety_fifth_percentile', 'dqv_computedOn': '', 'rdf_datatype': 'DateTime', 'ddqv_h\
asParameters': [{'parameter_name': 'date_format', 'value': 'null'}], 'dqv_value': 'null'}]
    >>> dm.to_dqv(pd.Series([1, 1, 1]))
    [{'dqv_isMeasurementOf': 'date.ninety_fifth_percentile', 'dqv_computedOn': '', 'rdf_datatype': 'Error', 'ddqv_hasP\
arameters': [{'parameter_name': 'date_format', 'value': 'null'}], 'dqv_value': 'null'}]
    """
    def __init__(self):
        super(DateNinetyFifthPercentile, self).__init__(
            identifier='date.ninety_fifth_percentile',
            keyword='DateNinetyFifthPercentile',
            title='Ninety-fifth percentile',
            definition='Ninety-fifth percentile of time data.',
    ...
    def run(self, data: pd.Series, **kwargs) -> Optional[pd.Timestamp]:
        """
        This method computes the ninety-fifth percentile of the data given as parameter.
        ...
        _: :obj:`pandas.Timestamp`, optional.
            Ninety-fifth percentile of the data.
        """
