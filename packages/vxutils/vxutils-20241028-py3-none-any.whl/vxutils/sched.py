...
def quit_handler(signum: int, frame: Any) -> None:
    global __context__
    logging.warning("========== Scheduler Executing exit handler... ==========")
...
def run(args: Optional[Namespace] = None) -> None:
    ...
    logging.info("Load Config: %s", __context__.config)

    class ParamsManager(SyncManager):
        pass

    ParamsManager.register("get_params", lambda: __context__.params)
    ParamsManager.register(
        "update_params", lambda key, value: __context__.params.update({key: value})
    )
    ParamsManager.register("publish", publish)
    ...
    logging.info("Load Params: %s", to_json(__context__.params))
    ...
    try:
        ...
        def run_forever(context:VXContext)->None:
            global __ACTIVE__
            while __ACTIVE__:
                time.sleep(1)

        if len(args.server) > 0:
            spec = importlib.util.spec_from_file_location("server", args.server)
            if spec is None:
                raise ImportError(f"Can not import module: {args.server}")
