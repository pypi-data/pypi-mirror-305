...
class HistogramPlot(MultiDataPlot):
    ...
    def draw_histogram_data(
        ...
        try:
            ...
            raise ValueError(f"Failed to draw histogram data: {str(e)}") from e
    ...
    def resolve_ylabel(
        ...
        if divide_bin_width:
            return ylabel.format(unit=unit, bin_width='' if unit else '1')
        ...
        if not all(h.uniform_binning for h in histograms):
            return ylabel.format(unit=unit, bin_width='bin')
    
        first_hist = histograms[0]
        if all(first_hist.binning == h.binning for h in histograms[1:]):
            return ylabel.format(unit=unit, bin_width=first_hist.bin_widths[0])
