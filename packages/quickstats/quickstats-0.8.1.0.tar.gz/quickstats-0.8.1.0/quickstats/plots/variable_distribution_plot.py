...
class VariableDistributionPlot(HistogramPlot):
    ...
    CONFIG = {
        ...
        'default_ylabel': {
            'unnormalized': 'Events / {bin_width:.2g}{unit}',
            'normalized': 'Fraction of Events / {bin_width:.2g}{unit}',
        }
    }
    ...
    def resolve_plot_options(
        ...
        for target in targets:
            ...
            if 'color' not in styles:
                if color_i == n_colors:
                    self.stdout.warning(
                        'Number of targets exceeds available colors in the color map. Colors will be recycled.'
                    )
                styles['color'] = colors[color_i % n_colors]
                color_i += 1
                
            if 'label' not in styles:
                styles['label'] = self.label_map.get(target, target) or 'None'
            ...
            if 'color' not in error_styles:
                error_styles['color'] = styles['color']
            if 'label' not in error_styles:
                error_styles['label'] = self.label_map.get(f'{target}.error', styles['label']) or 'None'
        ...
        if not stacked:
            for target in targets:
                options = {'components': {}}
                for key in ['plot_format', 'error_format', 'styles', 'error_styles']:
    ...
    def draw(
        ...
        self.legend_order = (
            list(legend_order) if legend_order is not None 
            else self.resolve_legend_order(plot_options)
        )
        ...
        if isinstance(bins, int) and bin_range is not None:
