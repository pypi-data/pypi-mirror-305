...
class MongoCustomerSQSNotifier(SQSClient):
    SERVICE_NAME = "MongoCustomerSQSProcessor"
    TOPIC_NAME = "reach-data-bridge-messages-queue"
    DEFAULT_REGION = "us-east-1"

    @staticmethod
    def is_valid_operation(operation):
        valid_operations = ["insert", "upsert", "update", "delete"]
    ...
    def notify_for_sync_customer(self, metadata: dict, records: [], delay_seconds=60):
        message_data = {"metadata": metadata, "records": records}

        operation = metadata["operation"]

        if not MongoCustomerSQSNotifier.is_valid_operation(operation):
        ...
        for record in records:
            ...
            if isinstance(validation, str):
                self.logger.error(f"{validation} - {str(record)}")
                raise ValueError(f"{validation} - {str(record)}")
