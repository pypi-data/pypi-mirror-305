...
class User__Section_Data(Type_Safe):
    ...
    def file_data   (self, path      ): return self.db_user.s3_file_data        (s3_key=self.s3_key(path)           )
    ...
    def folder_list (self, folder='' ): return self.db_user.s3_folder_files__all(folder=self.s3_key(folder)         )

    def user_id(self):
        return self.db_user.user_id

    def section_folder_files(self):
        return self.db_user.s3_folder_files(self.section_name)
