...
from cbr_shared.cbr_sites.CBR__Shared__Constants    import COOKIE_NAME__CBR__SESSION_ID__PERSONA, COOKIE_NAME__CBR__SESSION_ID__USER
...
class CBR__Session__Load(Type_Safe):
    ...
    def session_id__from_request(self, request: Request):
        ...
        if COOKIE_NAME__CBR__SESSION_ID__PERSONA in request.cookies:                    # then see if we are in 'persona mode'
            return request.cookies.get(COOKIE_NAME__CBR__SESSION_ID__PERSONA)           #

        if COOKIE_NAME__CBR__SESSION_ID__USER in request.cookies:                       # finally check for the user session cookie
            return request.cookies.get(COOKIE_NAME__CBR__SESSION_ID__USER)
