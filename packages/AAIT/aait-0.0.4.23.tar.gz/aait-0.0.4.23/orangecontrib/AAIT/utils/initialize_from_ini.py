...
def apply_modification_from_python_file(source_path=None,filename=None):
    ...
    def decorator(cls):
        if filename is None:
            ...
            if source_path!= None:
                user_modifications_path = MetManagement.get_widget_extention_path() + (os.path.basename(source_path))
        else:
            user_modifications_path = filename
        bool_dev=False
        if source_path!=None:
            if "site-packages/Orange/widgets" in source_path.replace("\\", "/"):
                user_modifications_path=user_modifications_path[:-3]+"_dev.py"
                bool_dev=True
            if bool_dev==False:#
                if os.path.exists(user_modifications_path):
                    new_path=source_path[:-3]+"_new.py"
                    ...
                    if os.path.getsize(source_path) != os.path.getsize(new_path):
                        try:
                            with open(source_path, 'r') as infile:
                                content = infile.read()
                            with open(new_path, 'w') as outfile:
