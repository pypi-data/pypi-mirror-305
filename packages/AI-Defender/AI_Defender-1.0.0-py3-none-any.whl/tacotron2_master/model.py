...
class Postnet(nn.Module):
    """Postnet: Five 1-d convolutions with 512 channels and kernel size 5"""
...
class Encoder(nn.Module):
    """Encoder module: Three 1-d convolution banks and Bidirectional LSTM"""
    def __init__(self, hparams):
    ...
    def forward(self, x, input_lengths):
        ...
        return nn.utils.rnn.pad_packed_sequence(outputs, batch_first=True)[0]
...
class Decoder(nn.Module):
    ...
    def get_go_frame(self, memory):
        ...
        B = memory.size(0)
        return memory.new_zeros(B, self.n_mel_channels * self.n_frames_per_step)

    def initialize_decoder_states(self, memory, mask):
        """Initializes attention and decoder RNN states."""
        B = memory.size(0)
        ...
        self.attention_context = memory.new_zeros(B, memory.size(2))
    ...
    def parse_decoder_inputs(self, decoder_inputs):
        """Prepares decoder inputs for teacher-forced training."""
        decoder_inputs = decoder_inputs.transpose(1, 2)
        return decoder_inputs.view(decoder_inputs.size(0), -1, self.n_mel_channels).transpose(0, 1)

    def parse_decoder_outputs(self, mel_outputs, gate_outputs, alignments):
        """Prepares decoder outputs for output."""
        alignments = torch.stack(alignments).transpose(0, 1)
        gate_outputs = torch.stack(gate_outputs).transpose(0, 1)
        mel_outputs = torch.stack(mel_outputs).transpose(0, 1).contiguous().view(-1, self.n_mel_channels)
    ...
    def decode(self, decoder_input):
        ...
        decoder_output = self.linear_projection(torch.cat((self.decoder_hidden, self.attention_context), dim=1))
        gate_prediction = self.gate_layer(torch.cat((self.decoder_hidden, self.attention_context), dim=1))
