...
from s3pro.utils import CaseInsensitiveDict
...
class AwsSigV4:
    ...
    def _sign(self, date: datetime, credential_scope: str, canonical_request: str) -> str:
        ...
        k_date = sign_msg(f'AWS4{self.aws_secret_access_key}'.encode(), datestamp)
    ...
    def sign(
        ...
        headers_to_sign = sorted(
            filter(
                ...
                (h_key.lower() for h_key in (headers | new_headers).keys()),
            )
        )
        canonical_headers = ''.join(':'.join((h, (headers | new_headers)[h])) + '\n' for h in headers_to_sign)
