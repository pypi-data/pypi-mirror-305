...
impl XLSXSheet {
    ...
    pub fn find_cells_between_patterns(
        ...
        // Сортируем ячейки по порядку строки и столбцов
        let mut sorted_cells = self._cells.par_iter().collect::<Vec<_>>();
        sorted_cells.sort_by_key(|(_, cell)| {
            let cell = cell.lock().unwrap();
            (cell.row, cell.column)
        });

        let rows_idx = sorted_cells
            .par_iter()
            .filter_map(|(_, cell)| {
                let cell = cell.lock().unwrap();
            ...
            if mr > 1 || mc > 1 {
                ...
                let cells: HashMap<_, _> = (1..=row)
                    ...
                    .flat_map(|r| {
                        ...
                        (1..=col).into_par_iter().filter_map(move |c| {
                            if !existing_cells.contains_key(&(r, c)) {
                                let cell =
                                    XLSXSheetCell::new(Arc::clone(&current_sheet), r, c, None);
                                Some(((r, c), cell))
                            } else {
                                None
                            }
