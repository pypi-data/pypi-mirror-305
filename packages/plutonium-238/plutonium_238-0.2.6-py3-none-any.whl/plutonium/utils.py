...
class ImageFileExtractor(object):
    ...
    def get_execute_file_data_from_image(self, image_info, image_file, result_file):
        ...
        try:
            for layer in image_data[0].get('Layers'):
                ...
                if sys.platform == 'darwin':
                    output = self.run_command(f"ls {image_extract_dir}/{layer} | xargs shasum -a 256")
                else:
                    output = self.run_command(f"ls {image_extract_dir}/{layer}| xargs sha256sum")
        ...
        for diff_id in layer_data:
            ...
            os.system(f"tar -xf {image_extract_dir}/{layer_file_map[diff_id]} --directory {layer_item_dir}")
            logger.info(f"tar -xf {image_extract_dir}/{layer_file_map[diff_id]} --directory {layer_item_dir}")
