...
class FlowBase:
    ...
    def oss_upload_file(self, path: str, source=1):
        oss = self.new_oss()
        return oss.upload_file(self.uid, path, CloudSourceType(source))

    def oss_upload_file_get_url(self, path: str, source=1):
        oss = self.new_oss()
        path = oss.upload_file(self.uid, path, CloudSourceType(source))
        return oss.get_url(path, CloudSourceType(source))
