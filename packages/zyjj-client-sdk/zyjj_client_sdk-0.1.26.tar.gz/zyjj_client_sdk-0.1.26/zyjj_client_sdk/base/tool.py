...
async def task_warp(async_fun, err_default, **kwargs):
    try:
        await async_fun(**kwargs)
...
async def _async_batch_run(data_list: list, split_size: int, async_fun, sleep: int, args: dict, err_default: dict):
    ...
    async with asyncio.TaskGroup() as tg:
        for i in range(num_segments):
            ...
            task_list.append(tg.create_task(task_warp(async_fun, err_default, data=data, **args)))
            # 延迟固定时间，避免并发太高
...
def async_batch_run(
    ...
    return asyncio.run(_async_batch_run(data_list, split_size, async_func, sleep, args, err_default))
