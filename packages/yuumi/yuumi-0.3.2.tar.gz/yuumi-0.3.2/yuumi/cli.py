...
def main():
    ...
    parser.add_argument("--token", "-t", type=str, help="Real-Debrid API access token")
    parser.add_argument("--user", "-u", action="store_true", help="Get user info")
    parser.add_argument(
        "--cached", "-c", type=str, help="Check instantAvailability of an infohash"
    )
    ...
    parser.add_argument(
        "--uncached", "-uc", type=str, help="Download a non-cached file"
    )
    args = parser.parse_args()

    rd_token = args.token if args.token else os.getenv("RD_KEY")
    if not rd_token:
        parser.error(
            "A Real-Debrid API access token is required. Please provide it using --token or set the RD_KEY environment variable."
        )
    api = RealDebridAPI(rd_token)
