...
class RealDebridAPI:
    ...
    def select_playable_files(
        self, torrent_id: str, availability_data: dict[str, dict[int, dict[str, int]]]
    ):
        """
        ...
        playable_files = [
            file_index
            for file_index, file_info in availability_data.items()
            if "filename" in file_info
            and os.path.splitext(file_info["filename"])[1].lower() in video_extensions
        ]
    ...
    def get_instant_availability(
        ...
        for hash, values in data.items():
            if "rd" in values and values["rd"]:
                result = {
                    int(k): {"filename": v["filename"], "filesize": v["filesize"]}
                    for container in values["rd"]
                    for k, v in container.items()
                }
                results[hash] = result
            else:
                results[hash] = {}
        results = {k: dict(sorted(v.items())) for k, v in results.items()}  # not required, but useful?
        return results
