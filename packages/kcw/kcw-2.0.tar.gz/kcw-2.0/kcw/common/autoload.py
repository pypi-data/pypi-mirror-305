# -*- coding: utf-8 -*-
import time,hashlib,json,re,os,sys,platform,shutil,requests,importlib,traceback,pip,tarfile,zipfile,signal,psutil
...
class create:
    ...
    def uploadplug(self,plug,username='',password='',cli=False,relyonlist=[]):
        ...
        ress=requests.get(config.domain['kcwebapi']+"/user/uploadplug/?username="+username+"&timestamp="+str(timestamp)+"&sign="+sign,'POST',
    ...
    def installplug(self,plug,edition='',token='',cli=False,mandatory=False,username=''):
        ...
        if os.path.exists(self.project+self.appname+"/"+self.modular+"/controller/"+plug) and not mandatory:
            ...
            while True:
                ress=requests.get(config.domain['kcwebapi']+"/pub/plug","GET",params={"modular":modular,"name":str(tplug),"edition":str(edition),"token":token,'username':username})
                arr=json_decode(ress.text)
                if arr:
    ...
    def uploadmodular(self,username='',password='',cli=False,relyonlist=[]):
        ...
        ress=requests.request(config.domain['kcwebapi']+"/user/uploadmodular/?username="+username+"&timestamp="+str(timestamp)+"&sign="+sign,'POST',
        data={'name':str(self.modular),'describes':'','relyonlist':relyonlist},
        files={'file':open(self.project+self.appname+"/"+self.modular+".zip", 'rb')})
        arr=json_decode(ress.text)
        if not arr:
            ...
            os.remove(self.project+self.appname+"/"+self.modular+".zip")
            return False,arr['msg']
    def installmodular(self,token='',cli=False,package='kcw'):
        ...
        if os.path.exists(self.project+self.appname+"/"+self.modular):
            ...
            modular=self.modular
            while True:
                ress=requests.get(config.domain['kcwebapi']+"/pub/modular","POST",params={"name":modular,"token":token})
                arr=json_decode(ress.text)
                if arr:
                    if arr['code']==-1 and cli:
                        if i >= 3:
