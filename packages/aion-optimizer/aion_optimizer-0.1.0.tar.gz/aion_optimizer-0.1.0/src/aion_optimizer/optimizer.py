...
class EnhancedSequentialOptimizer:
    ...
    def __init__(
        ...
        # Set default metrics based on task type if none provided
        DEFAULT_METRICS = {
            'classification': ['accuracy', 'precision', 'recall', 'f1'],
            'regression': ['r2', 'neg_mean_squared_error', 'neg_mean_absolute_error']
        }
    ...
    def _get_scorer(self, metric_name):
        ...
        return scorers.get(metric_name)
    ...
    def optimize_single_model(self, model_name, X, y):
        ...
        try:
            ...
            scoring_dict = {metric: make_scorer(self._get_scorer(metric)) for metric in self.scoring}
    ...
    def best_model_params(self, test_df):
        ...
        if self.task_type == 'classification':
            results = {
                ...
                'precision': precision_score(y_test_processed, predictions, zero_division=1),
                'recall': recall_score(y_test_processed, predictions),
                'f1_score': f1_score(y_test_processed, predictions),
            ...
            results = {
                'model_name': best_model_name,
                'r2_score': r2_score(y_test_processed, predictions),
                'mean_squared_error': mean_squared_error(y_test_processed, predictions),
                'mean_absolute_error': mean_absolute_error(y_test_processed, predictions),
                'best_params': best_params
            }
    ...
    def _print_results(self):
        ...
        for model_name, results in self.results_.items():
            for metric in self.scoring:
                ...
                print(f"{model_name:<20} {metric:<10} {score:<12.4f} {params_str}")
        ...
        for metric in self.scoring:
            ...
            print(f"Best Score: {self.best_scores[metric]:.4f}")
