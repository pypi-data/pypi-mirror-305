    ...
    def validate_default(cls, v, values):
        if v is not None and "type" in values:
            ...
            if expected_type == "str" and not isinstance(v, str):
                raise ValueError(
                    f"Default value must be a string for argument of type 'str'"
                )
            elif expected_type == "array" and not isinstance(v, list):
                raise ValueError(
                    f"Default value must be a list for argument of type 'array'"
                )
            elif expected_type == "bool" and not isinstance(v, bool):
                raise ValueError(
                    f"Default value must be a boolean for argument of type 'bool'"
                )
            elif expected_type == "int" and not isinstance(v, int):
                raise ValueError(
                    f"Default value must be an integer for argument of type 'int'"
                )
            elif expected_type == "float" and not isinstance(v, float):
                raise ValueError(
                    f"Default value must be a float for argument of type 'float'"
                )
        return v
...
class FileSpec(BaseModel):
    ...
    def expand_full_name(self) -> str:
        return (
            f"{os.path.expandvars(self.source)}:{os.path.expandvars(self.destination)}"
        )
...
class Tool(BaseModel):
    ...
    def to_mermaid(self):
        ...
        if self.env:
            ...
            mermaid.append(
                f'        EnvVars("Environment Variables:<br/>{env_vars}"):::envClass'
            )
        ...
        if self.with_volumes:
            volumes = "<br/>".join(
                f"{escape(v.name)}:{escape(v.path)}" for v in self.with_volumes
            )
