...
def create_terraform_plan(tf_files: dict, working_dir: str) -> tuple[bool, str, dict]:
    ...
    try:
        subprocess.run(["terraform", "init"], cwd=working_dir, check=True, capture_output=True, text=True)
        result = subprocess.run(["terraform", "plan", "-out=tfplan", "-json"], cwd=working_dir, check=True, capture_output=True, text=True)
...
def apply_terraform(tf_files: dict, working_dir: str) -> tuple[str, str]:
    ...
    try:
        subprocess.run(["terraform", "init"], cwd=working_dir, check=True, capture_output=True, text=True)
        result = subprocess.run(["terraform", "apply", "-auto-approve"], cwd=working_dir, check=True, capture_output=True, text=True)
        
        state_result = subprocess.run(["terraform", "show", "-json"], cwd=working_dir, check=True, capture_output=True, text=True)
