...
async def discover_endpoint(request: DiscoverRequest):
    if not request.source:
        ...
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Source is required"
        )
    ...
    try:
        ...
        logger.warning(
            f"Invalid setup command in the source '{request.source}': {str(e)}"
        )
        raise HTTPException(
        ...
        logger.warning(
            f"System exit encountered while processing '{request.source}': {str(e)}"
        )
        raise HTTPException(
...
async def run_endpoint(request: RunRequest):
    try:
        logger.info(
            f"Running workflow or tool '{request.name}' from source: {request.source}"
        )
        results = load_workflows_and_tools(request.source)
        workflow = next(
            (w for w in results["workflows"] if w["name"] == request.name), None
        )
...
async def describe_endpoint(request: DescribeRequest):
    try:
        logger.info(
            f"Describing workflow or tool '{request.name}' from source: {request.source}"
        )
        results = load_workflows_and_tools(request.source)
        workflow = next(
            (w for w in results["workflows"] if w["name"] == request.name), None
...
async def visualize_endpoint(request: VisualizeRequest):
    try:
        logger.info(
            f"Visualizing workflow '{request.workflow}' from source: {request.source}"
        )
        results = load_workflows_and_tools(request.source)
        workflow = next(
            (w for w in results["workflows"] if w["name"] == request.workflow), None
        )
