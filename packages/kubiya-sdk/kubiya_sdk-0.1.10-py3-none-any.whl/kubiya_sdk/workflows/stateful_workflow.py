from typing import Dict, Any, List, Callable, Optional, Union, AsyncIterator
...
class WorkflowStep:
    def __init__(self, func: Callable, name: str, description: str = None, icon: str = None, label: str = None, input_schema: Dict[str, Any] = None):
        self.func = func
        self.name = name
...
class StatefulWorkflow:
    ...
    def add_step(self, name: str, func: Callable, description: str = None, icon: str = None, label: str = None):
    ...
    async def run(self, initial_state: Union[Dict[str, Any], BaseModel], env_vars: Dict[str, str] = None, files: Dict[str, str] = None) -> List[Dict[str, Any]]:
    ...
    async def _execute_steps(self, initial_state: Dict[str, Any]) -> AsyncIterator[Dict[str, Any]]:
        ...
        while current_step_name != END:
            ...
            try:
                ...
                self.logger.log(f"Error in step '{current_step_name}': {str(e)}", step=current_step_name, level="ERROR")
                yield {
                    "step": current_step_name,
                    "status": "error",
                    "error": str(e),
    ...
    def add_parallel_group(self, name: str, steps: List[str], description: str = None, icon: str = None, label: str = None):
        group_step = self.add_step(name, lambda x: x, description, icon, label)
        for step_name in steps:
            group_step.then(step_name)
        return group_step

    def add_tool_step(self, name: str, tool_name: str, source_url: str, description: str = None, icon: str = None, label: str = None):
