...
def discover_python_files(source: str, ignore_patterns: List[str]) -> List[str]:
    ...
    for root, dirs, files in os.walk(source):
        dirs[:] = [
            d
            for d in dirs
            if not any(ignore_pattern in d for ignore_pattern in ignore_patterns)
        ]
...
) -> Tuple[List[Dict[str, Any]], List[Tool], Optional[Dict[str, Any]]]:
                    ...
                    tool = (
                        ...
                        if isinstance(tool_obj, dict)
                            ...
                            **{
                                ...
                                and not callable(getattr(tool_obj, attr))
                            }
                        )
                    )
                    if not tool.mermaid:
...
def discover_workflows_and_tools(
    ...
    if bundle_json.exists():
        sdk_logger.log(
            f"Found kubiya_bundle.json in source: {source}", component="discovery"
        )
    ...
    for file_path in python_files:
        try:
            ...
            errors.append(
                {"file": file_path, "error": str(e), "error_type": type(e).__name__}
            )
