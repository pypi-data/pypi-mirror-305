...
impl DirectoryManager {
    ...
    fn print_tree(&self, level: Option<usize>) -> PyResult<()> {
        let tree_string = self.get_sub_tree_string(&self.root_path, level.unwrap_or(0));
        println!("{}", tree_string);
        Ok(())
    }
    

    fn get_sub_tree_string(&self, current_path: &str, level: usize) -> String {
        ...
        for directory in &self.directories {
            ...
            {
                result.push_str(&self.get_sub_tree_string(&directory.path, level + 1));
            }
        }
