...
def service_to_state_type(service: ServiceName) -> StateTypeName:
    ...
    We currently support a single set of method definitions/service per state
    type, with a hardcoded name.
    """
    assert service.endswith("Methods"), f"Invalid service name: '{service}'"
    return StateTypeName(service[:-len("Methods")])
...
def state_type_to_service(state_type: StateTypeName) -> ServiceName:
    ...
    return ServiceName(f"{state_type}Methods")
