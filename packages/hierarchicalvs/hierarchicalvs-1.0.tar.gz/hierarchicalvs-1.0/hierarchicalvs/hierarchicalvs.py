...
def karmadock_prediction(protein_file , crystal_ligand_file , ligand_smi, out_dir,cuda_number):
        print('--shm-size="8g"')
        cmd = 'docker run -it --rm -v ./:/home  --shm-size="8g" docking_carsidock:rtx40_v4_0818 /bin/bash -c \
...
def change_path(old_path, old_substring='/home', new_substring='.'):
...
def change_path_reverse(old_path, old_substring='.', new_substring='/home'):
    if old_path.startswith(old_substring):
        new_path = old_path.replace(old_substring, new_substring, 1)
        return new_path
    else:
...
def hierarchicalvs():
    ...
    try:
        ...
        parser.add_argument('-p', '--protein_file', required=True, help='the relative path of the protein file.')
        parser.add_argument('-l', '--ligand_file', required=True, help='the relative path of the ligand file.')
        ...
        parser.add_argument('-n', '--topN', required=True, default = 50 ,  help='the number of molecules for CarsiDock')
        parser.add_argument('-P', '--Poses_keep', required=True, default = False ,  help='whether to keep the CarsiDock-based poses')
        ...
        protein_file_docker = change_path_reverse(protein_file_docker)
        ligand_file_docker = change_path_reverse(ligand_file_docker)
        compound_library_docker = change_path_reverse(compound_library_docker)
        output_path_docker = change_path_reverse(output_path_docker)
