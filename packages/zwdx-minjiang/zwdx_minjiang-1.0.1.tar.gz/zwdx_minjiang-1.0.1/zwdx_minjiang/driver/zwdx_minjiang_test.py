...
class MinJiangTest(object):
    ...
    def draw_line_chart(self,title='test',xlabel='x',ylabel='y'):
        ...
        Args:
            ...
            size (tuple, optional): 画布尺寸, = (x_length,ylength)
        """
        plt.ion() 
        time.sleep(1)
        while not self.STOP_FLAG:
            ...
            if _finish_len == _all_len:
                plt.savefig(f'{self.path}/{self.file_name}.png',bbox_inches='tight')
            plt.show()
    ...
    def delay_test(self,test_para,trig_para,data_save_path):
        """ 延时标定测试

        Args:
            test_para (_type_): 实验测试参数
            trig_para (_type_): 实验Trig设备参数
            data_save_path (_type_): 实验结果存储路径
        ...
        for index in range(len(test_point)):
            ...
            self.dev_qcs.trigger_ctrl(trig_para['trigger_source'],trig_para['trigger_us'],test_para['shots'],trig_para['trigger_continue'])
        ...
        MinJiangTools.write_data_to_csv(data_save_path,self.file_name,self.env_para,test_para,trig_para,self.test_data)
    ...
    def sweep_cavity_freq(self,test_para,trig_para,data_save_path):
        """ 扫腔频测试
    ...
    def sweep_qa_power_freq(self,test_para,trig_para,data_save_path):
        if self.dev_qcs is None or self.dev_ctp is None:
            return False
        if not os.path.exists(data_save_path):
            os.makedirs(data_save_path)
    ...
    def sweep_z_qa_freq(self,test_para,trig_para,data_save_path):
        if self.dev_qcs is None or self.dev_ctp is None:
            return False
        if not os.path.exists(data_save_path):
            os.makedirs(data_save_path)
    ...
    def sweep_cavity_freq_fine(self,test_para,trig_para,data_save_path):
        if self.dev_qcs is None or self.dev_ctp is None:
            return False
        if not os.path.exists(data_save_path):
            os.makedirs(data_save_path)
    ...
    def sweep_xy_freq(self,test_para,trig_para,data_save_path):
        if self.dev_qcs is None or self.dev_ctp is None:
            return False
        if not os.path.exists(data_save_path):
            os.makedirs(data_save_path)
    ...
    def sweep_x_z_y_xyfreq(self,test_para,trig_para,data_save_path):
        if self.dev_qcs is None or self.dev_ctp is None:
            return False
        if not os.path.exists(data_save_path):
            os.makedirs(data_save_path)
    ...
    def rabi_test(self,test_para,trig_para,data_save_path):
        if self.dev_qcs is None or self.dev_ctp is None:
            return False
        if not os.path.exists(data_save_path):
            os.makedirs(data_save_path)
    ...
    def rabi_test_len(self,test_para,trig_para,data_save_path):
        if self.dev_qcs is None or self.dev_ctp is None:
            return False
        if not os.path.exists(data_save_path):
            os.makedirs(data_save_path)
    ...
    def t1_test(self,test_para,trig_para,data_save_path):
        if self.dev_qcs is None or self.dev_ctp is None:
            return False
        if not os.path.exists(data_save_path):
            os.makedirs(data_save_path)
    ...
    def t2_test(self,test_para,trig_para,data_save_path):
        if self.dev_qcs is None or self.dev_ctp is None:
            return False
        if not os.path.exists(data_save_path):
            os.makedirs(data_save_path)
        ...
        for index in range(len(test_point)):
            ...
            self.set_xy_two_pluse_t_gauss(test_para['xy_ch'], test_para['xy_freq'], xy_one_pluse_timelen, amp=test_para['xy_amp'], t_s = t,t_delay=xy_delay)    
    ...
    def qubit_01_stateIQ(self,test_para,trig_para,data_save_path):
        if self.dev_qcs is None or self.dev_ctp is None:
            return False
        if not os.path.exists(data_save_path):
            os.makedirs(data_save_path)
    ...
    def sweep01_cavity_freq(self,test_para,trig_para,data_save_path):
        if self.dev_qcs is None or self.dev_ctp is None:
            return False
        if not os.path.exists(data_save_path):
            os.makedirs(data_save_path)
        ...
        self.test_data.clear()
        I0, Q0 = self.__sweep_freq_01state(0,test_para,trig_para)
        I1, Q1 = self.__sweep_freq_01state(1,test_para,trig_para)
