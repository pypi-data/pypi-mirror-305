...
class WorkflowProcessor:
    def __init__(self, user_config_path=None):
        ...
        prepare_directory(CryptoSeriesDatasetAssembler.dataset_out_root_folder)
    ...
    def _load_raw_data(self):
        ...
        if provider_config['re_download']:
    ...
    def _start_processor(self):
        optimizer = GlobalHyperparameterOptimizer(ConfigLoader().get_config())
        ...
        while True:
            ...
            run_report = RunProcessor(params).run_process()
            ...
            print(f"process-{os.getpid()}, run id: {run_report.run_id}, score: {score}")
    ...
    def run_processing(self):
        ...
        for _ in range(multiprocessing_size):
            p = multiprocessing.Process(target=self._start_processor)
            processes.append(p)
            p.start()
