...
class GlobalHyperparameterOptimizer:
    ...
    def _suggest_instruments(self, trial):
        ...
        all_combinations = []
        for r in range(1, len(available_instruments) + 1):
            combinations = list(itertools.combinations(available_instruments, r))
            all_combinations.extend(combinations)
        instruments = trial.suggest_categorical('instruments', [", ".join(c) for c in all_combinations])
    ...
    def print_final_results(self):
        ...
        formatted_params = {
            'instruments': best_trial.params['instruments'].split(", "),
