            ...
            while retry_count < self.max_retry_attempts:
                if step['method'] == 'bash':
                    ...
                    if return_code != 0 and return_code != None:
                            ...
                            if user_select == 'a':
                                ...
                                if return_code == 0 or return_code == None:
                                    ...
                                    if is_localhost_command == "1" or is_localhost_command == 1 or is_localhost_command == None:
                                        logger.info(f" #### `Command agent` believes this is localhost `{suggested_command}`. We have already shut it down, but we can open it separately in your terminal.")
                                        logger.info(" ### Press 'a' or 'Approve' to execute this step, or press Enter to skip, or type 'exit' to exit the entire process: ")
                                        user_run = input()
                        ...
                        break
                        if is_localhost_command == "1" or is_localhost_command == 1 or is_localhost_command == None:
                            logger.info(f"#### `Command agent` believes this is localhost `{step['command']}`. We have already shut it down, but we can open it separately in your terminal.")
                            logger.info(" ### Press 'a' or 'Approve' to execute this step, or press Enter to skip, or type 'exit' to exit the entire process: ")
                            user_run = input()
