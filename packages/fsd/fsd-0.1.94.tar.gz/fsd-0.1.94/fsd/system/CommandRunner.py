    ...
    def run_command(self, command: str, method: str = 'bash', inactivity_timeout: int = 5, use_timeout: bool = True) -> Tuple[int, List[str]]:
        ...
        try:
            ...
            if process.poll() is None:
                try:
                    logger.info(f"Attempting to terminate the process tree after inactivity timeout of {inactivity_timeout} seconds.")
                    # Terminate the entire process tree using psutil
                    ...
                    for proc in alive:
                        logger.debug(f"Force killing process PID: {proc.pid}")
                        proc.kill()
            ...
            # Ensure that the process is terminated in case of an exception
            if process and process.poll() is None:
                try:
                    logger.info("Attempting to terminate the process tree due to an exception.")
                    parent = psutil.Process(process.pid)
                    for proc in parent.children(recursive=True):
                        proc.terminate()
                    parent.terminate()
                    ...
                    logger.error(f"Failed to terminate process after exception: {terminate_error}")
            ...
            while retry_count < self.max_retry_attempts:
                if step['method'] == 'bash':
                    ...
                    if return_code != 0 and return_code != None:
                            ...
                            if user_select == 'a':
                                ...
                                if return_code == 0 or return_code == None:
                                    if is_localhost_command == "1" or is_localhost_command == 1:
                                        logger.info(f"\n #### `Command agent` believes this is localhost {suggested_command}. We have already shut it down, but we can open it separately in your terminal.")
                                        logger.info(" ### Press 'a' o 'Approve' to execute this step, or press Enter to skip, or type 'exit' to exit the entire process: ")
                                        user_choice1 = input()

                                        user_select1, _, _, _, _ = parse_payload(self.repo.get_repo_path(), user_choice1)
                                        user_select1 = user_select1.lower()
                                        if user_select1 == 'a':
                        ...
                        if is_localhost_command == "1" or is_localhost_command == 1:
                            logger.info(f"\n #### `Command agent` believes this is localhost {step['command']}. We have already shut it down, but we can open it separately in your terminal.")
                            logger.info(" ### Press 'a' or 'Approve' to execute this step, or press Enter to skip, or type 'exit' to exit the entire process: ")
                            user_choice1 = input()
