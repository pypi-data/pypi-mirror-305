...
module.exports = grammar({
  ...
  rules: {
            ...
            choice(
              ...
              choice(
                ...
                seq($.replace_region_clause, 'WITH',
                  choice($.content_literal, $.content_from_segment, $.case_stmt)
                )
              )
            )
          )
    ...
    */
    ...
    /**
    CASE WHEN (REGEX | PREFIX | SUFFIX | EMPTY | LENGTH) THEN (REPLACE | REMOVE | CONTENT | CONTINUE | INDENT)
    */
    // Matchers (WHEN clause):
    case_when: $ => seq('WHEN', choice(
      seq(field('empty', 'EMPTY')), // match empty line
    ...
    // Actions (THEN clause):
    case_action: $ => choice(
      seq(field('continue', 'CONTINUE')), // Continues to the next line
      seq(field('remove', 'REMOVE')), // Remove line
      seq('REPLACE', field('replace', $.string)), // Replace with regex capture groups
      seq('INDENT', field('indent', $.number)), // Change indentation
      choice($.content_literal, $.content_from_segment) // Replace with specific content
    ),
    case_stmt: $ => seq(
      'CASE', repeat(seq(
        $.case_when,
