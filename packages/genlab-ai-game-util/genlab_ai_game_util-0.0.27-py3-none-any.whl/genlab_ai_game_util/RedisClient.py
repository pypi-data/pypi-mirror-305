...
class RedisClient:
    ...
    def bl_pop(self, name, timeout_seconds):
        try:
            return {'code': 0, 'result': self.redis_client.blpop(name, timeout=timeout_seconds)}
        except Exception as e:
            print(f"set_hash:{e}")
            return {'code': 1, 'result': e}
    ...
    def get_hash_all(self, name):
        try:
            ...
            if hash_data is None:
                ...
                return {'code': 0,
                        'result': {key.decode('utf-8'): value.decode('utf-8') for key, value in hash_data.items()}}
        except Exception as e:
