...
class InferenceMetricAnalysis(object):
    ...
    def _format_input_to_image(self, predictions: List[Dict], references: List[Dict]):
        ...
        bcelogger.info(f"The number of prediction images after filter {len(prediction_dict)}")
        bcelogger.info(f"The number of reference images after filter {len(reference_dict)}")
