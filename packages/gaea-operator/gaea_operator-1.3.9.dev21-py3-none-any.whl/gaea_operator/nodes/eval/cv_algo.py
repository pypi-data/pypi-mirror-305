...
def cv_algo_eval(args):
    ...
    dataset.concat_dataset(dataset_name=args.dataset_name,
                           ...
                           save_label=True)
    eval_config = _generate_eval_config(args, output_model_uri, tracker_client.job_work_dir, metadata)
...
# TODO 生成cv_algo配置待优秀
def _generate_eval_config(args, output_model_uri, output_metric_uri, metadata):
    eval_config = {}
    ##暂时默认训练芯片为 NVIIDIA
    # 暂时添加默认值
    eval_config['epoch'] = 1
    eval_config['batch_size'] = 1
    eval_config['learning_rate'] = 0.01
    eval_config['device_type'] = get_accelerator(args.accelerator).get_kind.lower()
    eval_config['eval_height'] = metadata['inputSize']['height']
    eval_config['eval_width'] = metadata['inputSize']['width']
    eval_config['model_type'] = metadata['algorithmParameters']['networkArchitecture']
    ...
    eval_config['data_load']["train"] = [
        {"image_dir": "",
         "anno_path": [args.output_dataset_uri + "/train.txt"],
         "dataset_dir": "/",
         "sample_prob": 1.0,
         "label_description": ""
         }
    ...
    eval_config['data_load']["eval"] = [
        {"image_dir": "",
         "anno_path": [args.output_dataset_uri + "/val.txt"],
         "dataset_dir": "/",
         "sample_prob": 1.0,
         "label_description": ""
         }
