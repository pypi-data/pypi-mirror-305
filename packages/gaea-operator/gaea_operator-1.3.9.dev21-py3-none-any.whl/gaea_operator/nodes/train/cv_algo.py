...
from gaea_operator.config.generate_transform_config import KEY_EVAL_WIDTH, KEY_EVAL_HEIGHT
...
def train(args):
    ...
    try:
        train_config = _generate_train_config(args, train_advanced_parameters)
...
def _model_metadata(args, train_advanced_parameters, tracker_client, labels):
    ...
    create metadata
    """
    input_size = InputSize(width=train_advanced_parameters[KEY_EVAL_WIDTH],
                           height=train_advanced_parameters[KEY_EVAL_HEIGHT])
    ...
    bcelogger.info(f"Preprocess transforms: {transforms_list} input_size {input_size}")
...
# TODO 生成cv_algo配置待优秀
def _generate_train_config(args, train_advanced_parameters):
    ...
    train_config['epoch'] = int(train_config['epoch'])
    train_config['eval_height'] = int(train_config['eval_height'])
    train_config['eval_width'] = int(train_config['eval_width'])
    ...
    train_config['model_type'] = train_advanced_parameters[KEY_NETWORK_ARCHITECTURE]
    ...
    # TODO  anno_path 路径待优化
    train_config['data_load']["train"] = [
        {"image_dir": "",
         "anno_path": ["/home/windmill/tmp/dataset/train.txt"],
         "dataset_dir": "/",
         "sample_prob": 1.0,
         "label_description": ""
         }
    ...
    train_config['data_load']["eval"] = [
        {"image_dir": "",
         "anno_path": ["/home/windmill/tmp/dataset/val.txt"],
         "dataset_dir": "/",
         "sample_prob": 1.0,
         "label_description": ""
         }
