...
class MultiAttributeDataset(Dataset):
    ...
    def _label_valid(self, label_list: List[List]):
        ...
        for index, info in enumerate(info_list):
            for task_index, task in enumerate(info):
                if info_list[0][0][0] !=  task[0]:
                    bcelogger.error(f"The multi data set label task name is not consistent, \
                        please check {index} dataset {task_index} task")
                if info_list[0][0][1] !=  task[1]:
                    bcelogger.error(f"The multi data set label task anno key is not consistent, \
                        please check {index} dataset {task_index} task")
        # 解析 labels      
