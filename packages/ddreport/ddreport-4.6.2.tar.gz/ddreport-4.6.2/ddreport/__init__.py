...
def pytest_sessionfinish(session, exitstatus):
    ...
    if ddreport_path:
        # 报告内容标签替换
        test_result_str = json.dumps(test_result).replace('<', '&lt;').replace('>', '&gt;')
        # test_result_json = json.loads(test_result_str)
        # 报告路径
        ...
        with open(f'{template_path}', 'r', encoding='utf-8') as f:
            template = f.read()
        report_template = template.replace('templateData', test_result_str)
...
def node_handle(node, item, call):
    ...
    # 打印正确请求
    if _dict_data["temp_query"].get("ddquery") and _dict_data["apiresult"] is True:
        query_data = _dict_data["temp_query"]["ddquery"]
        if query_data:
            pro = Process()
