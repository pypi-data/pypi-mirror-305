...
def getAttributes(entity, filter=None):
    ...
    This function can be used to retrieve all attributes of an entity: ::
      ...
      >>> items = heimdall.getAttributes(enitity)  # retrieve all attributes

    To retrieve only *some* attributes, you can use a filter.
    A filter is a function which takes only one attribute as a parameter, and
    returns either ``True`` (we want this attribute to be part of the list
    returned by ``getAttributes``) or ``False`` (we don't want it). ::

      >>> import heimdall
      >>> ...  # create or load HERA tree
      >>> entity = heimdall.getEntity(tree, ...)  # retrieve an entity
      >>> def by_pid(attribute):  # create filtering function
      >>>     return attribute.get('pid') == 'dc:name'
      >>> # retrieve only attributes equivalent to a name
...
def createAttribute(entity, pid, **kwargs):
    ...
    :Keyword arguments:
        ...
        * **max** (``int``) -- (optional)
          Maximum occurences in an item: ``1`` means it is not repeatable, more
          (or no value) means it is; ``0`` or ``<min`` is undefined behaviour
        ...
        * **description** (``str|dict``) -- (optional) Description override;
          no value means the linked property ``description`` will be used
