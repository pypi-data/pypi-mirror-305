...
if __name__ == "__main__":
    ...
    # Sort and display by cumulative time
    ps_cumulative = pstats.Stats(profiler, stream=s_cumulative).sort_stats('cumulative')
    print("\nProfiling Results (Sorted by Cumulative Time):")
    ps_cumulative.print_stats(20)
    print(s_cumulative.getvalue())

    # Sort and display by total time
    ps_total = pstats.Stats(profiler, stream=s_total).sort_stats('tottime')
    print("\nProfiling Results (Sorted by Total Time):")
    ps_total.print_stats(20)
