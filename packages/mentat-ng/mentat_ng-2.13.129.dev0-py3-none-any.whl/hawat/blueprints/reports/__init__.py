...
def build_related_search_params(item):
    ...
    if item.statistics and all(field in item.statistics for field in ['sources', 'classes', 'targets']):
        if is_target:
            related_events_search_params.update({
                'target_addrs': ",".join(list(item.statistics['targets'].keys())),
                'target_classes': list(item.statistics['classes'].keys()),
            })
        else:
            related_events_search_params.update({
...
def section_event_search_url(data, item, section_name, ip):
    """
    Returns the URL for searching events related to the section of the report.
    """
    if item.type == mentat.const.REPORT_TYPE_TARGET:
        return flask.url_for('events.search',
                             dt_from=format_datetime_search(data['first_time']),
                             dt_to=format_datetime_search(data['last_time'], round_up=True),
                             target_addrs=",".join(ip for detector_data in data['detector_data'].values()
                                                   for ip in detector_data['Target']['ips']),
    ...
    def do_before_response(self, **kwargs):
        if 'item' in self.response_context and self.response_context['item']:
            self.response_context.update(
                ...
                REPORT_FIELDS_SOURCE_NUMBER_VIEW=mentat.const.REPORT_FIELDS_SOURCE_NUMBER_VIEW,
                REPORT_FIELDS_TARGET_NUMBER_VIEW=mentat.const.REPORT_FIELDS_TARGET_NUMBER_VIEW,
                REPORT_FIELDS_SOURCE_LIST=mentat.const.REPORT_FIELDS_SOURCE_LIST,
                REPORT_FIELDS_TARGET_LIST_VIEW=mentat.const.REPORT_FIELDS_TARGET_LIST_VIEW,
