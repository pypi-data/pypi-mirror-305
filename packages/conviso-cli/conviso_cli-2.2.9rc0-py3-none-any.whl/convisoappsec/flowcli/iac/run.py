            ...
            for issue in sarif_data:
                try:
                    ...
                    if error.code == 'RECORD_NOT_UNIQUE':
                        ...
                        LOGGER.warn(f"⚠️ Error creating issue. Our technical team has been notified.")
                        full_trace = traceback.format_exc()
                        try:
                            log_and_notify_ast_event(
                                flow_context=flow_context, company_id=company_id, asset_id=asset_id,
                    ...
                    LOGGER.warn(f"⚠️ Unexpected error creating issue: {e}. Our technical team has been notified.")
                    full_trace = traceback.format_exc()
                    try:
                        log_and_notify_ast_event(
                            flow_context=flow_context, company_id=company_id, asset_id=asset_id, ast_log=str(full_trace)
                        )
                    except Exception as log_error:
                        LOGGER.warn(f"⚠️ Failed to log and notify AST event: {log_error}")
                    continue

    msg = f"💬 {duplicated_issues} Issue/Issues ignored due to duplication."
    LOGGER.info(msg)
