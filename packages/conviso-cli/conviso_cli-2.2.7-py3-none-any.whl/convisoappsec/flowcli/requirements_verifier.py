...
class RequirementsVerifier:
    ...
    def prepare_context(flow_context, context, from_ast=False):
        ...
        if project_code:
            ...
            # Check for empty companies_filtered list
            if not companies_filtered:
                raise CreateDeployException(
                    "❌ Deploy not created. No companies found; please specify a valid company ID."
                )
            ...
            if new_management_flag not in company['customFeatures']:
                error_msg = "Deploy not created. The company '{}' does not have access to the new vulnerability management.".format(
                    company['label'])
                raise CreateDeployException(error_msg)

            if context.params['asset_name'] is not None:
                # if user uses --asset-name param or envvar CONVISO_ASSET_NAME, FLOW_ASSET_NAME
                ...
                # Check for empty assets list
                if not assets:
                    raise CreateDeployException("No assets found; unable to find or create asset.")
            ...
            if 'projects' not in asset or len(asset['projects']) == 0:
                ...
                # Check for empty project list after project creation
                if not project:
                    raise CreateDeployException("Failed to create or retrieve AST project.")

                project_code = project[0]['apiCode']
                RequirementsVerifier.update_project(project[0]['id'], asset['id'])
            else:
                ...
                for project in projects:
                    ...
                    # Check for empty project list if we try to create one
                    if not project:
                        raise CreateDeployException("Failed to create or retrieve AST project.")
