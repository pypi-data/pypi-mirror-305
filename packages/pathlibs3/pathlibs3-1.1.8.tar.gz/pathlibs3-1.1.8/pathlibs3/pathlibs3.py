...
class S3Path:
    ...
    def _is_dir(self) -> bool:
        # If only the bucket, then it is a folder
        if self.path == "":
            return True

        # If we cannot get the Head object then it is a folder
        try:
        ...
        # Of if the head has a content-type of "application/x-directory", then it is a folder
        if (
        ...
        # Or if we list more than one (object (the current folder/file is always listed)), then it is a folder
        response = self.client.list_objects_v2(Bucket=self.bucket, Prefix=self.path, Delimiter='/')
