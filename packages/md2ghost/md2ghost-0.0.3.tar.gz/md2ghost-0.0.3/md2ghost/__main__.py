...
class GhostClient:
    ...
    def __init__(self, base_url: str, token: str, tags: List[str]):
    ...
    def _post_data(self, article: Article, updated_at: Optional[str] = None):
        authors = re.findall(r"<([^@]+@[^>]+)>", str(subprocess.check_output(["git", "shortlog", "-s", "-e", "--", str(article.source)]), "utf-8"))
        html_suffix = ""
...
def main(base_url: str, token: str, input: Path, tags: List[str]):
    client = GhostClient(base_url, token, tags)
