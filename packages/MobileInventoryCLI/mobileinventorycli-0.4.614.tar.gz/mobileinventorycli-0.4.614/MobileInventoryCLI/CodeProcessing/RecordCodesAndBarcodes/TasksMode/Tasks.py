        ...
        with Session(ENGINE) as session:
            while True:
                ...
                if select in [None,]:
                    ...
                    while True:
                        try:
                            selector=Prompt.__init2__(None,func=FormBuilderMkText,ptext="which index?",helpText="number farthest from to the left of screen followed by /; enter will use the first Entry marked by a zero(0)",data="integer")
                            if selector in [None,]:
                                if not self.next_barcode():
                                    continue
                                else:
