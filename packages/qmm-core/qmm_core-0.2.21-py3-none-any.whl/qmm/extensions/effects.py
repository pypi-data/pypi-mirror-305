...
def define_state_space(
    ...
    if remove_disconnected:
        ...
        if len(connected) > 1:
            ...
            nodes_to_remove = [
                node for system in connected if system != largest for node in system
            ]
    ...
    while True:
        ...
        for node in list(G_def.nodes()):
            if G_def.nodes[node]["category"] == "state":
                if all(
                    G_def.nodes[pred]["category"] == "input"
                    for pred in G_def.predecessors(node)
                ):
                    G_def.nodes[node]["category"] = "input"
                    reclassified = True
                elif all(
                    G_def.nodes[succ]["category"] == "output"
                    for succ in G_def.successors(node)
                ):
...
def cumulative_effects(G: nx.DiGraph, form: str = "symbolic") -> sp.Matrix:
    ...
    cemat = sp.BlockMatrix(
        [[effects, effects * B], [C * effects, C * effects * B + D]]
    ).as_explicit()
    if form != "symbolic":
    ...
    for _ in range(n_sim * 100):
        ...
        if np.all(np.real(np.linalg.eigvals(sim_A)) < 0):
            try:
                ...
                if observe:
                    valid = all((node in state_nodes and ((tmat[node_idx[node], pert_idx] == 0 and obs == 0) or
                                 (obs != 0 and np.sign(effect[node_idx[node]]) == obs))) or
                                (node in output_nodes and ((tmat[len(state_nodes) + output_nodes.index(node), pert_idx] == 0
                                 and obs == 0) or (obs != 0 and np.sign(effect[len(state_nodes) + output_nodes.index(node)]) == obs)))
...
def simulation_effects(
    ...
    smat = sp.Matrix(smat)
    smat = sp.Matrix([[sp.nan if not tmat[i, j] else smat[i, j] for j in range(smat.cols)]
                      for i in range(smat.rows)])
    return smat
