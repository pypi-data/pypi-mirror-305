...
class TestBuilderTFM(unittest.TestCase):
    ...
    def test_BuilderTFM_init(self):
        ...
        bM_TFM.buildTFM(flag_build=False)

        self.assertEqual(hasattr(bM_TFM.builderTFM, 'verbose'), True)

        self.assertEqual(hasattr(bM_TFM.builderTFM, 'General'), True)
        self.assertEqual(hasattr(bM_TFM.builderTFM, 'Turns'), True)
        self.assertEqual(hasattr(bM_TFM.builderTFM, 'HalfTurns'), True)
        self.assertEqual(hasattr(bM_TFM.builderTFM, 'Strands'), True)
        self.assertEqual(hasattr(bM_TFM.builderTFM, 'Options'), True)

        self.assertEqual(hasattr(bM_TFM.builderTFM, 'PC'), True)
        self.assertEqual(hasattr(bM_TFM.builderTFM, 'IFCC'), True)
        self.assertEqual(hasattr(bM_TFM.builderTFM, 'ISCC'), True)
        self.assertEqual(hasattr(bM_TFM.builderTFM, 'ED'), True)
        self.assertEqual(hasattr(bM_TFM.builderTFM, 'Wedge'), True)
        self.assertEqual(hasattr(bM_TFM.builderTFM, 'CB'), True)
        self.assertEqual(hasattr(bM_TFM.builderTFM, 'CPS'), True)
        self.assertEqual(hasattr(bM_TFM.builderTFM, 'AlRing'), True)
        self.assertEqual(hasattr(bM_TFM.builderTFM, 'BS'), True)
        self.assertEqual(hasattr(bM_TFM.builderTFM, 'Shorts'), True)

        self.assertEqual(hasattr(bM_TFM.builderTFM, 'effs_cond'), True)
        self.assertEqual(hasattr(bM_TFM.builderTFM, 'frequency'), True)
        self.assertEqual(hasattr(bM_TFM.builderTFM, 'mu0'), True)
        self.assertEqual(hasattr(bM_TFM.builderTFM, 'print_nodes'), True)
    ...
    def test_setAttribute(self):
        ...
        for parameter in self.local_General:
            ...
            setattr(bM_TFM.builderTFM.General, parameter, true_value)
            # act
            test_value = bM_TFM.builderTFM._BuilderTFM__getAttribute('General', parameter)
            # assert
        ...
        for parameter in self.local_HalfTurns:
            ...
            setattr(bM_TFM.builderTFM.HalfTurns, parameter, true_value)
            # act
            test_value = bM_TFM.builderTFM._BuilderTFM__getAttribute('HalfTurns', parameter)
            # assert
            assert_two_parameters(true_value, test_value)

        for parameter in self.local_Strands:
            true_value = self.local_Strands[parameter]
            setattr(bM_TFM.builderTFM.Strands, parameter, true_value)
            # act
            test_value = bM_TFM.builderTFM._BuilderTFM__getAttribute('Strands', parameter)
            # assert
            assert_two_parameters(true_value, test_value)

        for parameter in self.local_Options:
            true_value = self.local_Options[parameter]
            setattr(bM_TFM.builderTFM.Options, parameter, true_value)
            # act
            test_value = bM_TFM.builderTFM._BuilderTFM__getAttribute('Options', parameter)
            # assert
            assert_two_parameters(true_value, test_value)
    ...
    def test_getAttribute(self):
        ...
        for parameter in self.local_General:
            true_value = self.local_General[parameter]
            # act
            bM_TFM.builderTFM._BuilderTFM__setAttribute('General', parameter, true_value)
            test_value = getattr(bM_TFM.builderTFM.General, parameter)
            # assert
            assert_two_parameters(true_value, test_value)
        ...
        for parameter in self.local_HalfTurns:
            true_value = self.local_HalfTurns[parameter]
            # act
            bM_TFM.builderTFM._BuilderTFM__setAttribute('HalfTurns', parameter, true_value)
            test_value = getattr(bM_TFM.builderTFM.HalfTurns, parameter)
            # assert
            assert_two_parameters(true_value, test_value)
        ...
        for parameter in self.local_Strands:
            true_value = self.local_Strands[parameter]
            # act
            bM_TFM.builderTFM._BuilderTFM__setAttribute('Strands', parameter, true_value)
            test_value = getattr(bM_TFM.builderTFM.Strands, parameter)
            # assert
            assert_two_parameters(true_value, test_value)
        ...
        for parameter in self.local_Options:
            true_value = self.local_Options[parameter]
            # act
            bM_TFM.builderTFM._BuilderTFM__setAttribute('Options', parameter, true_value)
            test_value = getattr(bM_TFM.builderTFM.Options, parameter)
            # assert
            assert_two_parameters(true_value, test_value)
    ...
    def test_assignAttribute(self):
        ...
        bM_TFM = BuilderModel(file_model_data=file_model_data_MBRD, case_model='magnet')
        bM_TFM.buildTFM(local_library_path=local_library_path, TFM_inputs=TFM_inputs, magnet_data=magnet_TFM, verbose=False)
        ...
        for parameter in self.local_Strands:
            ...
            if 'f_mag' not in parameter:
                ...
                assert_two_parameters(true_value, test_value[0, :25])
            print('PASSED')
    ...
    def test_assignMagnetData(self):
        ...
        file_model_data_MBRD = os.path.join(local_library_path, f'modelData_{magnet_TFM.name}.yaml')
        bM_TFM = BuilderModel(file_model_data=file_model_data_MBRD, case_model='magnet')
        bM_TFM.buildTFM(local_library_path=local_library_path, TFM_inputs=TFM_inputs, magnet_data=magnet_TFM,
                        verbose=False)
        ...
        for key, value in magnet_TFM.__dict__.items():
            if 'magnet_' in key and key != 'magnet_Couplings' and value is not None:
                ...
                test_value = getattr(bM_TFM.builderTFM, eff)
    ...
    def test_calculateEffect(self):
        ...
        if not os.path.exists(output_path):
            ...
            print(f'Directory {output_path} created')
        output_path_TFM = os.path.join(output_path, f'{magnet_TFM_name}.lib')
        bM_TFM.builderTFM._BuilderTFM__generate_library(output_path=output_path_TFM)

        for key, value in bM_TFM.builderTFM.Options.__dict__.items():
            if key == 'flag_BS' or key == 'flag_SC': continue
            setattr(bM_TFM.builderTFM.Options, key, True)
            eff = key.split('_')[-1]
            bM_TFM.builderTFM.change_coupling_parameter()
            dataclass = getattr(bM_TFM.builderTFM, eff)
    ...
    def test_assignTurnsToSections(self):
        ...
        if bM_TFM.builderTFM.HalfTurns.HalfTurns_to_sections.all() != bM_TFM.builderTFM.HalfTurns.HalfTurns_to_apertures.all():
        ...
        if bM_TFM.builderTFM.Strands.strands_to_sections.all() != bM_TFM.builderTFM.Strands.strands_to_apertures.all():
            raise Exception(
                f'Attribute strands_to_sections has to be equal to strands_to_apertures when key turn_to_section = None'
                f' in the input circuit yaml file')

        reference_HalfTurns_to_ap = bM_TFM.builderTFM.HalfTurns.HalfTurns_to_apertures
        reference_strands_to_ap = bM_TFM.builderTFM.Strands.strands_to_apertures
        ...
        if bM_TFM.builderTFM.HalfTurns.HalfTurns_to_sections.all() != np.tile(turns_to_sections, 2).astype(int).all():
            raise Exception(
        ...
        if bM_TFM.builderTFM.HalfTurns.HalfTurns_to_apertures.all() != reference_HalfTurns_to_ap.all():
            raise Exception(
        ...
        strands_to_sections = np.repeat(bM_TFM.builderTFM.HalfTurns.HalfTurns_to_sections, self.local_HalfTurns['n_strands'])
        if bM_TFM.builderTFM.HalfTurns.HalfTurns_to_sections.all() != strands_to_sections.all():
            raise Exception(
                f'Attribute strands_to_sections for turns_to_sections != None does not correspond to: '
                f'{strands_to_sections}')
        if bM_TFM.builderTFM.Strands.strands_to_apertures.all() != reference_strands_to_ap.all():
            raise Exception(
    ...
    def test_calculateInductanceSections(self):
        ...
        bM_TFM.buildTFM(local_library_path=local_library_path, TFM_inputs=TFM_inputs, magnet_data=magnet_TFM,
                        verbose=False)
        inductance_to_sections_dict = bM_TFM.builderTFM._BuilderTFM__calculate_Inductance_Sections()

        if round(bM_TFM.builderTFM.General.L_mag, 7) != L_mag_ref:
            raise Exception(f'Value of L_mag attribute in dataclass General does not correspond to its reference value: {L_mag_ref}')
        if not np.array_equal(np.round(bM_TFM.builderTFM.General.inductance_to_section, 6), np.round(inductance_to_section_ref, 6)):
            raise Exception(
        ...
        cont = 0
        for key, value in inductance_to_sections_dict.items():
            if np.round(value, 6) != np.round(inductance_to_section_ref[cont, cont], 6):
                raise Exception(f'In the dict returned by __calculate_Inductance_sections function, key {key} has not its expected value {inductance_to_section_ref[cont, cont]}')
            cont += 1
    ...
    def test_Shorts(self):
        ...
        bM_TFM.buildTFM(local_library_path=local_library_path, TFM_inputs=TFM_inputs, magnet_data=magnet_TFM,
                        verbose=False, output_path=output_path_TFM)
