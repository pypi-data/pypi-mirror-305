...
class NodeRunner:
    ...
    def _download_node(self):
        ...
        print("DOWNLOADED NODE BINARY")
        os.system(f"ls -l {self.base_dir}")
        ...
        print("EXTRACED TARBALL")
        os.system(f"ls -l {self.base_dir}")
        extracted_dir = next(self.base_dir.glob(f"node-v{self.node_version}-*"))
        extracted_dir.rename(self.node_dir)
        print("RENAMED EXTRACTED FOLDER")
        os.system(f"ls -l {self.base_dir}")

        # I think something weird happens in Github Action runners that removes the tar.gz extension
        # I haven't seen this happen anywhere else, and it's not a huge deal to leave the tar.gz file
        # there so this is my work around
