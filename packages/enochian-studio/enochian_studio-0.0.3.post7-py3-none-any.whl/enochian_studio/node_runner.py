    ...
    def _download_node(self):
        ...
        with FileLock(lock_file):
            ...
            if self.node_dir.exists():
                if not self.node_binary.exists() or not self.npm_binary.exists:
                    print("Node dir exists but binaries do not, cleaning up...")
                    pprint.pp(os.listdir(self.node_dir.parent))
                    ...
                    print(f"Node.js {self.node_version} is already installed at {self.node_dir}")
                    return
            ...
            try:
                ...
                pprint.pp(f"DOWNLOADED: {os.listdir(self.base_dir)}")
                ...
                pprint.pp(f"EXTRACTED: {os.listdir(self.base_dir)}")
                ...
                print(f"RENAMED: {os.listdir(self.base_dir)}\nSHOULD SEE {self.node_dir}")
            finally:
                # Clean up the downloaded file even if extraction fails
                if downloaded_file.exists():
                    print("DOWNLOADED FILE DOES EXIST")
