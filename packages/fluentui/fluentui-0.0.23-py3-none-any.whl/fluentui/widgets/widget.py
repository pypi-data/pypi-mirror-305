...
class WidgetMix(ObjectMix):
    ...
    def setStyleSheet(self, style: dict | Qss) -> None:
        self.qss = style if isinstance(style, dict) else Qss(style)
        super().setStyleSheet(f'{self.__class__.__name__} ' + style.build())

    def __init_size(self, size='', w='', h=''):
        size, w, h = f'{size}', f'{w}', f'{h}'  # 将传入的参数转为字符串
