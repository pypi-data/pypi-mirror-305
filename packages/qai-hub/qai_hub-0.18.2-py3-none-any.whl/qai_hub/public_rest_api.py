...
def _get_progress_bar(filename, file_size, block_size, verbose):
    if verbose:
        return tqdm(
            total=file_size,
            unit="B",
...
def _download_to_verified_path(
    ...
    # get() has built-in retries, but those cover only the initial
    # header response.  Any exceptions that occur while streaming content
    # are caught by the @retry_with_backoff handler so we can start over.
