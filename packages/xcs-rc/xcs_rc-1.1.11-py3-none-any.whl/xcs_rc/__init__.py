...
import re
from os import path
from random import choice, choices, shuffle, random, randrange
from numpy import ndarray, int64, float64, array, max as npmax, sum as npsum
from pandas import Series, DataFrame

# TODO: polish feature selections
...
class Agent:
        ...
        for cl in cls[title_line[-1]:]:
            if cl[0].isdigit():
                ...
                self.pop.add(Classifier(
                    cond=floatify(val_cond[:-1]), act=int(val[z - 6]), pred=float(val[z - 5]),
    ...
    def save(self, fname, title="", save_mode='w', report=False):
        """
        Save current population to a file.
        :param fname: file name
        :param title: title above the classifers data
        :param save_mode: w for write, a for append
        :param report: print save status if True
        :return: None
        """
        self.pop.save(fname, title, save_mode, report=report)
...
class ClassifierSet(object):
    ...
    def get_header(self, colnames=None):
        ...
        head = SEPARATOR.join(cols + ["act", "pred", "fit", "prederr", "num", "exp", "actsetsize"])
        return head

    def to_list(self):
        return [cl.to_list() for cl in self.cl]

    def print(self, title="", header=True, per_action=False, colnames=None):
        if per_action:
            counts = []
