...
class DataLink(object):
    ...
    def _set_cxn_info(self, cfg_name=None, db_file=None):
        ...
        try:
            ...
            raise FileNotFoundError(f"Cannot open the DB config file: {db_file}. Please check if the file exists and the path is correct.")
        except json.JSONDecodeError:
            raise ValueError(f"Failed to decode JSON from the DB config file: {db_file}. Please ensure the file contains valid JSON.")
        except Exception as e:
            raise RuntimeError(f"An unexpected error occurred while reading the DB config file: {db_file}. Error: {e}")
