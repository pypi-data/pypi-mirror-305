...
class NepTrainWorker:
    ...
    def vasp_work_path(self):
        return os.path.join(self.generation_path,"vasp")

    @property
    def gpumd_work_path(self):
        return os.path.join(self.generation_path, "gpumd")
    @property
    def nep_work_path(self):
        return os.path.join(self.generation_path, "nep")

    @property
    def last_generation_path(self):
        return os.path.join(os.path.abspath(self.config.get("work_path")), f"Generation-{self.generation-1}")

    @property
    def generation_path(self):
        return os.path.join(os.path.abspath(self.config.get("work_path")), f"Generation-{self.generation}")
