...
from featree.config import GenTreeConfig, GraphRelationAlgo
...
class _TreeBase(object):
    ...
    def __init__(
        ...
        self.config: GenTreeConfig = config
    ...
    def build_keyword_dict(self):
        ...
        for each_cluster in clusters:
            each_keywords = each_cluster.data.keywords
    ...
    def calc_keywords(self):
        ...
        for each_cluster_id, each_cluster in zip(cluster_ids, clusters):
            ...
            # each_symbols = each_cluster.data.symbols

            files_without_extension = [os.path.splitext(file)[0] for file in each_files]
            files_without_extension_repr = "\n".join(files_without_extension)
            docs.append(files_without_extension_repr)

        kw_model = KeyBERT(model="all-MiniLM-L6-v2")
        # supress warning
        os.putenv("TOKENIZERS_PARALLELISM", "False")
        word_score_list = kw_model.extract_keywords(
            vectorizer=TfidfVectorizer(
                stop_words=stopword_set, token_pattern=r"(?u)\b\w[\w-]+\b"
            ),
            docs=docs,
            stop_words=stopword_set,
        )
        for each_cluster, word_score in zip(clusters, word_score_list):
            filtered_keywords = [
                each[0]
                for each in word_score
                if each[1] > self.config.keyword_filter_threshold
            ]

            if not filtered_keywords:
                max_keyword = max(word_score, key=lambda x: x[1])[0]
                each_cluster.data.keywords = [max_keyword]
            else:
                each_cluster.data.keywords = filtered_keywords
    ...
    def calc_all_files_edges(self):
        ...
        for each_cluster_id, each_cluster in zip(cluster_ids, clusters):
            ...
            for each_file in each_files:
                neighbors = list(self._origin_di_graph.neighbors(each_file))
                for neighbor in neighbors:
                    for each_target_cluster_id, target_cluster in zip(
                        cluster_ids, clusters
                    ):
                        if neighbor in target_cluster.data.files:
                            edge = (each_cluster_id, each_target_cluster_id)
                            if edge in edge_count:
                                edge_count[edge] += 1
                            else:
...
def gen_tree(config: GenTreeConfig = None) -> Featree:
    ...
    if config.graph_relation_algo == GraphRelationAlgo.DEFAULT:
        ret.calc_leader_files()
    elif config.graph_relation_algo == GraphRelationAlgo.ALL:
        ret.calc_all_files_edges()
    else:
        raise RuntimeError(f"invalid algo: {config.graph_relation_algo}")
