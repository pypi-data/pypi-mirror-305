...
class YOLO2ONNXExporter:
    def __init__(
        ...
        num_classes: int = 1,
        crop_pecentages: List[float] = None,
        image_shape: List[float] = None,
        onnx_opset: int = 18,
        ...
        Args:
            torch_model_path (str): Path to the torch model file (.pt).
            num_classes (int, optional): Number of classes in the model. Defaults to 1.
            crop_pecentages (List[float], optional): List of percentages (height, width) for center crop. Defaults to None.
            image_shape (List[float], optional): Height, width
            onnx_opset (int, optional): ONNX opset version. Defaults to 18.
            onnx_model_path (str, optional): Path to the existing ONNX model file. Defaults to None.
        """
        ...
        self.crop_pecentages = crop_pecentages  # percentage for center crop
        ...
        self.onnx_model = (
            self.load_onnx_model() if onnx_model_path else None
        )  # onnx model
    ...
    Because we need to execute the model to determine the output shape in order to add the correct post-processing"""
        ...
        # TODO: h_in and w_in are not image size, they are the input size of the model.
        # NOTE: ConvertImageToBGR has to be done first before Cropping
        pre_processing_steps = [
            ConvertImageToBGR(),  # jpg/png image to BGR in HWC layout
        ]
        if self.crop_pecentages:
            if not self.image_shape:
                raise ValueError("Provide image shape as (h, w)")
            if len(self.crop_pecentages) == 2:
                # convert percentage to pixels based on original image size
                crop_area_pixel_coords = [
                    int(self.image_shape[0] * self.crop_pecentages[0]),
                    int(self.image_shape[1] * self.crop_pecentages[1]),
                ]
                pre_processing_steps.append(
                    # (
                        CenterCrop(
                            height=crop_area_pixel_coords[0],
                            width=crop_area_pixel_coords[1],
                        ),
                ...
                raise ValueError(
                    "The crop_pecentages should be a list of two float elements."
                )
        ...
        print("\n\n========================================================")
        print(pre_processing_steps)
        print("========================================================\n\n")

        self.pipeline.add_pre_processing(pre_processing_steps)
    ...
    def add_post_processing_object_detection(
        self,
        ...
        if result_as_image_format is not None:
            post_processing_steps += [
                ...
                (
                    ...
                    [
                        utils.IoMapEntry(
                            "CenterCrop", producer_idx=0, consumer_idx=0
                        ),
                        utils.IoMapEntry(
    ...
    def add_post_processing_segmentation(
        self,
        result_as_image_format: str = None,
        iou_threshold: float = 0.7,
        score_threshold: float = 0.5,
        ...
        if result_as_image_format is not None:
            post_processing_steps += [
                ...
                # CenterCrop(
                #         int(2048 * self.crop_pecentages[0]),
                #         int(2448 * self.crop_pecentages[1]),
                #     ),
                # Encode to jpg/png
                ConvertBGRToImage(image_format=result_as_image_format),
            ]
    ...
    def add_post_processing(
        ...
        if self.task == "classify":
            ...
            self.add_post_processing_object_detection(
                result_as_image_format, iou_threshold, score_threshold
            )
        elif self.task == "segment":
            self.add_post_processing_segmentation(
                result_as_image_format, iou_threshold, score_threshold
            )
    ...
    def export_to_onnx_extensions(
        ...
        #
        if onnx_model_name is not None:
            output_file = Path(onnx_model_name).with_suffix(
