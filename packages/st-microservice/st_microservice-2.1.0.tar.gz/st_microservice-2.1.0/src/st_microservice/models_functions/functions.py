...
from ..querybuilder import (
    Table, Column, Term, And, In, Tuple, DBTypes, OnConflict,
...
async def insert[T: BaseModel](db: DBInterface, model: type[T], obj: T, on_conflict: OnConflict | None = None):
    obj_dict = asdict(obj)
    q = Insert(model).set_columns_from_table().values(**{key: KeyParameter(key) for key in obj_dict}).on_conflict(on_conflict)
    await db.execute(*q.to_sql(**obj_dict))


async def insert_many[T: BaseModel](db: DBInterface, model: type[T], objs: Iterable[T], on_conflict: OnConflict | None = None):
    obj_dicts = [asdict(obj) for obj in objs]
    if not obj_dicts:  # Ensure at least one
        return
    q = Insert(model).set_columns_from_table().values(**{key: KeyParameter(key) for key in obj_dicts[0]}).on_conflict(on_conflict)
