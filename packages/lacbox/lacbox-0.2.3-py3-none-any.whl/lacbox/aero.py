...
def _get_chord_poly(chord_interp, r_root, c_root, c_max, r_range):
    ...
    # Get poly
    chord_poly = _get_poly(r_root, c_root, r_start, chord_interp(
        r_start), chord_interp(r_start, 1))
    return r_start, chord_poly


def _find_r_start(chord_interp, r_root, c_root, c_max, r_range):
    # Solve for r_start
    return brenth(_r_start_obj, *r_range,
                  args=(chord_interp, r_root, c_root, c_max))


def _r_start_obj(r_start, chord_interp, r_root, c_root, c_max):
    # Get poly
    poly = _get_poly(r_root, c_root, r_start, chord_interp(
        r_start), chord_interp(r_start, 1))
...
def _get_poly(r_root, c_root,
              r_start, c_start,
              dcdr_start):
    return poly_fit([r_root, r_root, r_start, r_start],
