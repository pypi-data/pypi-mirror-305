...
from lion_core.communication import MESSAGE_FIELDS, MailManager, RoledMessage
...
class Session(BaseSession):
    ...
    def new_branch(
        ...
        user: IDTypes.SenderRecipient = None,
        name: str | None = None,
        imodel: iModel = None,
    ...
    def split_branch(self, branch: IDTypes.Ref) -> Branch:
        """
    ...
    def change_default_branch(self, branch: IDTypes.Ref):
        """
    ...
    def collect(self, from_: IDTypes.RefSeq = None):
        """
    ...
    def send(self, to_: IDTypes.RefSeq = None):
        """
    ...
    def to_df(self, branches: IDTypes.RefSeq = None) -> pd.DataFrame:
        out = self.concat_messages(branches=branches)
        return out.to_df(columns=MESSAGE_FIELDS)

    def concat_messages(
        self, branches: IDTypes.RefSeq = None
    ) -> Pile[RoledMessage]:
        if not branches:
