...
.. function:: py_limited_api()
   ...
   Returns a tuple of two integers. First integer describes the
   minimum supported Python Stable ABI. The seconds number
   describes what version of Python was used to compile the
   module.

   Both integers can easily be interpreted using something like
   ``hex(berkeleydb.py_limited_api()[0])``.
...
Oracle Berkeley DB is very powerful and versatile, but it is
complex to use correctly. :Oracle:`Oracle documentation
<index.html>` is very complete. Please, review it.
