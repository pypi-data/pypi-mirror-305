...
class AdaptiveContextNorm(tf.keras.layers.Layer):
    def __init__(self, num_contexts, epsilon=1e-3, momentum=0.9, **kwargs):
        self.num_contexts = num_contexts
        self.epsilon = epsilon
        self.momentum = momentum
        super(AdaptiveContextNorm, self).__init__(**kwargs)
    ...
    def build(self, input_shape):
        ...
        self.prior = self.add_weight(
            name='prior',
            shape=(self.num_contexts, 1),
            initializer=self.custom_prior_initializer,
            trainable=True
        )
        super(AdaptiveContextNorm, self).build(input_shape)
    ...
    def get_config(self):
        config = super(AdaptiveContextNorm, self).get_config().copy()
        config.update(
            {
