...
def start_packing_process(data_dir, cache_dir):
    ...
    try:
        while True:
            ...
            # Quét mã QR từ khung hình
            label_text = qr.capture_label(frame_with_timestamp_and_green_frame)
            if label_text:
            ...
            if current_tracking_id:
                frame_with_timestamp_and_green_frame = display_tracking_id_on_frame.display_tracking_id_on_frame(
                    frame_with_timestamp_and_green_frame, current_tracking_id
                )
