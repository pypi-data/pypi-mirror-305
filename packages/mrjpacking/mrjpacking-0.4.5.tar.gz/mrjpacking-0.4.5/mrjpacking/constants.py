    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if version.parse(latest_version) > version.parse(current_version):
                print(f"Đã có phiên bản mới {latest_version}. Đang cập nhật...")
                update_package()
            else:
                print("Đã có phiên bản mới nhất.")
        else:
            print("Không thể kiểm tra phiên bản mới từ PyPI.")
    except Exception as e:
        print("Có lỗi xảy ra khi kiểm tra phiên bản:", e)
...
def update_package():
    try:
        ...
        print("Có lỗi xảy ra khi cập nhật:", e)
