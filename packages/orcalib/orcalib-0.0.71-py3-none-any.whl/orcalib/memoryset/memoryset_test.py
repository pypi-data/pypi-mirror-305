...
from .memoryset import LabeledMemoryset, _get_embedding_hash
...
def memoryset():
    ...
    yield memoryset
    memoryset._drop_database(yes_i_am_sure=True)
...
def test_insert_and_lookup_multimodal(memoryset):
    ...
    assert _images_are_approximately_equal(result2[0][0].value, test_image)  # type: ignore -- we know value is an image
...
def test_existing_metadata(memoryset):
    ...
    memoryset_reconnect = LabeledMemoryset(memoryset.url, embedding_model=memoryset.embedding_model)
