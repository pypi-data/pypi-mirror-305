...
class DBTool(object):
            ...
            with self.get_connection() as connection:
                ...
                if query_string is None:
                    ...
                    try:
                        ...
                        LOGGER.error('Optimized read failure = {}', tb)
                    ...
                    try:
                        ...
                        tb = normEx.__traceback__
                        LOGGER.error('Optimazed read failure = {}', tb)
                        raise DBToolException(normEx).with_traceback(tb)
    ...
    def bulk_insert(self, data_df: pd.DataFrame, db_table: str, primary_keys: list, truncate=False):
        ...
        try:
            ...
            tb = err.__traceback__
            LOGGER.error('Failed executing bulk insert: {}', tb)
            raise err
        except Exception as err:
            tb = err.__traceback__
            LOGGER.error('Failed: executing bulk_insert to table: {} - {}', db_table, tb)
...
class DBToolFactory(object):
    ...
    def get_tool(self, ds_key, verbose=False) -> DBTool:
        ...
        try:
            ...
            tb = err.__traceback__
            LOGGER.error('Failed executing get_tool for datasource key: {} - {}', ds_key, tb)
            raise err
        except Exception as err:
            tb = err.__traceback__
            LOGGER.error('DBToolFactory encountered a problem configuring datasource with key = {}, {}', ds_key, tb)
            raise DBToolException(err).with_traceback(tb)
...
class DSWrapper(object):
    ...
    def apply_to_datasource(self, data_df, ds_config=None):
        ...
        try:
            ...
            tb = err.__traceback__
            LOGGER.error('Failed executing apply_to_datasource: {}', tb)
            raise err
        except Exception as err:
            tb = err.__traceback__
            LOGGER.error('Failure: {}', tb)
