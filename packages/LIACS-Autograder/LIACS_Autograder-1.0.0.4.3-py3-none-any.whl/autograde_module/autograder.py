...
class BrightspaceHandler():
    ...
    def get_student_grade(self, row):
        ...
        """
        try:
            return re.search(f"(?<=s{self.config['student_n_format']}{self.grade_config['brightspace_delimiter']})\d+(?={self.grade_config['brightspace_delimiter']})", row, flags=re.IGNORECASE).group(0)
        except AttributeError:
