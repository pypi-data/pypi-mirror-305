...
def get_result_by_config(pos_config):
    ...
    df = pd.read_csv(conf[file_info[0]]['pos_file'])
    mc_file = conf[file_info[0]]['mc_file']
    output_file = conf[file_info[0]]['output_file']
    for column in file_info[1:]:
        ...
        meshtally = scale_with_multiplier(meshtally, conf[column].getfloat('multiplier'), particle)
        result_pos = np.array([])
        error_pos = np.array([])
        result_total_pos = np.array([])
        error_total_pos = np.array([])
        for i, pos in enumerate(df['Coordinate']):
            print(f"        dealing with {i}-th position: {pos}")
            # check in range
            if pos[0] < meshtally.x_bounds[0] or pos[0]>meshtally.x_bounds[-1]:
                raise ValueError(f"FATAL: position {pos} out of x-dimension range")
            if pos[1] < meshtally.y_bounds[0] or pos[1]>meshtally.y_bounds[-1]:
                raise ValueError(f"FATAL: position {pos} out of y-dimension range")
            if pos[2] < meshtally.z_bounds[0] or pos[2]>meshtally.z_bounds[-1]:
                raise ValueError(f"FATAL: position {pos} out of z-dimension range")
            midx = calc_midx(pos, meshtally.x_bounds,
                        meshtally.y_bounds, meshtally.z_bounds)
            if midx<0:
                raise ValueError(f"FATAL: position {pos} with calculated midx {midx} wrong!")
            ptype = get_ptype(particle)
            result_tag_name = f"{ptype}_result"
            ...
            if i == 0:
                ...
                result_pos = np.vstack((result_pos, result))
                error_pos = np.vstack((error_pos, error))
            result_total_tag_name = f"{ptype}_total_result"
            ...
            result_total_pos = np.append(result_total_pos, result_total)
            error_total_pos = np.append(error_total_pos, error_total)
        result_pos = result_pos.transpose()
        error_pos = error_pos.transpose()
        for j in range(len(meshtally.e_bounds)-1):
            df[f"{conf[column]['id_name']}_e_{meshtally.e_bounds[j+1]}_result"] = result_pos[j]
            df[f"{conf[column]['id_name']}_e_{meshtally.e_bounds[j+1]}_error"] = error_pos[j]
        df[f"{conf[column]['id_name']}_{result_total_tag_name}"] = result_total_pos
        df[f"{conf[column]['id_name']}_{error_total_tag_name}"] = error_total_pos
