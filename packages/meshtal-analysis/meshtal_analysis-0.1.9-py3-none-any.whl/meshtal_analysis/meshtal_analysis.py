...
def get_result_by_config(pos_config):
    ...
    for column in file_info[1:]:
        ...
        tags = create_tags(conf[column].getint('tally_id'), particle=particle)
        meshtal = MeshtalWithNumber(mc_file, tags=tags, tally_number=conf[column].getint('tally_id'))
        meshtally = meshtal.tally[conf[column].getint('tally_id')]
