...
def get_result_by_config(pos_config):
    ...
    for column in file_info[1:]:
        ...
        for j in range(len(meshtally.e_bounds)-1):
            df[f"{pos_name}_e_{meshtally.e_bounds[j+1]}_result"] = result_pos[j]
            df[f"{pos_name}_e_{meshtally.e_bounds[j+1]}_error"] = error_pos[j]
        df[f"{pos_name}_{result_total_tag_name}"] = result_total_pos
        df[f"{pos_name}_{error_total_tag_name}"] = error_total_pos
