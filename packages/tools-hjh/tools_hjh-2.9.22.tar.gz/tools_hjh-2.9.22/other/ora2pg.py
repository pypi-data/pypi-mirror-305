...
max_page_num = int(conf_map['max_page_num'])
...
def copy_data():
    ...
    for table_mess in tables_metadata:
        mess = table_mess[1]
        pp.run(get_data_from_oracle, table_mess, process_name=mess)
    while True:
                ...
                if max_page_num == 1:
                    ...
                    if page_num <= max_page_num:
                        ...
                        for page in range(1, max_page_num + 1):
                            tables_metadata.append((table_map, mess, table_scn, table_metadata, page, page_rn))
        # 其他类型
        else:
            table_metadata = None
            mess = src_schema + '."' + src_table + '"'
...
def compare_data_number():
    ...
    if run_mode == 'copy_data':
        ...
        if scn_time != 0 and if_only_scn == 'true':
