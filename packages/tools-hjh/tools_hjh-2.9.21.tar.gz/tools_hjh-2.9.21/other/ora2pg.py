...
def copy_data():
    ...
    if if_truncate == 'true':
        ...
        for table_map in table_map_list:
            tp.run(truncate_table, (table_map,))
        tp.wait()
    ...
    tp = ThreadPool(parallel_num, while_wait_time=0.1)
    for table_map in table_map_list:
        tp.run(get_table_metadata, (table_map, global_scn), thread_name=table_map[0] + '.' + table_map[1])
    while True:
    ...
    for table_mess in tables_metadata:
        table_map = table_mess[0]
        mess = table_mess[1]
        table_scn = table_mess[2]
        table_metadata = table_mess[3]
        pp.run(get_data_from_oracle, (table_map, mess, table_scn, table_metadata), process_name=mess)
    while True:
        log.info('复制数据，尚未完成的任务：' + str(pp.get_running_name()))
...
# 表分析程序
def get_table_metadata(table_map, global_scn):
    try:
                ...
                for partition in partition_mess['partitions']:
                    ...
                    if len(subpartitions) > 0 and smallest_object == 'subpartition':
                        for subpartition in subpartitions:
                            ...
                            tables_metadata.append((table_map, mess, table_scn, table_metadata))
                    else:
...
def get_data_from_oracle(table_map, mess, table_scn, table_metadata=None, page=None, page_rn=None):
    ...
    try:
        ...
        i = 1
        tp = ThreadPool(3, while_wait_time=0.1)
        while True:
            if page is None:
                my_mess = mess + '(' + str(i) + ')'
            else:
        ...
        log.error('到Oracle获取数据失败', mess, select_sql, str(_))
...
def save_to_pg(table_map, rows, cols, mess, select_time, table_scn):
    ...
    try:
        ...
        log.error('写入PG过程中，连接PG失败', mess, str(_))
        return
    ...
    try:
        ...
        try:
            ...
            log.error('写入PG失败', mess, str(_).replace('\n', ' '))
            dst_db.close()
            return
    ...
    if src_where == '1=1':
        src_where = ''
    log.info('写入PG成功', mess, src_where, num, '读耗时=' + str(round(select_time, 2)) + 's', '写耗时=' + str(round(save_time, 2)) + 's', 'scn=' + table_scn)
...
def copy_index():
    ...
    for table_map in table_map_list:
        src_schema = table_map[0]
        dst_cur.execute('create schema if not exists ' + src_schema)
        dst_conn.commit()
...
def copy_table():
    ...
    tp = ThreadPool(parallel_num, while_wait_time=0.1)
    for table_map in table_map_list:
        tp.run(copy_one_table, (table_map,))
    tp.wait()
    src_db.close()
    dst_cur.close()
    dst_conn.close()
...
def copy_constraint():
    ...
    def copy_one_constraint_1(table_map):
        src_schema = table_map[0]
        src_table = table_map[1]
        dst_schema = table_map[2].lower()
        dst_table = table_map[3].lower()
    ...
    def copy_one_constraint_2(table_map):
        src_schema = table_map[0]
        src_table = table_map[1]
        dst_schema = table_map[2].lower()
        dst_table = table_map[3].lower()
    ...
    tp = ThreadPool(parallel_num, while_wait_time=0.1)
    for table_map in table_map_list:
        tp.run(copy_one_constraint_1, (table_map,))
    tp.wait()
    
    for table_map in table_map_list:
        copy_one_constraint_2(table_map)
...
def compare_data_number():
    ...
    def count(db, sql, k):
        ...
        try:
            ...
            num = -1
            e1 = str(_)
        tables_data_num[k][2] = num
        log.info('表数据量是否一致：' + str(tables_data_num[k][1] == tables_data_num[k][2]), k, 'src=' + str(tables_data_num[k][1]), 'dst=' + str(tables_data_num[k][2]), 'scn=' + str(tables_data_num[k][0]), str(e1))
    ...
    def count2(table_map, src_db, dst_db, global_scn):
        ...
        report_table_id = src_schema + '.' + src_table + '-->' + dst_schema + '.' + dst_table
        tables_data_num[report_table_id] = [global_scn, src_num, dst_num]
        log.info('表数据量是否一致：' + str(tables_data_num[report_table_id][1] == tables_data_num[report_table_id][2]), report_table_id, 'src=' + str(tables_data_num[report_table_id][1]), 'dst=' + str(tables_data_num[report_table_id][2]), 'scn=' + global_scn, str(e1), str(e2))
...
def compare_index():
    ...
    def compare_index_one(table_map):
        src_schema = table_map[0]
        src_table = table_map[1]
        dst_schema = table_map[2].lower()
        dst_table = table_map[3].lower()
        try:
            ...
            log.error('比较索引数量时，连接数据库失败', str(_))
    ...
    tp = ThreadPool(parallel_num, while_wait_time=0.1)
    for table_map in table_map_list:
        tp.run(compare_index_one, (table_map,))
    tp.wait()
...
def compare_constraint():
    ...
    def compare_constraint_one(table_map):
        src_schema = table_map[0]
        src_table = table_map[1]
        dst_schema = table_map[2].lower()
        dst_table = table_map[3].lower()
    ...
    tp = ThreadPool(parallel_num, while_wait_time=0.1)
    for table_map in table_map_list:
        tp.run(compare_constraint_one, (table_map,))
    tp.wait()
