        ...
        try:
            ...
            while not await self.is_stopped():
                ...
                if now - last_queues_update > 5:
                    ...
                    #     if (
                    #         qd.new_plugin_max_instances is not None
                    #         and qd.plugin_max_instances != qd.new_plugin_max_instances
                    #     ):
                    #         logger.info(
                    #             f"restarting queue with prefetch_{qd.plugin_max_instances} => {qd.new_plugin_max_instances} {session_id=}"
                    #         )
                    #         qd.plugin_max_instances = qd.new_plugin_max_instances
                    #         qd.new_plugin_max_instances = None
                    ...
                    #logger.info("last_queues_update finish")
            ...
            if message.routing_key == self.cfg.WORKER_STUDY_URL_TASKS_R_QUEUE_NAME:
                ...
                if storage_limit is not None:
                    ...
                    if cur_size >= storage_limit:
                        logger.info(f"Out of storage size limit for {session.id=}: {cur_size} >= {storage_limit}")
                        plugin.is_busy = False
                ...
                #     if self.todo_queues[qm.session_id].new_plugin_max_instances is None:
                #         self.todo_queues[qm.session_id].new_plugin_max_instances = max_instances
                    ...
                    async for process_res in plugin.process(task):
                        ...
                        if isinstance(process_res, CrawlerContent):
                            ...
                            if storage_limit is not None and time.time() - last_storage_size_check > 10:
                                ...
                                if await storage.get_total_size() > storage_limit:
                                    plugin.is_busy = False
                                    await self._reject_message(message, qm.session_id, requeue=True)
                                    return ProcessTodoMessageResult.TryAgain
        ...
        if content_state is None or (
            ...
            if not cc.content:
                ...
                with tempfile.TemporaryFile("wb+") as tmp:
                    try:
                        async for chunk in plugin.astream(download_url, expected_type=cc.type, timeout=3):
                            tmp.write(chunk)
                            if await stopper():
                                break
                        cc.content = tmp
    ...
    def _get_plugin_config_entry(self, cls_name):
        ...
        if config_entry is not None:
            ...
            if "storage_limit" in config_raw:
                config_raw["storage_limit"] = self.parse_size(config_raw["storage_limit"])
            else:
                config_raw["storage_limit"] = self.parse_size(self.cfg.PLUGIN_STORAGE_LIMIT_DEFAULT)
        ...
        async with self.plugins_lock:
            for plugin_data in self.plugins:
                if plugin_data.cls[0] != "DefaultPlugin":
                    ...
                    if task.force_plugin == plugin_data.cls[0] or plugin_data.cls[1].is_supported(task.url):
