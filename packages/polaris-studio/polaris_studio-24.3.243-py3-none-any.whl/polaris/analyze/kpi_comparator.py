...
class KpiComparator:
    ...
    def plot_tnc(self, **kwargs):
        ...
        df = self._get_results("tnc_times_by_tnc_operator", **kwargs)
        df["operator_mode"] = df["tnc_operator"] + "-mode_" + df["service_mode"].astype(str)

        if df is None or df.empty:
            logging.warning("There were no results for 'tnc_times_by_tnc_operator'")
            return
    ...
    def plot_calibration_timing(self, **kwargs):
        ...
        def f(type, ax, title):
            self._overlay_barplots(df[df["period"] == type], "act_type", ax, title)

        fig, axes = plt.subplots(nrows=3, ncols=2, figsize=(20, 8), sharey="all", sharex="all")

        f("NIGHT", axes[0][0], "00:00 to 06:00")
        f("AMPEAK", axes[0][1], "06:00 to 09:00")
        f("AMOFFPEAK", axes[1][0], "09:00 to 12:00")
        f("PMOFFPEAK", axes[1][1], "12:00 to 16:00")
        f("PMPEAK", axes[2][0], "16:00 to 19:00")
        f("EVENING", axes[2][1], "19:00 to 24:00")

        KpiComparator._style_axes(axes[0], rotate_x_labels=90, fontsize=12)
        KpiComparator._style_axes(axes[1], rotate_x_labels=90, fontsize=12)
        KpiComparator._style_axes(axes[2], rotate_x_labels=90, fontsize=12)
