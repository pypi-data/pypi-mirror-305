...
class WebullTA:
    ...
    async def get_ta_signals(self, ticker, interval, close_price):
        ...
        try:
            ...
            signals = {
                ...
                'Ultimate Oscillator': ultimate_signal,
            }
    ...
    async def all_signals(ticker, interval:str='d', headers=None):
        try:
            df = await trading.async_get_td9(ticker, headers=headers, interval=interval)

            # Add momentum and volume indicators
            momentum_ta = ta.add_momentum_ta(df, high='High', low='Low', close='Close', volume='Volume', fillna=True, vectorized=True)
            ease_of_movement = ta.volume.ease_of_movement(df['High'], low=df['Low'], volume=df['Volume'], fillna=True)
            momentum_ta['ease_of_movement'] = ease_of_movement

            force_index = ta.volume.force_index(close=df['Close'], volume=df['Volume'], fillna=True)
            momentum_ta['force_index'] = force_index

            volume_price_trend = ta.volume.volume_price_trend(close=df['Close'], volume=df['Volume'], fillna=True, dropnans=True)
            momentum_ta['volume_price_trend'] = volume_price_trend

            obv = ta.volume.on_balance_volume(close=df['Close'], volume=df['Volume'], fillna=True)
            momentum_ta['obv'] = obv
            ...
            boll_mavg = ta.volatility.bollinger_mavg(close=df['Close'], fillna=True)
            ...
            kelter_pband = ta.volatility.keltner_channel_pband(high=df['High'], low=df['Low'], close=df['Close'], fillna=True)
            kelter_wband = ta.volatility.keltner_channel_wband(high=df['High'], low=df['Low'], close=df['Close'], fillna=True)
            kelter_mband = ta.volatility.keltner_channel_mband(high=df['High'], low=df['Low'], close=df['Close'], fillna=True)

            # Add the indicators to the DataFrame
            momentum_ta['boll_hband'] = boll_hband
            momentum_ta['boll_lband'] = boll_lband
            momentum_ta['boll_mavg'] = boll_mavg
            momentum_ta['boll_wband'] = boll_wband
            momentum_ta['boll_pband'] = boll_pband

            momentum_ta['donchain_hband'] = donchain_hband
            momentum_ta['donchain_lband'] = donchain_lband
            momentum_ta['donchain_pband'] = donchain_pband
            momentum_ta['donchain_wband'] = donchain_wband
            momentum_ta['donchain_mband'] = donchain_mband

            momentum_ta['kelter_hband'] = kelter_hband
            momentum_ta['kelter_lband'] = kelter_lband
            momentum_ta['kelter_pband'] = kelter_pband
            momentum_ta['kelter_wband'] = kelter_wband
            momentum_ta['kelter_mband'] = kelter_mband

            # Add daily returns and MACD
            daily_log_return = ta.others.daily_log_return(close=df['Close'], fillna=True)
            daily_return = ta.others.daily_return(close=df['Close'], fillna=True)
            momentum_ta['daily_log_return'] = daily_log_return
            momentum_ta['daily_return'] = daily_return

            macd_signal = ta.trend.macd_signal(close=df['Close'], fillna=True)
            macd_diff = ta.trend.macd_diff(close=df['Close'], fillna=True)
            momentum_ta['macd_diff'] = macd_diff
            momentum_ta['macd_signal'] = macd_signal

            rsi = ta.momentum.rsi(close=df['Close'])
            stoch_rsi = ta.momentum.stochrsi(df['Close'], fillna=True)
            momentum_ta['stoch_rsi'] = stoch_rsi
            momentum_ta['rsi'] = rsi

            momentum_ta.to_csv('test.csv', index=0)
            first_row = momentum_ta.iloc[0]
            ...
            elif first_row['rsi'] > 70:
                bearish_signals.append("RSI Overbought (> 70)")

            # MACD check
            if first_row['macd_diff'] > first_row['macd_signal']:
                bullish_signals.append("MACD Bullish Crossover")
            elif first_row['macd_diff'] < first_row['macd_signal']:
                bearish_signals.append("MACD Bearish Crossover")

            # Stochastic RSI check
            if first_row['stoch_rsi'] < 20:
                bullish_signals.append("Stochastic RSI Oversold (< 20)")
            elif first_row['stoch_rsi'] > 80:
                bearish_signals.append("Stochastic RSI Overbought (> 80)")

            # Bollinger Bands check
            if first_row['Close'] > first_row['boll_hband']:
                bearish_signals.append("Close above Bollinger Upper Band (Overbought)")
            elif first_row['Close'] < first_row['boll_lband']:
                bullish_signals.append("Close below Bollinger Lower Band (Oversold)")

            # Donchian Channels check
            if first_row['Close'] > first_row['donchain_hband']:
                bearish_signals.append("Close above Donchian Upper Band")
            elif first_row['Close'] < first_row['donchain_lband']:
                bullish_signals.append("Close below Donchian Lower Band")

            # Keltner Channels check
            if first_row['Close'] > first_row['kelter_hband']:
                bearish_signals.append("Close above Keltner Upper Band")
            elif first_row['Close'] < first_row['kelter_lband']:
                bullish_signals.append("Close below Keltner Lower Band")
