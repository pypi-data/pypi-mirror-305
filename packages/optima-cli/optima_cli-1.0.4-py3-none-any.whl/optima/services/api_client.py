...
def get_resource(endpoint: str) -> Tuple[StatusAPI, requests.Response | None]:
    """
    Returns:
...
def fetch_all_problem_types() -> Dict[int, str] | StatusAPI:
    ...
    if status == StatusAPI.OK:
        prob_types = {int(prob_type["id"]): prob_type["type_name"] for prob_type in response.json().get("data")}
        return prob_types
    return status
