...
def _schema_init(dirname: str = '.mtax', filename: str = 'schema'):
    ...
    _schema.change('$id', 'montyalex.python-cli.settings-v0.1.1')
...
def inspect_initalize(uo, dirname, filename, use_json, use_toml, use_yaml, use_mpck):
    if not uo.exists:
...
def inspect_(
    ...
    inspect_initalize(uo, dirname, filename, use_json, use_toml, use_yaml, use_mpck)
    if full_inspection:
        inspection = True
