...
class POAM(IntegrationIssue):
    ...
    def __init__(self, file_path, module, module_id):
        super().__init__()
    ...
    @staticmethod
    def find_start_row(ws) -> Optional[int]:
        """
        Find the first row with 'V-' in column A.
        ...
        for row_index, row in enumerate(ws.iter_rows(min_row=1, max_col=1, values_only=True), 1):
            if row[0] and str(row[0]).startswith("V-"):
                return row_index
        return None
