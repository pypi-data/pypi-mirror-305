...
class TableTransformerExtractor(BaseTableExtractor):
    ...
    def _return_cell_results(
        ...
        for row in cell_coordinates_for_rows:
            ...
            for cell in row["cells"]:
                ...
                cell_images.append(cell_block_image)

            row_texts = self._ocr_detector.process(cell_images)
            if row["row_type"] == "table column header":
                results["header"].append(row_texts)
            else:
                results["rows"].append(row_text)
    ...
    def _return_row_results(
        ...
        row_images, row_header_images, column_images = [], [], []
        for block in table_blocks:
            if block.type in [
                "table row",
                "table column header",
                ...
                if block.type == "table column header":
                    row_header_images.append(row_image)
                elif block.type == "table row":
                    row_images.append(row_image)
                else:
                    column_images.append(row_image)

        row_texts = self._ocr_detector.process(row_images)
        column_texts = self._ocr_detector.process(column_images)
        header_texts = self._ocr_detector.process(row_header_images)

        table_dict["rows"] = row_texts
        table_dict["columns"] = column_texts
        table_dict["header"] = header_texts
