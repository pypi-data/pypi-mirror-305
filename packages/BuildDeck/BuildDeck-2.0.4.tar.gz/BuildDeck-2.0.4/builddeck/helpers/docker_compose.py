...
def generate_docker_compose(services, environment, additional_services=None):
    ...
    compose['services'] = {
        **compose['services'],  # Keep existing services
        **{service['name']: service for service in additional_services}  # Add or update with additional services
    }
    ...
    with open(compose_file, 'w') as file:
        yaml.dump(compose, file, default_flow_style=False, sort_keys=False, indent=4)
