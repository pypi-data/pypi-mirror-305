...
class SensiConfig:
    ...
    def _parse_stresses(self):
        """Parses the stresses for each sensitivity.

        Organizes stresses by sensitivity and groups commands by file to minimize I/O operations.
        """
        logger.info("Parsing stresses for all sensitivities.")
        self.parsed_stresses = {}

        for sensi_name in self.sensi_list:
            ...
            for stress_name in self.sensi_list[sensi_name]:
                ...
                for command in self.param_map[stress_name]:
                    ...
                    if syntax.expression.startswith("$"):
                        ...
                        try:
                            settings_path = os.path.join(self.sensi_dirs[sensi_name], "resources", "settings.json").replace("\\", "/")
                            settings_json = util.read_json_file(settings_path)
    ...
    def apply(self, sensi_dirs={}):
        ...
        self.sensi_dirs = self._get_sensi_dirs_to_process(sensi_dirs)
        ...
        for sensi_name, sensi_dirpath in self.sensi_dirs.items():
