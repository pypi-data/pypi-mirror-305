...
def merge_df_into(df, newdf, key_cols):
    ...
    # merge preserving order of rows in df
    ...
    merged_df = (
        merged_df.sort_values("temp_index")
        .drop("temp_index", axis=1)
        .reset_index(drop=True)
    )
    df.drop("temp_index", axis=1, inplace=True)
    # update columns in df
    cols_to_update = [col for col in newdf.columns if col not in key_cols]
    for col in cols_to_update:
        if col in df.columns:
            df.update({col: merged_df[col + "_update"]})
    # add any new columns to df
    new_cols = [
        col
        for col in newdf.columns
        if col not in df.columns and col not in key_cols
    ]
    for col in new_cols:
