...
class SwarmMatcherConfig(BaseModel):
    ...
    embedding_function: str = "sentence-transformers/all-mpnet-base-v2"  # Better model than MiniLM
...
class SwarmMatcher:
    ...
    def __init__(self, config: SwarmMatcherConfig):
        ...
        logger.info(f"Initialized SwarmMatcher with collection '{config.collection_name}'")
    ...
    def add_swarm_types(self, swarm_types: List[SwarmType]) -> None:
        ...
        try:
            self.collection.add(
                ...
                metadatas=[{"name": st.name, **st.metadata} for st in swarm_types]
            )
            logger.info(f"Added {len(swarm_types)} swarm types")
        except Exception as e:
            logger.error(f"Error adding swarm types in batch: {str(e)}")
            raise
    ...
    def find_best_matches(
        ...
        try:
            ...
            for metadata, distance in zip(results["metadatas"][0], results["distances"][0]):
                ...
                score = 1 - (distance / 2)  # Normalize cosine distance to [0,1]
    ...
    def auto_select_swarm(self, task: str) -> str:
        ...
        logger.info(f"Selected swarm type '{best_match}' with confidence {score:.3f}")
        return best_match
    ...
    def save_swarm_types(self, filename: str) -> None:
        ...
        try:
            ...
            with open(filename, "w") as f:
                json.dump([st.dict() for st in swarm_types], f, indent=2)
            logger.info(f"Saved swarm types to {filename}")
        except Exception as e:
            logger.error(f"Error saving swarm types: {str(e)}")
            raise
...
def initialize_default_swarm_types(matcher: SwarmMatcher) -> None:
    ...
    swarm_types = [
        SwarmType(
            ...
            """,
            metadata={"category": "optimization", "complexity": "high"}
        ),
        SwarmType(
            name="MixtureOfAgents",
            description="""
            ...
            """,
            metadata={"category": "collaboration", "complexity": "high"}
        ),
        SwarmType(
            name="SpreadSheetSwarm",
            description="""
            ...
            """,
            metadata={"category": "data_processing", "complexity": "medium"}
        ),
        SwarmType(
            name="SequentialWorkflow",
            description="""
