...
from typing import Any, Literal, Optional, Union, overload, TypeVar
...
def send(
    ...
    *,
    ...
    def on_send_done(f: Future):
        try:
            f.result()
        except Exception as e:
            if futrue is not None and not futrue.done():
                futrue.set_exception(e)
            if failed_to_broadcast and not msg.get(
                "init_data", None
            ):  # 不是初始化连接的消息，并且failed_to_broadcast为True，则重发
                msg["to_host"] = conn.host_id
                broadcast(msg)  # type: ignore
                logger.warning("消息发送失败，消息已广播")
            raise MessageSendError(f"消息发送失败: {e}") from e

    conn.send(msg).add_done_callback(on_send_done)  # add_done_callback 处理异常  待优化
    return futrue
...
class IPC_Handler:
    ...
    async def on_first_message(self, payload: UtType.FirstMessageTypedDict):
        ...
        if from_host.__len__() != 1:
            raise ConnectionIdentifyAuthFailed("身份验证失败, 首次消息中from_host长度大于1")
    ...
    def on_local_message(self, payload: UtType.MessageTypedDict):
        ...
        if from_host[0] == self.LOCAL_HOST_ID:
            if future_id:  # 本地完成future
                logger.debug(f"完成future: {future_id}, \n结果: {payload}")
    ...
    async def __call__(self):
        try:
            ...
            try:
                ...
                logger.info(f"移除连接: {self.conn.host_id}")
            except:
                pass
