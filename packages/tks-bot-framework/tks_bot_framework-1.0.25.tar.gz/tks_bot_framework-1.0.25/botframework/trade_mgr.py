...
class TradeMgr:
    ...
    async def get_active_positions(self, ksqldb_query_url, view_name, direction: Direction = None):
        # Extract the market (e.g., BTC or SOL) from view_name
        market_symbol = view_name.split('_')[-1].upper()
        
        algo_config = botframework_utils.get_app_config()
        position_capacities_by_asset = algo_config.get('position_capacities_by_asset', [])
        individual_positions = algo_config.get('individual_positions', [])
        
        # Consolidate all filtered positions
        filtered_positions = []
        
        # Process position_capacities_by_asset and filter by market and direction
        for asset in position_capacities_by_asset:
            asset_market_symbol = asset["market_symbol"].split('/')[0].upper()  # Extract BTC from BTC/USDT
            asset_direction = asset["common_specs"]["direction"]
            
            # Check if the asset matches the market symbol and direction
            if asset_market_symbol == market_symbol and (direction is None or asset_direction == direction.value):
                # Add IDs from 0 to count - 1 for each asset
                filtered_positions.extend(range(asset["count"]))
        
        # Process individual_positions and filter by market and direction
        for pos in individual_positions:
            pos_data = pos["position_capacity"]
            pos_market_symbol = pos_data["market_symbol"].split('/')[0].upper()
            
            # Check if the position matches the market symbol and direction
            if pos_market_symbol == market_symbol and (direction is None or pos_data["direction"] == direction.value):
                filtered_positions.append(pos_data["id"])
        
        occupied_positions = []
        if not filtered_positions:
            return occupied_positions  # Early return if no positions

        logger.info(f"Filtered positions: {filtered_positions}")

        # Check each filtered position in Kafka
        for pos_idx in filtered_positions:
            ...
            if results:
                ...
                status_of_position = latest_message[4]  # Assuming 'status_of_position' is at index 4
