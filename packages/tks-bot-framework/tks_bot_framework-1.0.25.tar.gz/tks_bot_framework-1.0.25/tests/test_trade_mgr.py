        ...
        "individual_positions": [
            {
                "position_capacity": {
                    "id": 0,
                    "direction": "short",
                    "position_size_in_percentage": "1/14",
                    "take_profit": 12,
                    "alternative_take_profit": None,
                    "stop_loss": 6,
                    "alternative_stop_loss": None,
                    "market": "BTC-USDT",
...
@pytest.mark.asyncio
@patch("botframework.botframework_utils.get_app_config")
@patch("botframework.botframework_utils.execute_pull_query")
async def test_get_active_positions_short(mock_execute_pull_query, mock_get_app_config, app_config):
    # Arrange
    ...
    mock_execute_pull_query.return_value = [
        [None, None, None, None, "open", None, None, None, None, None, None]  # Mocking status of position as open
    ]
    ...
    # Assert
    assert occupied_positions == [0]  # Only the BTC position should be marked as occupied
    mock_execute_pull_query.assert_called_once_with(
        ksqldb_query_url=ksqldb_query_url,
        view_name=view_name,
        select_columns="*",
        where_clause="POS_IDX = 0",
        offset_reset="earliest"
    )

@pytest.fixture
def app_config_with_assets():
    return {
        "position_capacities_by_asset": [
            {
                "market": "BTC-USDT",
                "market_symbol": "BTC/USDT",
                "count": 14,
            ...
            },
            {
                ...
                "common_specs": {
                    "direction": "long",
                    "position_size_in_percentage": "1/10",
                    "take_profit": 8,
                    "alternative_take_profit": 12,
                    "stop_loss": 5,
                    "alternative_stop_loss": 7
                }
            }
        ],
        "individual_positions": [
...
@patch("botframework.botframework_utils.get_app_config")
@patch("botframework.botframework_utils.execute_pull_query")
async def test_get_active_positions_with_individual_id(mock_execute_pull_query, mock_get_app_config, app_config_with_assets):
    # Arrange
    mock_get_app_config.return_value = app_config_with_assets
    trade_mgr = TradeMgr()
    ksqldb_query_url = "http://mock_ksqldb_url"
    view_name = "queryable_pull_tks_gatherer_trades_btc"  # Only BTC should be checked based on this view_name
    
    # Mock Kafka query to return an "open" status for pos_idx = 14
    mock_execute_pull_query.return_value = [
        [None, None, None, None, "open", None, None, None, None, None, None]  # Status is open
    ]

    # Act
    occupied_positions = await trade_mgr.get_active_positions(
        ksqldb_query_url=ksqldb_query_url,
        view_name=view_name,
        direction=Direction.SHORT
    )
    ...
    # Assert
    assert occupied_positions == [14]  # Only BTC's position 14 should be marked as occupied
    mock_execute_pull_query.assert_called_once_with(
        ksqldb_query_url=ksqldb_query_url,
        view_name=view_name,
        select_columns="*",
        where_clause="POS_IDX = 14",
        offset_reset="earliest"
    )


@pytest.fixture
def app_config_only_assets():
    return {
        "position_capacities_by_asset": [
            {
                "market": "BTC-USDT",
...
@patch("botframework.botframework_utils.get_app_config")
@patch("botframework.botframework_utils.execute_pull_query")
async def test_get_active_positions_only_assets(mock_execute_pull_query, mock_get_app_config, app_config_only_assets):
    # Arrange
    mock_get_app_config.return_value = app_config_only_assets
    trade_mgr = TradeMgr()
    ksqldb_query_url = "http://mock_ksqldb_url"
    view_name = "queryable_pull_tks_gatherer_trades_btc"  # Specifying BTC trades for filtering
    
    # Mock Kafka query to return an "open" status only for pos_idx = 5
    def mock_execute_pull_query_side_effect(ksqldb_query_url, view_name, select_columns, where_clause, offset_reset):
        if "POS_IDX = 5" in where_clause:
            return [[None, None, None, None, "open", None, None, None, None, None, None]]  # Status is open
        else:
            return []  # No result for other indices

    mock_execute_pull_query.side_effect = mock_execute_pull_query_side_effect
    ...
    # Assert
    assert occupied_positions == [5]  # Only position 5 should be marked as occupied
    # Ensure the query was called correctly
    mock_execute_pull_query.assert_any_call(
        ksqldb_query_url=ksqldb_query_url,
        view_name=view_name,
        select_columns="*",
        where_clause="POS_IDX = 5",
        offset_reset="earliest"
    )
