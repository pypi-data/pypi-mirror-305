...
from wktools_langflow.loan_toolkit import UpdateOutstandingRequest
...
class LoanWrapper(BaseModel):
    ...
    def get_user_loan_profile_by_loan_account_number(self):
        ...
        def update_outstanding(self, update_outstanding_request: UpdateOutstandingRequest):
            url = f"http://localhost:8000/api/loans/update-outstanding"
            headers = {
                "Accept": "application/json",
                "Content-Type": "application/json"
            }
            data = json.dumps(update_outstanding_request)
