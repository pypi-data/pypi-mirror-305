...
namespace RVO {
	...
	void RVO::Agent::prefVelocityCorrection()
	{
		float pref_v_abs = RVO::abs(prefVelocity_);
		float v_abs = RVO::abs(velocity_);
		if (pref_v_abs < 1e-4) {
			return;
		}
		Vector2 unit_prev = prefVelocity_ / pref_v_abs;
		Vector2 unit_v;
		if (v_abs < 1e-4) {
			unit_v = Vector2(0.0f, 0.0f);
		}
		else {
		...
		if (unit_prev * unit_v < 0.5f) {
			...
			if (v_abs < maxSpeed_ / 3) {
				prefVelocity_ = unit_v * pref_v_abs;
				//return;
			}
			// g = unit_v;
			return;
		...
		float score = 0.0f;
		for (auto nbrs : agentNeighbors_) {
			size_t id = nbrs.second->id_;
			Vector2 nbrs_x = sim_->agents_[id]->position_;
			Vector2 nbrs_v = sim_->agents_[id]->velocity_;

			float nbrs_v_abs = RVO::abs(nbrs_v);
			if (nbrs_v_abs < 0.1 * v_abs) {
				continue;
			}

			Vector2 dx = nbrs_x - position_;
			float v_x_inner = g * dx;
			if (v_x_inner <= 0) {
				continue;
			}

			float abs_dx = RVO::abs(dx);
			float v_v_inner = g * nbrs_v;
			float v_dx_cos = v_x_inner / abs_dx;
			float v_dx_sin = RVO::det(g, dx) / abs_dx;

			float sign_vx_sin = v_dx_sin < 0 ? -1 : 1;

			score += sign_vx_sin * v_dx_cos * (1.2f / abs_dx) * v_v_inner - 1e-4;
		}

		float sin_theta = 0.5 * tanh(0.5 * score) - 1e-4;
		float theta = asin(sin_theta);
		float cos_theta = cos(theta);

		float new_x = cos_theta * unit_prev.x() - sin_theta * unit_prev.y();
		float new_y = sin_theta * unit_prev.x() + cos_theta * unit_prev.y();

		prefVelocity_ = pref_v_abs * Vector2(new_x, new_y);
	}
