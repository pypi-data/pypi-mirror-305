...
if len(filtered_pkg_squared) > 0:
    for module in filtered_pkg_squared:
        ...
        if app_mode == 'plugins':
            if isfile(join(list_module_path, 'urls')):
                if module not in urlpatterns:
                    urlpatterns += [
            ...
            if module == settings.PROJECT_NAME:
                ...
                for url in find_urls:
                    if isfile(join(join(list_module_path, url), 'urls')):
                        if f'{settings.PROJECT_NAME}.{url}' not in urlpatterns:
                            urlpatterns += [
...
if len(filtered_current_path) > 0:
    for module_name in filtered_current_path:
        ...
        app_mode_config.read(join(settings.WORKING_SPACE, 'bomiotconf.ini'), encoding='utf-8')
        app_mode = app_mode_config.get('mode', 'name')
        if app_mode == 'plugins':
            if isfile(join(join(settings.WORKING_SPACE, module_name), 'urls')):
                if module_name not in urlpatterns:
                    urlpatterns += [
            ...
            if module_name == settings.PROJECT_NAME:
                ...
                for url in find_urls:
                    if isfile(join(join(project_path, url), 'urls')):
                        if f'{settings.PROJECT_NAME}.{url}' not in urlpatterns:
                            urlpatterns += [
                                path(f'{settings.PROJECT_NAME}/{url}/', include(f'{settings.PROJECT_NAME}.{url}.urls')),
                            ]
