...
@task_deco('fs_put', function_timeout=120, concurrent_mode=ConcurrentModeEnum.ASYNC, concurrent_num=4, broker_kind=BrokerEnum.NSQ, specify_async_loop=consumer_loop, max_retry_times=1)
...
@task_deco('fs_get_{}'.format(cfg['NODE_ID']), function_timeout=60, concurrent_mode=ConcurrentModeEnum.ASYNC, concurrent_num=4, broker_kind=BrokerEnum.NSQ, specify_async_loop=consumer_loop, max_retry_times=1)
...
async def extra_tasks():
    global exception_queue, last_exception_sync_time, last_process_ts, last_in_msg_ts
    while True:
        try:
            ...
            if time.time() - last_in_msg_ts > 120:
