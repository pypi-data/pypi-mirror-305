...
def mgk_interpret_training_mols(arguments=None):
    args = InterpretTrainingMolsArgs().parse_args(arguments)
    df_train = pd.read_csv(args.data_path)
    df_test = pd.read_csv(args.test_path)
    y_pred, y_std, df_interpret = interpret_training_mols(
        df_test[args.pure_columns[0]].tolist(),
        df_train[args.pure_columns[0]].tolist(),
        df_train[args.target_columns[0]].tolist(),
        alpha=args.alpha_,
        n_mol=args.n_mols,
        output_order='sort_by_value',
        mgk_hyperparameters_file=args.graph_hyperparameters[0],
        features_generator=args.features_generator,
        features_hyperparameters_file=args.features_hyperparameters_file,
        n_jobs=args.n_jobs)
