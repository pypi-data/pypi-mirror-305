...
class JenkinsEvaluateClient():
    def __init__(self, host, user, token) -> None:
        self.setup_db()
        self.server = MultiProcessJenkins(host, username=user, password=token)
    ...
    def setup_db(self):
        ...
        cur = con.cursor()
        cur.execute("CREATE TABLE jobs(_class, name, url, color, fullname)")
    ...
    def build_full_job_list(self):
        return list(self.multi.start_multi_process(self.build_job_data, self.jobs))
