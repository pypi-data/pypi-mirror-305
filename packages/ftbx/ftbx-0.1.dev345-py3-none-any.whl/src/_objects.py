...
class JobStatus(Enum):
    """
    Enum to represent the different status a Flex's job can be in.
    """
    CANCELLED          = "Cancelled"
    ...
    SUBMITTED          = "Submitted"
    TIMED_OUT          = "Timed out"
    WAITING_AUTO_RETRY = "Waiting for auto retry"
    ...
    def from_string(string: str):
        """
        Returns an JobStatus given a String.

        :param string: the string to convert to JobStatus
        """

        for status in JobStatus:
            if status.value == string:
                logger.debug(f"Match found for {string}: {status}")
                return status

        raise Exception(f"Cannot find JobStatus that matches '{string}'")
