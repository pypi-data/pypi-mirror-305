...
class SmallScaleOptimizer(BaseOptimizer):
    def __init__(self, objective_func, search_space, acq_func='EI', gp=None, n_steps=10, n_initial_points=1, X_init=None, Y_init=None):
        """
        ...
        X_init : list of points or None, optional (default=None)
            Initial points to start the optimization. If None, random points are generated.

        Y_init : array-like or None, optional (default=None)
            Initial objective values corresponding to X_init points. Must be provided if X_init is provided,
            and must have the same length as X_init.
        """
        ...
        if n_initial_points <= 0 and not X_init:
            raise ValueError("Either set `n_initial_points` > 0,"
                         " or provide `X_init`")
        
        if X_init is not None:
            if not isinstance(X_init[0], list):
                X_init = [X_init]
            if not isinstance(X_init, list):
                raise ValueError("`X_init` should be a list, but got %s" % type(X_init))
            
            if Y_init is not None:
                if not isinstance(Y_init, list):
                    raise ValueError("`Y_init` should be a list, but got %s" % type(X_init))
                if len(X_init) != len(Y_init):
                    raise ValueError("X_init and Y_init must have the same length")
        else:
            if Y_init is not None:
                raise ValueError("Y_init provided without corresponding X_init")
    ...
    def optimize(self):
        ...
        if self.X_init is not None:
            self.X_iters = np.array(self.X_init, dtype=object)
            if self.Y_init is not None:
                self.Y_iters = np.array(self.Y_init)
            else: 
