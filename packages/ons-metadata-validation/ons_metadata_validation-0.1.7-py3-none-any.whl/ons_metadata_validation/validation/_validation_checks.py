...
def must_not_say_office_of_national_statistics(item: str) -> bool:
    return not _regex_match("office of national statistics", item.lower())


def must_not_have_capitalised_for(item: str) -> bool:
    return not _regex_match("Office For National Statistics", item)
...
def _date_in_plausible_range(item: str, format: str) -> bool:
    ...
    if not all([isinstance(item, str), isinstance(format, str)]):
        raise TypeError(
            "must_be_in_long_date_format expects arg types: [str, str], "
            f"received: [{type(item).__name__}, {type(item).__name__}]"
        )
...
def must_be_in_short_date_format(item: str) -> bool:
    ...
    if not all([isinstance(item, str)]):
        raise TypeError(
            "expects arg types: [str], " f"received: [{type(item).__name__}]"
        )
