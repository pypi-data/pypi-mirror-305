...
class CryptoSeriesDataProvider:
    ...
    def _final_report(self) -> None:
        period_from = self.selected_days[0].strftime('%Y-%m-%d')
        period_to = self.selected_days[-1].strftime('%Y-%m-%d')
        ...
        for instrument in self.instruments:
            ...
            for day in self.selected_days:
                file_name = f"{instrument}_{day.strftime('%Y-%m-%d')}.parquet"
                file_path = os.path.join(self.raw_series_folder, file_name)

                if not os.path.exists(file_path):
                    missing_days.append(day.strftime('%Y-%m-%d'))

            missing_days_str = f"[{', '.join(missing_days)}]" if missing_days else "[]"
            report_lines.append(f"{instrument}, period:{period_from}:{period_to}, missed {missing_days_str}")
