...
from common.common import generate_date_range
...
class CryptoSeriesDataProvider:
    ...
    def _load_for_day(self, instrument: str, target_day: datetime) -> None:
        ...
        if os.path.exists(archive_path):
            print(f'{archive_name} already exist')
            return
        ...
        if ohlc_loader.is_exist() and trades_loader.is_exist():
            ...
            print(
                f'process-{os.getpid()}, {instrument}: saved {archive_name}, ohlc_series: {len(ohlc_series)}, trades: {len(trades)}, book_depths: {len(book_depths)}')
