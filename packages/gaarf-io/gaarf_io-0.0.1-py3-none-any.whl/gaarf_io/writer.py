...
from gaarf_io.writers import abs_writer
...
def create_writer(
  ...
  if writer_option in ('bq', 'bigquery'):
    writer_module = import_module('gaarf_io.writers.bigquery_writer')
    return writer_module.BigQueryWriter(**kwargs)
  if writer_option == 'sqldb':
    writer_module = import_module('gaarf_io.writers.sqldb_writer')
    return writer_module.SqlAlchemyWriter(**kwargs)
  if writer_option in ('sheet', 'sheets'):
    writer_module = import_module('gaarf_io.writers.sheets_writer')
    return writer_module.SheetWriter(**kwargs)
  if writer_option == 'console':
    writer_module = import_module('gaarf_io.writers.console_writer')
    return writer_module.ConsoleWriter(**kwargs)
  if writer_option == 'csv':
    writer_module = import_module('gaarf_io.writers.csv_writer')
    return writer_module.CsvWriter(**kwargs)
  if writer_option == 'json':
    writer_module = import_module('gaarf_io.writers.json_writer')
    return writer_module.JsonWriter(**kwargs)
  return import_module('gaarf_io.writers.null_writer').NullWriter(writer_option)
