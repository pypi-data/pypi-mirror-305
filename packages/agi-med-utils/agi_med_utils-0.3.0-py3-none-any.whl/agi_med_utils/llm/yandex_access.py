...
class YandexGPTEntryPoint:
    ...
    def get_response(self, total_input: str) -> str:
        ...
        try:
            response = requests.post(
                url=self.text_url, headers=self.headers, json=data
            ).json()
    ...
    def get_embeddings(self, total_input, input_is_long=False) -> list:
        ...
        try:
            response = requests.post(
                url=self.emb_url, headers=self.headers, json=data
            ).json()
    ...
    def warmup(self) -> None:
        ...
        assert (
            self.get_embeddings("Прогрев", input_is_long=True) != self.ZEROS
        ), "Нет доступа к ллм!"
