...
longest_name = max([len(store.store_type) for store in SecretStoreBase.__subclasses__()])

for store in SecretStoreBase.__subclasses__():
    print(f"{store.store_type.ljust(longest_name)} - Required env vars: {store.required_config}")
