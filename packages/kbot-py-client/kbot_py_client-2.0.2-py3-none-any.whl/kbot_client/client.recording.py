...
class Client:
    ...
    def record_start(self, file_descriptor):
        self._recording_fd = file_descriptor
        self._recording_fd.write("import requests\n\n")
    ...
    def impersonate(self, username, usertype='local', external_auth='', application_uuid='', timeout=5):
    ...
    def __request(self, method: str, uri : str = None, data: dict = None, params: dict = None, files: dict = None, attempt=0):
        ...
        r = requests.request(method.upper(), self.url + '/api/%s/' % (uri), params=params, data=dump_data, headers=headers, verify=self.verify)

        # The recording if configured to do so
        if self._recording_fd:
            self._recording_fd.write("\n##  {uri}".format(uri='/api/%s/' % (uri)))
            self._recording_fd.write("\nheaders = {headers}".format(headers=json.dumps(headers)))
            self._recording_fd.write("\nparams = {params}".format(params=json.dumps(params)))
            self._recording_fd.write("\ndata = {data}".format(data=json.dumps(data)))
            self._recording_fd.write("\nr = requests.request({method}, {url}, params=params, data=data)".format(
                method=method.upper(), url=self.url + '/api/%s/' % (uri)))
            self._recording_fd.write("\nr.raise_for_status()")
    ...
    def request(self, method: str, uri: str, data: dict = None, params: dict = None):
        return self.__request(method, uri=uri, data=data, params=params)
