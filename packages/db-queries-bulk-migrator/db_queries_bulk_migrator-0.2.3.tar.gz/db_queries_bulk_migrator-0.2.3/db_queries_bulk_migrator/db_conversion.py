...
def activation(endpoint_name: str, props: dict, credential_vault_id: str = None):
...
class EF2SqlExtension:
    def __init__(
        self, dt: Dynatrace, endpoint_name: str, ef1_config_properties: Union[Dict, List], credential_vault_id = None, pre_cron = False
    ) -> None:
        ...
        if type(ef1_config_properties) == list:
            ...
            self.activation_config = activation(endpoint_name, ef1_config_properties[0], credential_vault_id)
        else:
            db_type = ef2_datasource(ef1_config_properties["database_type"])
            queries = queries_from_ef1_config(ef1_config_properties)
            self.activation_config = activation(endpoint_name, ef1_config_properties, credential_vault_id)
        ...
        for query in queries:
            ...
            if not pre_cron:
                subgroup.update({"schedule": query.schedule})
