...
class TaskService:
    ...
    def task_run(data):
        ...
        match code_type: 
            case '1':#上传文件方式
                ...
                process = subprocess.Popen(["bash", f"{project_app_root}/scripts/run_task.sh", project_url, script_run_url_new, project_file_uri, task_id, code_type, version_type, version_name]) 
            case '2':#git方式
                # 构建项目URL
                project_url = data['code_url']
    ...
    def task_restart(data):
        ...
        # 使用subprocess.Popen 让命令在后台执行，避免阻塞主线程
        # 使用subprocess.Popen执行shell脚本，传入项目URL和脚本运行的URL作为参数
        process = subprocess.Popen(["bash", f"{project_app_root}/scripts/rerun_task.sh", script_run_url_new, task_id, load_run, checkpoint]) 
