...
from dapu.perks import lookup_by_first_return_second, sum_column
...
class DapuManager(DapuProcess):
    ...
    def clear_fantoms(self):
        """
        If machine dies without any warning then job status remains -1 (in progress)
        ...
        Default is '2 days' (during this task is considered running and no new runs for it)
        """
        agenda = self.context.find_registry_table_full_name('agenda')
    ...
    def clear_manager_fantoms(self):
        """
        In case managar got killed then jobs status remains in preparation (21) and it prevents new jobs too
    ...
    def add_jobs_to_agenda(self, commander: str='Manager') -> int:
        ...
        # kattuvaid päevaaegasid ei tohi olla!! NO overlapings in day divisions (cte_paevaaeg)
        ...
        for rec_new_task in result_set_tasks_to_add:  # add to Agenda one-by-one
            (task_id, *_) = rec_new_task # unpack esimene (hetkel ongi üks ainult)
            new_count += self.add_task_to_agenda(task_id, commander, False, 0, JobStatus.IDLE.value)
        #
        return new_count


    @DapuProcess.task_id_eventlog(flag='AGENDA')
    def add_task_to_agenda(self, task_id: str, commander:str, run_all_checks:bool = True, task_priority_shift: int=0, task_status:int = JobStatus.IDLE.value) -> int:
        """
        Return 1 if succeeded, 0 if not
        run_all_check is not needed, et our big SQL detect only possible tasks, but later if some other way appers...
        Wish to add tasks which are not in Registry gives 0 new rows
        """
        registry = self.context.find_registry_table_full_name('registry')
        agenda = self.context.find_registry_table_full_name('agenda')
    ...
    @DapuProcess.task_id_eventlog(flag='DEPENDANT')
    def add_dependent_tasks_to_agenda(self, task_id: str, task_priority: int):
        """
    ...
    def count_jobs_in_agenda(self) -> tuple[int, int, int]:
        ...
        busy_count = lookup_by_first_return_second(result_set, JobStatus.BUSY.value)
        idle_count = lookup_by_first_return_second(result_set, JobStatus.IDLE.value)
        #total_count = busy_count + idle_count
        total_count = sum_column(result_set, 1)
        return (total_count, busy_count, idle_count)
    
    def get_oldest_busy_start_time(self) -> str:
        agenda = self.context.find_registry_table_full_name('agenda')
