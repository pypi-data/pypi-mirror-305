...
def content_to_file(content : str, dir: str, name: str) -> bool:
    full_name: str = os.path.join(dir, name)
...
def version_string_from_timeint(time_int) -> str:
    ...
    ts = datetime.fromtimestamp(time_int)
    seconds_passed = (ts.hour * 60 + ts.minute) * 60 + ts.second
    day_number = ts.strftime("%j") # 001
    year_number = ts.year - 2000
    ver = f"{year_number}.{day_number:>3}.{seconds_passed:>5}"
    return ver
...
def isodate_from_version_string(ver_str) -> str:
    ...
    parts = ver_str.split('.')
    years = int(parts[0])
    ts = datetime(2000 + years, 1, 1) - timedelta(days=1)
    
    days = int(parts[1])
    secs = int(parts[2])
    ts2 = ts + timedelta(days=days, seconds=secs)
...
def lookup_by_first_return_second(result_set: list[tuple], search: int, search_col: int=0, return_col: int=1) -> int:
...
def sum_column(result_set: list[tuple], num_column: int) -> int:
    total = 0
    for row in result_set:
        total += row[num_column]
    return total
...
def is_file_inside_git_area(file_or_dir: str | None) -> bool | None:
    """
    Nice way to to it is via 
    pip install gitpython
    git_repo = git.Repo(path, search_parent_directories=True)
    git_root = git_repo.git.rev_parse("--show-toplevel")
      = repo.working_tree_dir
      = repo.working_dir
    ...But what happenes if not git is not installed? some try/except structure?
    Altenative way here is just look for folder (or file) named '.git' somewhere up-path
...
def fileobject_parent(file_or_dir):
    path_object = pathlib.Path(file_or_dir)
    if path_object.parent == path_object.parents[-1]: 
        # if parent is the last item in list of parents (maybe it is true as well: parents list lenght is 1)
...
def make_replacements(original: str | None, replacements: list[tuple[str, str]] | None = None) -> str | None:
    """
    One way of two ways to replace markers inside unsafe strings (other way is Jinja, f-strings are not 'couse of use of variables)
    """
    if original is None:
        return None
    ...
    def inner(func: Callable[..., list[tuple]]) -> Callable:
        def wrapper(*args: Any, **kwargs: Any) -> int:
            ...
            for changed_row in result_set:
                ...
                sql_reg_log = f"""INSERT INTO {worker_log} (task_id, flag) 
                    VALUES ('{changed_row_task_id}', '{flag}')"""
