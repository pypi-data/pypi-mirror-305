...
from dapu.perks import get_dirs_with_definition, load_task_id_list_from_file, convert_to_bool, is_file_inside_git_area
...
class DapuRegistrar(DapuProcess):
    ...
    def check_deletions_and_delete(self):
        ...
        deletion_marker_file: str = self.context.full_name_from_pull(self.context.FILENAME_FOR_DELETION)
        ...
        if task_list_to_delete: # kustutame enne, et ei tekiks halbu anomaaliaid (hea anomaalia on failide allesjäämine, halb on registry ebakõla) 
            ...
            for task_id in task_list_to_delete:
                ...
                #logging.info(f"Task {task_id} unregistered")
    ...
    def task_max_ver_file(self, task_id: str) -> int:
        ...
        dir_path = os.path.join(task_dir, self.context.SUBDIRNAME_VERSION_FILES) # subdir ver, with files 001.sql, 002.sql etc
        max_file_nr = 0
    ...
    def task_table_upgrade(self, task_id: str, start_table_version: int) -> bool:
        ...
        task_dir = self.find_task_dir_path(task_id) 
        replacements = [(self.context.PLACEHOLDER_TARGET_SCHEMA, f'{schema_name}'), (self.context.PLACEHOLDER_TARGET_TABLE, f'{table_name}')]
        
        dir_path = os.path.join(task_dir, self.context.SUBDIRNAME_VERSION_FILES) # subdir ver, with files 001.sql, 002.sql
        file_list = []
        ...
        for sub_name in file_list:
            ...
            sql_file_name = os.path.join(task_dir, self.context.SUBDIRNAME_VERSION_FILES, sub_name)
