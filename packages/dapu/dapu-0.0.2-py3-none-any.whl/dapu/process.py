...
#import dbpoint.datacontroller.datacontroller_exceptions as unified
...
class DapuProcess:
    ...
    context: DapuContext = None # class variable, mostly for our decorator

    def __init__(self, args: list | None):
        #self.version(False, False) # just print out version (uncomment it later)
        ...
        if args is None:
            ...
            #logging.debug(f"from zero work_dir is {DapuProcess.context.work_dir}")
            return
        ...
        if isinstance(args, list):
            # argument is not DapuContext, so context must be generated, assumably first time
            self.context = DapuContext()
            work_dir = real_path_from_list(args) # first argument is working directory which have files (connections, haulings)
            if work_dir is None:
            ...
            self.context.log_level = 10 if 'debug' in self.context.flags else 20 # here we cannot use standard imported logging module!
            reconf_logging(self.context.log_level) # after that line real use of logging can appear
            self.context.list_of_profiles = self.load_project_connections() # this one uses self.context
    ...
    def check_stopper(self) -> bool:
        ...
        sql = f"""SELECT allowed_version FROM {stopper} ORDER BY id DESC LIMIT 1"""
        try:
    ...
    def load_project_connections(self) -> list[dict] | None:
        ...
        Uses self.context.SQL_CONN_FILE_NAME for yaml-file name
        and validates existence of needed profile name (self.context.TARGET_ALIAS)
        """

        conn_file_full = self.context.full_name_from_root(self.context.SQL_CONN_FILE_NAME)
