    ...
    def _summary_table(self, y: int, metric: GAMetrics, scan: AltrisScan) -> int:
        ...
        for key in SUMMARY_TABLE_KEYS:
            ...
            if (
                ...
                self._add_cell(
                    GA_DICT_KEYS_TO_LABELS[key],
                    str(round(float(metric[key]), 3)) + GA_METRICS_UNITS[key],  # type: ignore[literal-required] # Reason: key is explicitly iterating over SUMMARY_TABLE_KEYS # noqa: E501
                    x=x,
                    y=y,
                    horizontal_distance=table_horizontal_distance,
                    fixed_spacing=fixed_spacing,
                )
            if index % 2 == 1 and index != len(metric) - 1:
                y += table_vertical_distance
            index += 1
        y += table_vertical_distance
        return y
