...
class _WorkerSide(BaseWorkerAlgorithm):
    ...
    def map_types(self, schema: BitfountSchema) -> None:
        ...
        for column in self.column_ranges:
            ...
            for stype in SemanticType:
                for feature in schema.table.get_feature_names(stype):
                    if feature == column:
    ...
    def map_types_multi(self, schema: BitfountSchema) -> list[str]:
        ...
        for table_name, columns in self.column_ranges.items():
            for column in columns:
                ...
                if table_name == schema.table.name:
                    # Determine what type we have in the schema for the column
                    for stype in SemanticType:
                        for feature in schema.table.get_feature_names(stype):
                            if feature == column:
        ...
        for stype in SemanticType:
            for feature in schema.table.get_feature_names(stype):
                ...
                if feature not in already_added:
                    mapped_type = schema.table.features[
                    ...
                    if schema.table.name not in self.column_ranges:
                        self.column_ranges[schema.table.name] = {}
                    self.column_ranges[schema.table.name][feature] = {
                        "type": mapped_type
                    }
                    ...
                    if default_max is not None:
                        ...
                        self.column_ranges[schema.table.name][feature].update(
                            {"lower": 0}
                        )
                        self.column_ranges[schema.table.name][feature].update(
                            {"upper": default_max}
    ...
    def run(self, **kwargs: Any) -> pd.DataFrame:
        ...
        if isinstance(self.datasource, FileSystemIterableSource):
            ...
            if schema.table.name == self.table:
