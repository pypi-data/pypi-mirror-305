...
class _BitfountSchemaMarshmallowMixIn:
    ...
    class _Schema(_CamelCaseSchema):
        table = fields.Nested(TableSchema._Schema)
        ...
        @pre_load
        def convert_v1_to_v2(self, data: _JSONDict, **kwargs: Any) -> _JSONDict:
            """Convert schema version 1 to version 2.

            This method is a pre-load hook that checks the schema version
            in the provided data. If the schema version is "1", it converts it
            to version "2" by sorting the tables by the attribute `name`, grabbing
            the first one, updating the schema version, and moving the table data
            ...
            if schema_version == "1":
                ...
                if tables:
                    ...
                    logger.debug("Schema v1 detected, converting to v2...")
                    data["metadata"]["schema_version"] = "2"
                    data["table"] = tables[0]  # Grab the first table after sorting
                    del data["tables"]
            return data

        @post_dump
        def sort_tables(self, data: _JSONDict, **kwargs: Any) -> _JSONDict:
            """Add metadata as a key after table at the bottom of the YAML file."""
            new_data = {}
        ...
        def recreate_schema(self, data: _JSONDict, **_kwargs: Any) -> BitfountSchema:
            """Recreates Schema."""
            table = data["table"]
            new_schema = BitfountSchema(table=table)
...
class BitfountSchema(_BitfountSchemaMarshmallowMixIn):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self._table: Optional[TableSchema] = None
        ...
        if table is not None:
            ...
            self._add_datasource_table(datasource, **kwargs)
        else:
            raise BitfountSchemaError(
                "A schema must be initialised with either a datasource or a table."
            )
    ...
    def table(self) -> TableSchema:
        ...
        Raises:
            BitfountSchemaError: If the table is None.
        """
        if self._table is None:
            raise BitfountSchemaError("The table is not set in the schema.")
        return self._table

    @table.setter
    def table(self, value: Optional[TableSchema]) -> None:
        self._table = value
    ...
    def _add_datasource_table(
        ...
        """Adds the table from a `BaseSource` to the schema.
        ...
        self.table = TableSchema(
            ...
            else table_descriptions.get(table_name, None),
        )
        self.table.add_datasource_features(
            datasource=datasource,
            ignore_cols=None if not ignore_cols else ignore_cols.get(table_name, None),
            force_stype=None
            if not force_stypes
            else force_stypes.get(table_name, None),
            descriptions=None
            if not column_descriptions
            else column_descriptions.get(table_name, None),
        )
    ...
    def get_feature_names(
        ...
        Returns:
            features: A list of feature names.
        """
        return self.table.get_feature_names(semantic_type)
    ...
    def get_categorical_feature_size(
        ...
        Returns:
            The number of unique value in the categorical column.
        """
        return self.table.get_categorical_feature_size(var)
    ...
    def get_categorical_feature_sizes(
        ...
        Args:
            ...
            ignore_cols: The column(s) to be ignored from the schema.
        """
        return self.table.get_categorical_feature_sizes(ignore_cols)
    ...
    def apply(
        ...
        """
        return self.table.apply(dataframe, keep_cols)
