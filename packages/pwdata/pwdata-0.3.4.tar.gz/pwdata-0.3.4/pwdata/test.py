...
def test_scale_cell():
    ...
    for config in configs:
        ...
        if "atom_types" in config.keys():
            ...
            atom_types = None
        for format in ["pwmat/config","vasp/poscar","lammps/lmp"]:
            save_path =  os.path.join(save_dir, "scale_{}".format(FORMAT.get_filename_by_format(format)))
            cmd_list = ["", "scale_cell", "-r", "0.98", "0.99", "0.97", "0.95",  "-i", config_file, "-f", config_format, "-s", save_path, "-o", format, "-c"]
            if atom_types is not None:
                cmd_list.append("-t")
                cmd_list.extend(atom_types)
            main(cmd_list)
...
def test_super_cell():
    ...
    for config in configs:
        ...
        if "atom_types" in config.keys():
            ...
            atom_types = None
        for format in ["pwmat/config","vasp/poscar","lammps/lmp"]:
            save_path =  os.path.join(save_dir, "super_{}".format(FORMAT.get_filename_by_format(format)))
            cmd_list = ["", "super_cell", "-m", "2", "3", "4",  "-i", config_file, "-f", config_format, "-s", save_path, "-o", format, "-c"]
            if atom_types is not None:
                cmd_list.append("-t")
                cmd_list.extend(atom_types)
            main(cmd_list)
...
def test_pertub():
    ...
    for config in configs:
        ...
        if "atom_types" in config.keys():
            ...
            atom_types = None
        for format in ["pwmat/config","vasp/poscar","lammps/lmp"]:
            save_path =  os.path.join(save_dir, "perturb_{}".format(FORMAT.get_filename_by_format(format).split('.')[0]))
            cmd_list = ["", "perturb", "-e", "0.01", "-d", "0.04", "-n", "20", "-i", config_file, "-f", config_format, "-s", save_path, "-o", format, "-c"]
            if atom_types is not None:
                cmd_list.append("-t")
                cmd_list.extend(atom_types)
            main(cmd_list)
...
def test_convert_configs():
    ...
    for idi, config in enumerate(configs):
        ...
        for idj, format in enumerate([FORMAT.pwmlff_npy, FORMAT.extxyz]):
            ...
            cmd_list.extend([ "-f", config_format, "-s", save_path, "-o", format, "-p", "1.0", "-r", "-g", "1", "-m", '1'])
            if atom_types is not None:
                cmd_list.append("-t")
                cmd_list.extend(atom_types)
            main(cmd_list)
...
def test_meata_data():
    ...
    for idj, format in enumerate([FORMAT.extxyz]):
        ...
        cmd_list.extend([ "-f", "meta", "-s", save_path, "-o", format, "-p", "0.8", "-r"])#, "-c", "10" , "-q", "HfGeH"
        if atom_types is not None:
            cmd_list.append("-t")
            cmd_list.extend(atom_types)
        main(cmd_list)
