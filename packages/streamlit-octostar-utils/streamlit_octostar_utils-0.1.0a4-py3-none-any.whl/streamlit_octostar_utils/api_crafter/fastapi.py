...
class Route(object):
    def route(route, **route_kwargs):
        def decorator(func):
            ...
            wrapper._route_kwargs = {**route_kwargs, '_funcname': func_name}
            if getattr(route, func_name, None):
                raise ValueError("Cannot have two FastAPI routes with the same name within the same Route!")
            setattr(route, func_name, wrapper)
    ...
    def include_routes(self, router_mapping=None):
        for func in self.routed_funcs:
            router = self.router
            if 'router' in func._route_kwargs:
                router = func._route_kwargs['router']
            else:
                router = router_mapping.get(func._route_kwargs['_funcname'], self.router)
