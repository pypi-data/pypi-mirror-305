...
class NifiContextManager(object):
    ...
    def __enter__(self):
        self.ontology = fetch_ontology_data.sync(ontology_name=self.ontology_name, client=self.client)
        return self
...
class NifiRoute(Route):
    def __init__(self, app, tasks_routes, router=None):
        super.__init__(self, app, router)
        self.tasks_routes = tasks_routes
    
    def define_routes(self):
        @Route.route.get("/nifi/task-state/{task_id}")
        async def get_task_status(
            task_id: str
        ) -> str:
            task_status = await self.tasks_routes.get(task_id)
            task_status = task_status.model_dump(mode='json')['data']['task_state']
            return task_status

        @Route.route.get("/nifi/task-result/{task_id}")
        async def get_task_result(
            task_id: str
        ) -> dict:
            return_data = await self.tasks_routes.get(task_id)
            return_data = return_data.model_dump(mode='json')['data']['data'] 
            return return_data
        
class NifiRoute(Route):
    def __init__(self, app, tasks_routes, processor_name, router=None):
    ...
    def define_routes(self):
        ...
        async def get_task_status(
            ...
            task_status = await self.tasks_routes.get_task(task_id)
            task_status = task_status.model_dump(mode='json')['data']['task_state']
            return task_status
        ...
        async def get_task_result(
            ...
            return_data = await self.tasks_routes.get_task(task_id)
            return JSONResponse(return_data)
        
        @Route.route(self, path="/{op}")
        async def send_task(
            op: str,
            request: Request
        ) -> str:
            ...
            task = self.endpoints[op].apply_async(args=[body, processor_name])
            return task.id
    
    def nifi_task(func):
        @wraps(func)
        async def wrapper(body, processor_name, *args, **kwargs):
            with NifiContextManager(body) as nifi_context:
                entity_batches = nifi_context.receive_input(body, processor_name)
                entity_batches = await func(nifi_context, entity_batches, processor_name, *args, **kwargs)
                return nifi_context.send_output(entity_batches, processor_name)
        return wrapper
    
class NifiResultPickler(object):
    def __init__(self, nifi_context, entity_batches, processor_name):
        self.nifi_context = nifi_context
        self.entity_batches = entity_batches
        self.processor_name = processor_name