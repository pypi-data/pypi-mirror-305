        ...
        for df in self.node_batches:
            ...
            for _, row in df.iterrows():
                ...
                if external_id not in existing_nodes:
                    self.hygraph.add_pgnode(oid=external_id, label=row['label'], start_time=start_time, end_time=end_time,properties=row.drop(['id', 'start_time', 'end_time']).to_dict())
                else:
            ...
            for _, row in df.iterrows():
                ...
                if source_node and target_node:
                    ...
                    if not edge_exists:
                        ...
                        self.hygraph.add_pgedge(oid=external_edge_id, source=source_node.oid, target=target_node.oid, label=row['label'], start_time=start_time, end_time=end_time,properties=row.drop(['source_id', 'target_id', 'start_time', 'end_time']).to_dict())
    ...
    def process_subgraph_batches(self):
        for df in self.subgraph_batches:
            for _, row in df.iterrows():
                ...
                self.hygraph.add_subgraph(subgraph_id=subgraph_id, label=label, start_time=start_time, end_time=end_time,properties=properties)
    """def process_subgraph_batches(self):
