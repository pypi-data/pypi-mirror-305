...
@option(
    ...
    char="i",
    required=True,
    description="Inputfiles (tsv, csv); wildcard pattern is possible for file series.",
)
@option(
    "headerrow",
    char="r",
    ...
    description="Specify that the files have no header. Per default they do have a"
    " header.",
)
@option("outputfile", char="o", required=True, description="Name of the output file.")
@option(
    "row_col",
    char="d",
    required=True,
    description="Concatenate row-wise (same number of columns) or column-wise (same"
    " number of rows).",
...
def files_combine(files, headerrow, outputfile, row_col, separator):
    """Combines files row-wise or column-wise."""
    files = files.split(",")

    for file_in in files:
        if "*" not in file_in and len(files) <= 1:
            click.echo("At least two files or a file series is expected")
            sys.exit(1)

    if outputfile is None:
        click.echo("Outputfile has to be specified")
        sys.exit(1)
    ...
    for df_n in list_of_df:
        if row_col == "rows" and list_of_df[0].shape[1] != df_n.shape[1]:
            click.echo(
                "For rows to be combined properly, header names and number of columns"
                " in the files need to equal."
            )
            sys.exit(1)
        elif row_col == "columns" and list_of_df[0].shape[0] != df_n.shape[0]:
            click.echo(
                "For columns to be combined properly, number of rows in the files needs"
                " to be equal."
            )
            sys.exit(1)
