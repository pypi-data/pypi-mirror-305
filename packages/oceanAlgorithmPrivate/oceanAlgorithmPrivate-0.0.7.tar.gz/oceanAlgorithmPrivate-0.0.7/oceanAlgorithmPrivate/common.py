...
class Private(LoggerHelper):
        ...
        for param_name, details in selected_value.items():
            ...
            for col_name in columns_to_check:
                ...
                if col_name not in dtype_dict:
                    print(f"DataFrame에 '{col_name}' 컬럼이 존재하지 않습니다.")
                    param_valid = False
                    continue
                
                # 컬럼의 데이터 타입 확인
                if dtype_dict[col_name] not in allowed_dtypes:
                    print(f"'{display_name}'의 컬럼 '{col_name}'의 데이터 유형 '{dtype_dict[col_name]}'이 허용된 유형({allowed_dtypes})과 일치하지 않습니다.")
                    param_valid = False
                ...
                if unique_value_constraints:
                    ...
                    if constraint:
                        ...
                        if not (min_val <= unique_count <= max_val):
                            print(f"'{display_name}'의 '{col_name}'의 고유값 개수 '{unique_count}'가 '{pred_type}'에 대한 허용된 범위({min_val}-{max_val})를 벗어납니다.")
                            param_valid = False
    ...
    def validate_unique_value(self, df, json_data):
        selected_value = {item['parameter_name']: item['selected_value']
                          for item in json_data['parameters'] if item.get('area') == 'var_select'}
        
        # validate target variable
        if selected_value['pred_type'] == "clssf_binary":
            unique_values = df[selected_value['dep_var']].unique()
            if len(unique_values) == 2:
                print(f"오류: {selected_value['dep_var']}가 2개의 값을 가지지 않습니다.")
                return False
            
        elif selected_value['pred_type'] == "clssf_multi":
            unique_values = df[selected_value['dep_var']].unique()
            if len(unique_values) > 2:
                print(f"오류: {selected_value['dep_var']}가 3개 이상의 값을 가져야 합니다.")
                return False
        
        elif selected_value['pred_type'] == "regression":
            unique_values = df[selected_value['dep_var']].unique()
            if len(unique_values) > 5:
                print((f"오류: {selected_value['dep_var']}가 범주형이 아닌 연속형 자료여야 합니다."))
                return False
        return True
    ...
    def validate_selected_value(self, json_data):
        for data in json_data['parameters']:
            ...
            if value_type == "float":
                ...
                print(f"오류: '{ui_display_name}'의 value_type '{value_type}'는 지원되지 않는 타입입니다. 올바른 유형인지 확인해 주세요.")
                return False
            ...
            if value_range:
                ...
                if isinstance(selected_value, list):
                    if not all(min_value <= item <= max_value for item in selected_value):
                        print(f"오류: '{ui_display_name}'의 한 개 이상의 요소가 범위 [{min_value}, {max_value}]를 벗어납니다.")
                        return False
                else:
