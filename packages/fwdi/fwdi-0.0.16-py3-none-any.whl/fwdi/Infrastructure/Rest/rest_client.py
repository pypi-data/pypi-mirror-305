...
class RestClientFWDI(BaseRestClientFWDI):
    ...
    def get(self, path:str, _data:BaseModel=None, _params=None)->any:
        if len(self.__token) > 0:
            if _params != None > 0:
                response_get = requests.get(f"{self.__base_url}{path}", data=_data.model_dump_json(), params=_params, headers=self.__headers)               
            else:
            ...
            if response_get.status_code == 200:
                ...
                if 'detail' in error_json:
                    if error_json['detail'] == 'Could not validate credentials':
                        if self.login():
                            return self.get(path, _data, _params)
    ...
    def post(self, path:str, _data:BaseModel=None, _params=None)->any:
        if len(self.__token) > 0:
            if _params != None > 0:
                response_post = requests.post(f"{self.__base_url}{path}", data=_data.model_dump_json(), params=_params, headers=self.__headers)               
            else:
