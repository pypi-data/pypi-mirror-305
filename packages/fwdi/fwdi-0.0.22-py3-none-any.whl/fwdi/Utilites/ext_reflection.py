...
class ExtReflection():
    ...
    def is_generic_of(t: object, origin: object) -> bool:
        return ExtReflection.is_generic(t) and t.__origin__ == origin  # type: ignore
            ...
            if  len([item for item in info_method_args['params'] if item['name'] == 'self']) == 0:
                if len(args) > 1:
                    if type(args[0]) is not info_method_args['class']:
                        ...
                        find_args = [item for item in info_method_args['params'] if item['name'] != 'self'][2:]
            else:
                if len(kwargs) == 0:
                    find_args = [item for item in info_method_args['params'] if item['name'] != 'self'][1:]
    ...
    def method_inject(func: _C) -> _C:
        ...
        def internal_sync(*args, **kwargs)->_C:
            if 'is_inject' not in kwargs:
                ...
                if len(kwargs) > 0 and len(kwargs) == len(info_method_args['params']):
