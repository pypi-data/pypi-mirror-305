...
from examples.Infrastructure.rag_llm_service import BaseLlmService, LLMService
from Utilites.tools import PostProccessingText, TextTools_v1


class TaskManager(BaseTaskManager):

    def __init__(self, tools: TextTools_v1, post_procceing_text: PostProccessingText, llm_service: BaseLlmService) -> None:
        super().__init__()
        ...
        self.lst_id_discussion:list = []
        self.main_thread: threading.Thread = None
        self.tools: TextTools_v1 = tools
    ...
    def add_task_for_update(self, task: UpdateRequestToLLM) -> None:
        ...
        if self.main_thread is None:
                self.main_thread = threading.Thread(target=self.execute, daemon=True)
                self.main_thread.start()
            ...
            if not self.lst_requests.empty():
                ...
                if task.context != '':
                    ...
                    if response is not None and code == 200:
                        ...
                        TaskManager.lst_id_discussion.remove(task.request.id_discussion)
                        TaskManager.lst_answer.put(answer_from_LLM)
                else:
                    ...
                    response: ResponseRagInferenceViewModel = LLMService.request_to_llm("alitrix",
                                                                                "123123",
                ...
                response: ResponseRagInferenceViewModel = LLMService.request_v2(task.username,
                                                                                task.password,
                                                                                task.project,
                                                                                task.custom_prompt)
