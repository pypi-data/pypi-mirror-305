...
class LLMService(BaseLlmService):
    ...
    def request_v2(self,
        ...
        #rest_client = UploadRestClient(f"http://localhost:5000/api/v1.0/{project}")      # сервер сервер
        # rest_client = UploadRestClient(f"http://cd-host-va.codev.dom:5100/api/v1.0/{project}")   # локально сервер
        # rest_client = UploadRestClient(f"http://172.21.94.31:5000/api/v1.0/{project}")    # локально локально
        try:
            if self.__rest_client.login():
                #logger.info('Auth to Rest service is OK !')
                
                result = self.__rest_client.get('/rag_inference', view_model)
                return result
            else:
                return None
