...
class ManagerDbContext(BaseManagerContextDB):
    ...
    def __init__(self, lsi_lda_db: BaseLsiLda, search_lsi_lda: BaseSearchLsiLda) -> None:
        super().__init__()
        full_db_path = 'examples/Persistance/Databases'
        self.vectore_store = faiss.read_index(f"{full_db_path}/vectorstore/index.faiss") if ExtPath.exists_file(f"{full_db_path}/vectorstore/index.faiss") else None
        self.pickle_store = pd.read_pickle(f"{full_db_path}/picklestore/ParseData.pkl") if ExtPath.exists_file(f"{full_db_path}/picklestore/ParseData.pkl") else None
