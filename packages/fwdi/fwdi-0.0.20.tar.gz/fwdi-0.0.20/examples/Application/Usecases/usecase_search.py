...
class ServiceSearch(BaseServiceSearch):
    def relevant_search(self, 
        ...
        embedding_question = embeding.encoder.encode(question)
        ...
        res = self.__vector_search(index, _vector, question_pack, distance, manager_db)
        if res is None:
    ...
    def __vector_search(self, index: faiss.IndexIVFFlat, 
        ...
        context_LLM = DataForLlmModel(question_pack, context_for_llm_str, lst_link_distance)
    ...
    def __check_quantity_words(self,
        ...
        if max_words >= 500 and method == 'VECTOR':
            lst_answer = self.__search_result_lsi(question_pack)
            union_answer_list = self.__unity_answer(lst_answer, lst_answer, question_pack)
                
            lst_context, lst_link_distance = self.check_quantity_words(union_answer_list, question_pack, method='UNION')
