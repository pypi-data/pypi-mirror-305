...
class UsecaseContextDataBase(BaseUsecaseContextDataBase):
    ...
    def __parse_stand(web_parse_service: BaseProxyWebParse, data_store: BaseSimilarityDataStore) -> bool:
        try:
    ...
    def __preprocessing_text(stage_data: BaseStageDataProcessing, data_store: BaseSimilarityDataStore) -> list[dict]:
        lst_doc = data_store.read('Persistance/Databases/picklestore/ParseData.pkl')
        lst_doc = stage_data.preprocessing_text(lst_doc[:3:])

        return lst_doc

    def __parse_bsp(web_parse_service_bsp: BaseProxyWebParse) -> list[dict]:
        web_parse_service_bsp: BaseWebParse = web_parse_service_bsp.get_parse_bsp()
        lst_doc = web_parse_service_bsp.parse()

        return lst_doc 
    
    def __merge_parse_data(lst_docs: list[list], data_store: BaseSimilarityDataStore) -> list[dict]:
    ...
    def __prepared_sentences(lst_result: list[dict], list_field: list[str], stage_data: BaseStageDataProcessing, data_store: BaseSimilarityDataStore) -> list[dict]:
    ...
    def __create_new_vectore_store(lst_result: list[dict], context_field: str, vectore_path: str, vectore_service: BaseVectoreService) -> bool:    # context_field = 'context'
        try:
    ...
    def create_vectore_database(config: ServiceConfig):
        if not UsecaseContextDataBase.__parse_stand():
            return False
        lst_doc = UsecaseContextDataBase.__preprocessing_text()
        lst_doc_bsp = UsecaseContextDataBase.__parse_bsp()
        total_lst_doc = []
        total_lst_doc.append(lst_doc)
        total_lst_doc.append(lst_doc_bsp)
        lst_doc = UsecaseContextDataBase.__merge_parse_data(total_lst_doc)
        lst_doc = UsecaseContextDataBase.__prepared_sentences(lst_doc, ['article', 'context'])
        UsecaseContextDataBase.__create_new_vectore_store(lst_doc, 'context', 'Persistance/Databases/vectorstore/temp/index.faiss')
