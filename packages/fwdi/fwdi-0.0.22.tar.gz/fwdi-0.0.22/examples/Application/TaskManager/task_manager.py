...
from Utilites.tools import PostProccessingText, TextTools


class TaskManager(BaseTaskManager):

    def __init__(self, tools: TextTools, post_procceing_text: PostProccessingText, llm_service: BaseLlmService) -> None:
        super().__init__()
    ...
    def __execute(self):
        while not self.lst_requests.empty() or not self.lst_update_db.empty():
            if not self.lst_requests.empty():
                ...
                response: ResponseRagInferenceViewModel = LLMService.request_v2(task.custom_prompt)
