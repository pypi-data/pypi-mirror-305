...
class WebParseBSP(BaseWebParse):
    ...
    def __iter_for_tags(lst_tags_document: list, name_document: str, flag = False) -> str:
        context = ''
        for tag in lst_tags_document:
            if tag != '\n' and tag.name != 'script' and tag.name != 'hr':
                if tag.name == 'div':
                    ...
                    context = WebParseBSP.__iter_for_tags(lst_tag_stage_2, name_document)
                    if context != '':
                        return context
    ...
    def __create_child(item_menu, parent: BaseManagerContextMenu, context='') -> BaseManagerContextMenu:
    ...
    def __iteration_items_menu(items_menu: list, parent: BaseManagerContextMenu, cookies, web_parse_tools: BaseWebParseTools):
        for item in items_menu:
            item_menu = WebParseBSP.__create_child(item, parent)
            if item.get('class') == ['collapsed']:
                items_layer = item.find('ul', recursive=False).find_all('li', recursive=False)
                WebParseBSP.__iteration_items_menu(items_layer, item_menu, cookies)
            else:
                ...
                if tags is not None:
                    ...
                    name_doc = item.text
                    context = WebParseBSP.__iter_for_tags(lst_tags, name_doc)
                    if context != '':
                        WebParseBSP.__create_child(item, parent, context=context)
                else:
    ...
    def parse(web_parse_tools: BaseWebParseTools, config: WebParseBSPConfig, text_tools: BaseTextTools) -> list:
        try:
            driver = WebParseBSP.__create_driver()
            driver.get(
                "https://its.1c.ru/user/auth?backurl=%2F")
            ...
            WebParseBSP.__iteration_items_menu(items_menu=items, parent=menu, cookies=cookies)
