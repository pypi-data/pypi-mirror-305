...
from Application.Usecases.usecase_task_manager import answer
...
class StageDataProcessing(BaseStageDataProcessing):
    ...
    def preprocessing_text(lst_doc: list[dict]) -> list[dict]:
                ...
                if text != None:
                    if check_answer_summarize_pref in text:
                        ...
                        if text_index != -1:
                            ...
                            final_dict = ExtDictionary.merge(item, {'llm_about_text': text, 'llm_about_text_cleared': clear_text[6::]})
    ...
    def __request_about_text_v1(text: str, task_manager: BaseUsecaseTaskManager) -> str:
        default_prompt = """<s>[INST]Ты - эксперт-аналитик. Тщательно проанализируй прилагаемый документ и составь краткое содержание. Для ответа используй не более 10 предложений. Ответ должен быть напиисан на русском языке, кратким,  лаконичным и понятным.[/INST]"""
        lst_arg:list[dict] = []
        lst_arg.append({
            'type':'system',
            "arg_prompt": "\"[INST]### Context:\n{context}[/INST]</s>[INST] ### Ответ:", 
                "arg_name":"context",
                "arg_value": text
            })
        
        req_custom_prompt = CustomPromptViewModel(type=TypeInference.RAG, username='alitrix', max_token=1000, prompt=default_prompt, lst_args=lst_arg, type_prompt='default')
        event: Event = Event()
    ...
    def __request_questions_text_v1(text: str, task_manager: BaseUsecaseTaskManager) -> str:
        default_prompt = """<s>[INST]Ты - эксперт-аналитик. Тщательно проанализируй контекст не упуская ни каких деталей. Я хочу получить не более 30 возможных вопросов по контексту ниже. Убедись, что в вопросах отражены основные положения, ключевые аргументы. Вопросы должны быть только на русском языке.[/INST]"""
        lst_arg:list[dict] = []
        lst_arg.append({
            'type':'system',
            "arg_prompt": "\"[INST]### Context:\n{context}[/INST]</s>[INST] ### Ответ:", 
        ...
        req_custom_prompt = CustomPromptViewModel(type=TypeInference.RAG, username='alitrix', max_token=1000, prompt=default_prompt, lst_args=lst_arg, type_prompt='default')
        event:Event = Event()
        add_request_update_LLM = UpdateRequestToLLM(custom_prompt=req_custom_prompt, source=event)
        task_manager.add_task_for_update(add_request_update_LLM)
        event.wait()
        response = answer.get()
