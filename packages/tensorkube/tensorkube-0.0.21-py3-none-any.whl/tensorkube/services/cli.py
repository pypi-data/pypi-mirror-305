...
from tensorkube.helpers import create_mountpoint_driver_role_with_policy, sanitise_assumed_role_arn, \
    track_event
...
@click.option('--gpu-type', type=click.Choice(['V100', 'A10G', 'T4', 'L4', 'L40S'], case_sensitive=False), help='Type of GPU.')
...
def create(secret_name, key_value_pairs, env, force):
    ...
    click.echo("Creating secret " + click.style(secret_name, bold=True)
               + " in environment " + click.style(env, bold=True) + " ...")
...
def delete(secret_name, env):
    ...
    if click.confirm(click.style(msg, fg=CliColors.WARNING.value), abort=True):
        click.echo("Deleting secret " + click.style(secret_name, bold=True)
                   + " from environment " + click.style(env, bold=True) + " ...")
...
@dev.command()
@click.option('--gpu-type', type=click.Choice(['V100', 'A10G', 'T4', 'L4', 'A100', 'L40S', 'H100', 'None'], case_sensitive=False), help='Type of GPU.')
...
@click.option('--secret', default=DEFAULT_SECRET, type=str, multiple=True, help='Secret to use for the deployment.')
def train(gpus, gpu_type, env, job_id, axolotl, config_path, secret):
    if axolotl:
