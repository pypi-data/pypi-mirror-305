from tensorkube.services.deploy import build_app
import uuid
import os
...
from tensorkube.services.k8s_service import get_efs_claim_name, get_tensorkube_cluster_context_name, create_configmap, create_aws_secret
from tensorkube.services.knative_service import apply_anti_affinity_for_gpus, get_instance_family_from_gpu_type
...
def apply_training_job(job_name: str, namespace: str, yaml_file_path: str,
                                      gpus: int, gpu_type: str,
                                      context_name: Optional[str] = None,
                                      secrets: List[str] = [], image_url: Optional[str] = None, bucket: Optional[str]=None):
    # Load kube config
    if not context_name:
    ...
    if bucket:
        yaml_dict['spec']['template']['spec']['containers'][0]['env'].append({
            'name': 'LORA_ADAPTER_BUCKET',
            'value': bucket #TODO: should we take region as well?
        })
    yaml_dict['spec']['template']['spec']['containers'][0]['env'].append({
        'name': 'JOB_NAME',
        'value': job_name
...
def axolotl_train(env: str, secrets: list, gpus: int, gpu_type: str, job_id:str, config_path:str):
    ...
    job_name = f"{job_name}-gpus-{gpus}-{str(gpu_type).lower()}"
    try:
    ...
    try:
        ...
        apply_training_job(job_name, namespace, yaml_file_path, gpus, gpu_type,secrets=secrets,image_url=AXOLOTL_IMAGE, bucket=bucket_name)
    except Exception as e:
