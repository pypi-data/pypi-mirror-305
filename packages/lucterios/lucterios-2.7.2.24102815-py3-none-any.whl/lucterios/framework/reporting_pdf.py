...
from reportlab.platypus.tables import TableStyle, CellStyle
...
class ConvertHTML_XMLReportlab(object):
    ...
    def _parse_span(self, text):
        ...
        if 'font-size' in self.current_style:
            ...
            for font_attr in ('color', 'background-color'):
                if font_attr in self.current_style:
                    xmlrl_item.attrib[font_attr] = self.current_style[font_attr]
        return xmlrl_item
...
class LucteriosPDF(LucteriosBase):
    ...
    def create_para(self, xmltext, current_w, current_h, offset_font_size=0, no_para=False):
        ...
        # getLogger('lucterios.core.print').debug("%s:%s", xmltext.tag, para_text)
        texts = []
    ...
    def add_page(self):
        if not self.is_changing_page:
            ...
            try:
                ...
                self.draw_header()
                self.draw_footer()
                # getLogger('lucterios.core.print').debug("before page %f - %f => %f", self.position_y, self.y_offset, self.header_h + self.t_margin)
