...
class LucteriosGlobal(LucteriosManage):
    ...
    def installed(self):
        ...
        if ('lucterios' in package_list.keys()) and (len(package_list['lucterios']) > 2) and (len(package_list['lucterios'][2]) > 0):
    ...
    def get_pip_outdated(self):
        ...
        outdated_list = check_output("%s -m pip list %s --outdated --format json" % (sys.executable, options), shell=True, env=env_variables)
