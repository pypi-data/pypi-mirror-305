...
def evaluate_backtest(df_og):
    ...
    win_rate = count_profit_trades / df.shape[0]
...
def get_ohlc(symbol, timeframe, start_dt, end_dt):
    res = requests.get('http://80.190.82.41:80/ohlc-data',
                       params={
                           'symbol': symbol,
                           'timeframe': timeframe,
                           'start-dt': start_dt,
