...
class IconRpcHelper:
    def __init__(self, url="", wallet=None, network_info: NetworkInfo = None, raise_on_failure=True, debug=False,
                 required_sign_methods=None, wait_sleep=1, tx_method="icx_getTransactionResult", logger=None, margin_steps=0, verbose=0, **kwargs):
        self.wallet = wallet
    ...
    def get_stake(self, address=None, return_as_hex=False, return_key="result.stake"):
        if not address:
            address = self.wallet.get('address')
    ...
    def get_bond(self, address=None, return_as_hex=False, return_key="result.bonds")-> list:
        if not address:
            address = self.wallet.get('address')
    ...
    def get_delegation(self, address=None, return_as_hex=False, return_key="result.delegations"):
        if not address:
            address = self.wallet.get('address')
    ...
    def get_iscore(self, address=None, return_as_hex=False, return_key="result.estimatedICX"):
        if not address:
            address = self.wallet.get('address')
    ...
    def preview_transaction_fee(self, payload, margin_steps=0, return_in_loop=False, use_decimal=False):
        """
    ...
    def stake_all_icx(self, fee=None, step_limit=None, min_balance=0, margin_steps=1000):
        """
        ...
        """
        # Get the current balance and staked amount
        balance_loop = int(self.get_balance(return_as_hex=True), 16)  # Balance in LOOP (smallest unit)
        balance_icx = balance_loop / 1e18  # Convert to ICX using float

        current_staked_hex = self.get_stake(return_as_hex=True)
        current_staked_loop = int(current_staked_hex, 16)  # Staked amount in LOOP
        current_staked_icx = current_staked_loop / 1e18  # Convert to ICX using float

        pawn.console.log(f"[INFO] Current Balance: {balance_icx} ICX")
        pawn.console.log(f"[INFO] Current Staked Amount: {current_staked_icx} ICX")

        # If the current balance is equal to the current staked amount, no need to perform staking
        if balance_icx == current_staked_icx:
        ...
        # If a custom fee is provided, use that. Otherwise, use the estimated fee.
        transaction_fee_icx = fee if fee is not None else estimated_fee_icx

        # Calculate the staking amount, excluding the fee
        staking_amount_icx = (balance_icx - transaction_fee_icx - min_balance) + current_staked_icx

        # If the staking amount is less than or equal to zero, raise an error
        if staking_amount_icx <= 0:
        ...
        payload['params']['data']['params']['value'] = hex(int(staking_amount_icx * 1e18))  # Convert ICX to LOOP

        # Sign and send the staking transaction
        try:
            self.sign_tx(payload=payload, step_limit=step_limit)
            result = self.sign_send()
            pawn.console.log(f"[SUCCESS] Staked {staking_amount_icx} ICX successfully. TX Hash: {result.get('result')}")
        except Exception as e:
            pawn.console.log(f"[ERROR] Staking transaction failed: {e}")
            raise

        # Check the updated stake after the transaction
        updated_staked_loop = int(self.get_stake(return_as_hex=True), 16)
        updated_staked_icx = updated_staked_loop / 1e18

        now_balance = self.get_balance()
        # pawn.console.log(f"[INFO] Updated Staked Amount: {updated_staked_icx} ICX, now_balance={now_balance}")
        # Calculate the increase in staked amount
        staked_difference = updated_staked_icx - current_staked_icx

        # Display the staking results with emojis
        pawn.console.log(f"[INFO] Updated Staked Amount: {updated_staked_icx} ICX, now_balance={now_balance}")
        pawn.console.log(f"📈 [INFO] Staked increase: {staked_difference} ICX")


        if staked_difference > 0:
            pawn.console.log(f"🔼 Increased staking by: {staked_difference:.18f} ICX 🎉")
        elif staked_difference < 0:
            pawn.console.log(f"🔽 Decreased staking by: {abs(staked_difference):.18f} ICX ⚠️")
        else:
...
class AsyncGoloopWebsocket(AsyncCallWebsocket):
    ...
    async def run_from_blockheight(self, blockheight=None, api_path: str = "/api/v3/icon_dex/block"):
        ...
        latest_blockheight = await self.api_client.get_last_blockheight()
