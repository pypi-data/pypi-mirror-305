...
class FireBaseLoader(Loader):
    ...
    def iterate_documents(self):
        ...
        while True:
            docs = []  
            if cursor:
                docs = [snapshot for snapshot in self.collection.limit(self.batch_size).order_by(self.order_by).start_after(cursor).stream()]
            else:
                docs = [snapshot for snapshot in self.collection.limit(self.batch_size).order_by(self.order_by).stream()]
            for doc in docs:
                # return doc to iterate  
                yield doc                           
            if len(docs) == self.batch_size:
                cursor = docs[self.batch_size-1]
                continue
            break
...
class LoaderFactory:
    ...
    def create_loader(source_type, 
        ...
        if source_type.value == SourceType.SQLSERVER.value:
            ...
            return FireBaseLoader(source_path, loader_type, collection_name, order_by, batch_size, init_load, end_load)
        else:
