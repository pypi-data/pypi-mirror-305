...
class SilverTable(ProcessLayerTable):
    ...
    def create_table(self, silver_table_name=None):     
        """
        ...
        if silver_table_name is None:
            silver_table_name = self.table_name
        self.create_tbl = CreateTableSilver(self.target_df.schema, silver_table_name, self.yml).execute()           
        self.create_vw = CreateView(self.layer, silver_table_name, self.create_tbl.yml, self.create_tbl.tbl_comment).execute()        
        return self    
