...
def list_models(args: Namespace) -> None:
    ...
    if args.json:
        render.print_json(format_json(response, registries_api))
    elif args.yaml:
        print(render.format_object_as_yaml(format_json(response, registries_api)))
    else:
        format_models(response, args, registries_api)


def format_json(
    response: List[PackagedModel], registries_api: aiolirest.RegistriesApi
) -> List[Dict[str, str]]:
    models = []
    for m in response:
