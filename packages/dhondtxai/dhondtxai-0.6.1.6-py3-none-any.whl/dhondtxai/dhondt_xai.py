...
class DhondtXAI:
    ...
    # Step 1: Assign votes based on feature importance, accounting for rounding issues
    votes = []
    total_importance = sum(self.feature_importances)
    for feature, importance in zip(self.features, self.feature_importances):
        # Calculate the vote share based on feature importance, ensure proper rounding
        vote_share = (importance / total_importance) * num_votes
        vote_count = max(0, int(round(vote_share)))  # Allow 0 votes if importance is extremely low
        votes.append(vote_count)

    # Step 2: Apply exclusion of features if specified
    if exclude_features:
        votes = [vote if feature not in exclude_features else 0 for feature, vote in zip(self.features, votes)]

    # Step 3: Handle alliances (grouping of features)
    if alliances:
        # Group votes based on alliances, merging votes where features are allied
        alliance_dict = {feature: i for i, features in enumerate(alliances) for feature in features}
        grouped_votes = [0] * len(alliances)
        for feature, vote in zip(self.features, votes):
            if feature in alliance_dict:
                grouped_votes[alliance_dict[feature]] += vote
            else:
                grouped_votes.append(vote)
        votes = grouped_votes

    # Step 4: Allocate seats using the D'Hondt method, only after votes have been assigned independently
    adjusted_votes = votes.copy()  # Keep votes unchanged for allocation purposes
    if threshold:
        # Apply threshold for seat allocation, features/alliances below the threshold are excluded from seat distribution
        total_votes = sum(adjusted_votes)
        adjusted_votes = [
            vote if (vote / total_votes * 100) >= threshold else 0 for vote in adjusted_votes
        ]

    return self.features, adjusted_votes
(self, num_votes, num_mps, threshold=None, alliances=None, exclude_features=None):
