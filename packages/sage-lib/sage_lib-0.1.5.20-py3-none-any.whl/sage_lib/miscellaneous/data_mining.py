...
class LinearAnalysis:
    ...
    def bootstrap_analysis(self, linear_predict, compositions, energies, n_iterations=1000):
        ...
        for _ in tqdm(range(n_iterations), desc="Processing bootstrap_analysis"):
            ...
            coeffs, _, _ = linear_predict(X_boot, y_boot, regularization=1e-5, verbose=False, zero_intercept=True, force_negative=False)
    ...
    def residuals_analysis(self, linear_predict, X, y):
        ...
        if self.result.get('residuals', None) is None:
            _, predictions, residuals = linear_predict(X, y, regularization=1e-5, verbose=False, zero_intercept=True, force_negative=False)
        else:
    ...
    def evaluate_dynamic_permutation_impact(self, X, y, linear_predict, tolerance=10, min_sampling=5):
        ...
        """
        def create_equally_spaced_points(start, end, num_points):
            """
            Creates a list of equally spaced points from start to end inclusive.
            If num_points is greater than the range, just use the entire range.
            """
            if num_points > end - start + 1:
                return list(range(start, end + 1))
            else:
                return np.linspace(start, end, num=num_points, dtype=int).tolist()

        def adjust_permutation_vector(max_permutations, max_points=500, initial_fixed_points=20):
            """
            Adjusts the n_permutations_list to include a maximum of `max_points` elements.
            The first `initial_fixed_points` are taken directly, and the rest are equally spaced up to `max_permutations`.

            Parameters:
            - max_permutations (int): The maximum number of permutations to consider.
            - max_points (int): The maximum number of points in the returned list.
            - initial_fixed_points (int): The number of initial points to take directly from the start.

            Returns:
            - list: Adjusted list of permutation counts.
            """
            if max_points <= initial_fixed_points:
                # If the total number of points allowed is less than or equal to initial points, just return the initial segment
                return list(range(initial_fixed_points))

            # First take the initial fixed points directly
            n_permutations_list = list(range(initial_fixed_points))

            # Calculate the remaining points to be distributed
            remaining_points = max_points - initial_fixed_points

            # Create equally spaced points from the end of initial points to the max_permutations
            if max_permutations > initial_fixed_points:
                spaced_points = create_equally_spaced_points(initial_fixed_points, max_permutations, remaining_points)
                n_permutations_list.extend(spaced_points)

            return n_permutations_list

        rows, cols = X.shape
        max_permutations = int((rows * cols)/2)
        n_permutations_list = adjust_permutation_vector( max_permutations )  # Range from 0 to the total number of elements in X
        
        mean_errors = []
        previous_error = float('inf')

        # Iterate over different numbers of permutations
        for n_permutations in tqdm(n_permutations_list, desc="Evaluating permutations"):
