...
class XYZ(FileManager):
    ...
    def export_as_xyz(self, file_location:str=None, save_to_file:str='w', verbose:bool=False,
        ...
        properties_list = [
            ...
            f'Properties={":".join(filter(None, [
                    f"species:S:1" if include_species else "",                 
                    f"{position_tag}:R:3" if include_position else "",                 
                    f"{forces_tag}:R:3" if include_forces else "",                 
                    f"{charge_tag}:R:1" if include_charge and hasattr(self.charge, "shape") and len(self.charge.shape) > 1 else "",                 
                    f"{magnetization_tag}:R:1" if include_magnetization and hasattr(self.magnetization, "shape") and len(self.magnetization.shape) > 1 else "",                
                    f"{fixed_tag}:I:3" if include_fixed else "",                f"{classID_tag}:I:1" if include_classID else "",                 
                ]))}',
        ...
        '''
        # Preparing atom data lines, fill missing values with -1
        atom_lines = []
        for i in range(self.atomCount):
            atom_label = self.atomLabelsList[i] if include_species and i < len(self.atomLabelsList) else 'X'
            position_data = ' '.join(map('{:13.10f}'.format, self.atomPositions[i])) if include_position and i < len(self.atomPositions) else '-1.0 -1.0 -1.0'
            forces_data = ' '.join(map('{:14.10f}'.format, self.total_force[i])) if include_forces and i < len(self.total_force) else '-1.0 -1.0 -1.0'
            charge_data = ' '.join(map('{:14.10f}'.format, self.charge[i] if np.ndim(self.charge) == 1 else [self.charge[i, -1]])) if include_charge and i < len(self.charge) else '-1.0'
            magnetization_data = ' '.join(map('{:14.10f}'.format, self.magnetization[i] if np.ndim(self.magnetization) == 1 else [self.magnetization[i, -1]])) if include_magnetization and i < len(self.magnetization) else '-1.0'
            fixed_data = ' '.join(map('{:2d}'.format, np.array(self.atomicConstraints[i], dtype=np.int32))) if include_fixed and i < len(self.atomicConstraints) else '-1 -1 -1'
            class_id_data = f"{self.class_ID[i]}" if include_classID and i < len(self.class_ID) else '-1'

            atom_lines.append(f"{atom_label} {position_data} {forces_data} {charge_data} {magnetization_data} {fixed_data} {class_id_data}")
        '''
