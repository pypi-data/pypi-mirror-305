...
class CtRedis:
    ...
    Attributes:
        cache_enabled (bool): A flag indicating if caching is enabled in the service level.
    ...
    """
    def __init__(self, is_cache_enabled, logger, redis_pool=None, expire_time=60):
        self.logger = logger
        self.cache_enabled = is_cache_enabled
        self.redis_pool = redis_pool
        self.expire_time = expire_time

    def redis_cache(self, template_cache_key: str, field: str, expire_time: Optional[int] = None,
        ...
        Args:
            ...
            is_global (Optional[bool]): Flag indicating if the cache key is global for services.
            ...
            def wrapper(*args, **kwargs):
                ...
                try:
                    if 'redis_conn' not in g:
                        redis_conn = redis.Redis(connection_pool=self.redis_pool, retry_on_timeout=True,
    ...
    def __generate_cache_key_and_hash_field(self, template_cache_key: str, func: Callable, template_hash_field: str,
        ...
        cache_key = f"{env}{cache_key}" if is_global else f"{env}{self.redis_pool.connection_kwargs.get('client_name')}:{cache_key}"
