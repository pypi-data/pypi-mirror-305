...
from shared_kernel.messaging.event_executor import AWSReceiveEventMessage
...
class AWSDataBus(DataBus):
    ...
    def subscribe_async_event(self, event_name: str):
        """
        Subscribe to an event by creating a queue for the event specific queue and
        EventBridge rule.
    ...
    def delete_message(self, message: AWSReceiveEventMessage):
        """
        Delete a message from the SQS queue.

        Args:
            message (AWSReceiveEventMessage): AWSReceiveEventMessage object which contains event_name and
