...
def create_api_key(description: str, expires_at: Optional[datetime] = None) -> CreateApiKeyResponse:
    ...
    response_dict = config.api_client.request(
        ...
        json=CreateApiKeyRequest(description=description, expires_at=expires_at).model_dump(mode="json"),
    )
