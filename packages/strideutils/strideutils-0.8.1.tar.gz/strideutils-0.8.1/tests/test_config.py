...
class TestParseConfig:
    ...
    def test_parse_valid_config(self) -> None:
        ...
        assert set(config.host_zones.keys()) == set(self.host_chain_names)
        assert set(config.app_chains.keys()) == set(self.app_chain_names)
        ...
        assert len(list(config.host_zones.values())) == len(self.host_chain_names)
        assert all(isinstance(zone, HostChainConfig) for zone in config.host_zones.values())
    ...
    def test_parse_config_extra_host_chain_field(self) -> None:
        ...
        host_chain = config.host_zones["cosmoshub"]
    ...
    def test_parse_config_extra_app_chain_field(self):
        ...
        config = _parse_raw_configs(raw_config, self.raw_secrets)
        app_chain = config.app_chains["neutron"]
