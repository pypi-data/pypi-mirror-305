...
def get_redis_password_env_var(db_name: str) -> str:
    ...
    return f"REDIS_STRIDE_{db_name.upper()}_PASSWORD"
...
class Config(ConfigObj):
    ...
    host_zones: Dict[str, HostChainConfig] = field(default_factory=dict)
    app_chains: Dict[str, ChainConfig] = field(default_factory=dict)

    @property
    def stakeibc_host_zones(self) -> List[HostChainConfig]:
        return [host_zone for host_zone in self.host_zones.values() if host_zone.ica_enabled]
    ...
    def _create_mappings(self) -> None:
        ...
        for chain_info in itertools.chain(self.host_zones.values(), self.app_chains.values()):
            try:
    ...
    def __iter__(self):
        """Allow iterating over host zones"""
        return iter(self.host_zones.values())
