...
class SqliteQueue:
    ...
    def put_mul(self, data_list):
        self.cursor.executemany(f"INSERT INTO {self.queue_name} (data) VALUES (?)", [(data,) for data in data_list])
        self.conn.commit()
        return 'ok'
    ...
    def run(self):
        ...
        while self.switch:
            ...
            if self._clear:
                ...
                self._clear = False
                continue
            while self.get_queue.qsize() and sql_queue.qsize():
                self.get_queue.get()
            ...
            while self.put_queue.qsize():
                ...
                if self.put_queue.qsize() > 100:
                    sql_queue.put_mul([self.put_queue.get() for _ in range(100)])
