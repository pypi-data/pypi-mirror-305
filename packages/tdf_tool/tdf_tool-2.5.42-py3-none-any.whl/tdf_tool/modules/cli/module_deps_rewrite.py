...
class ModuleDependenciesRewriteUtil:
    ...
    def _writeNewDeps(self, modulePath: str, deps: list[dict], isShell: bool = True):
        ...
        if isYamlOverrideSame:
            Print.str("yaml重写节点无变化")
        else:
            Print.str("yaml重写节点有变化")
        
        if isLockAndYamlOverrideSame:
            Print.str("lock和package_config 与yaml重写节点一致")
        else:
            Print.str("lock和package_config 与yaml重写节点不一致")
