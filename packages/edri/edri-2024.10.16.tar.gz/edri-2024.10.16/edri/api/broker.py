...
from typing import Optional, cast, Any, DefaultDict, Type
...
from edri.dataclass.event import Event, EventHandlingType
...
class Broker(ManagerBase):
    ...
    def solve(self, event: Event) -> None:
        ...
        if (event_info := self.events.get(event.__class__)) is None:
            ...
            if handling_type & EventHandlingType.SPECIFIC and event._key:
                self.send_specific(event)
        ...
        if isinstance(event, group.Client):
            ...
            for middleware in self.middlewares_request:
                ...
                if not isinstance(event, Event):
                    ...
                    event.response.set_status(ResponseStatus.FAILED)
                    event.response.add_directive(
                        InternalServerErrorResponseDirective(f"Wrong type returned by {middleware.__class__.__name__}"))
                    self.resolve(event)
                    return
