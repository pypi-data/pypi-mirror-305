...
class WebsocketHandler(BaseHandler):
    ...
    def sort_events() -> tuple[dict[str, Type[Event]], dict[Type[Event], str]]:
        return ({event.resource: event.event for event in api_events if ApiType.WS not in event.exclude},
                {event.event: event.resource for event in api_events if ApiType.WS not in event.exclude})
    ...
    async def parse_body(self) -> bool:
        data = await self.receive()
        if data["type"] == "websocket.receive":
            ...
            await self.response_error(1003, "Parse body failed")
            return False
        try:
            ...
            await self.response_error(1003, "Cannot process json data")
            return False
        except Exception as e:
            self.logger.error("Unknown error", exc_info=e)
            await self.response_error(1002, "Unknown error")
            return False
        return True
