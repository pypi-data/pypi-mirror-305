...
class JobEntry(Base):
    ...
    # Auditing data
    create_timestamp: Mapped[Optional[datetime.datetime]]
    queued_timestamp: Mapped[datetime.datetime] = mapped_column(index=True)
    queued_time_duration: Mapped[Optional[int]]
    worker_start_timestamp: Mapped[Optional[datetime.datetime]]
    worker_completed_timestamp: Mapped[Optional[datetime.datetime]]

    # Logstream identifiers
    stdout_stream_name: Mapped[Optional[str]]
    stdout_stream_write_name: Mapped[Optional[str]]
    stderr_stream_name: Mapped[Optional[str]]
    stderr_stream_write_name: Mapped[Optional[str]]
    ...
    __table_args__ = (
        Index(
            ...
            sqlite_where="worker_start_timestamp IS NOT null",
        ),
        Index(
            "ix_worker_completed_timestamp",
            "worker_completed_timestamp",
            unique=False,
            postgresql_where="worker_completed_timestamp IS NOT null",
            sqlite_where="worker_completed_timestamp IS NOT null",
        ),
        Index(
...
class LeaseEntry(Base):
    ...
    state: Mapped[int]
    worker_name: Mapped[Optional[str]] = mapped_column(index=True, default=None)
...
class RequestMetadataEntry(Base):
    ...
    tool_version: Mapped[Optional[str]]
    invocation_id: Mapped[Optional[str]]
    correlated_invocations_id: Mapped[Optional[str]]
    action_mnemonic: Mapped[Optional[str]]
    target_id: Mapped[Optional[str]]
    configuration_id: Mapped[Optional[str]]
...
class OperationEntry(Base):
    ...
    client_identity_id: Mapped[Optional[int]] = mapped_column(ForeignKey("client_identities.id"))
    client_identity: Mapped[Optional[ClientIdentityEntry]] = relationship("ClientIdentityEntry")

    request_metadata_id: Mapped[Optional[int]] = mapped_column(ForeignKey(RequestMetadataEntry.id))
    request_metadata: Mapped[Optional[RequestMetadataEntry]] = relationship(RequestMetadataEntry)
...
class IndexEntry(Base):
    ...
    inline_blob: Mapped[Optional[bytes]]
...
class BotEntry(Base):
    ...
    job: Mapped[Optional[JobEntry]] = relationship(JobEntry, primaryjoin="foreign(BotEntry.lease_id) == JobEntry.name")
