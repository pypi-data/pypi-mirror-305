...
def split_plan_into_stages(plan: NodeRef) -> List[List[Stage]]:
    ...
    def find_inputs(tree: NodeRef, output: StagingNode) -> Iterable[NodeRef]:
        if isinstance(tree.node, StagingNode) and tree.node != output and tree.node.dialect != output.dialect:
...
class QueryTreeExecutor:
    ...
    def exec_qt(self, logical_plan: NodeRef) -> QueryTreeOutput:
        ...
        if self.is_debug:
            ...
            logger.warning(f"QT: \n{logical_plan.pretty_str(columns=True)}")
            logger.warning("---------------------------------- Physical plan -----------------------------------------")
            logger.warning(f"QT: \n{physical_plan.pretty_str(columns=True)}")
    ...
    def _process_staging_nodes(
        ...
        for stage in stages:
            ...
            if self.is_debug:
                ...
                logger.warning(f"QT: \n{stage.output.pretty_str(columns=True)}")
    ...
    def _process_staging_node(
        ...
        reader = compute.run_sql(
            sql_string, return_dataframe=True, expected_output_schema=expected_output_schema, monitor=monitor
        )
