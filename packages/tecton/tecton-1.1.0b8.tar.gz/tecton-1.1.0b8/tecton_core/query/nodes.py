...
from tecton_core.query.calculation_sql_builder import build_calculation_sql_columns
...
class MultiRtfvCalculationNode(QueryNode):
    ...
    def _to_query(self) -> pypika.queries.QueryBuilder:
        ...
        calculation_sql_columns = build_calculation_sql_columns(
            self.feature_definition_namespaces, self.use_namespace_feature_prefix
        )
        return (
            self.func.query().with_(input_query, uid).from_(AliasedQuery(uid)).select(Star(), *calculation_sql_columns)
        )
...
class UserSpecifiedDataNode(QueryNode):
    ...
    def to_arrow_reader(self, context: ExecutionContext) -> "pyarrow.RecordBatchReader":
        ...
        if self.row_id_column:
            ...
            df[self.row_id_column] = range(df.shape[0])
        return cast(df, self.output_schema)
