...
class SFNPromptManager:
    @staticmethod
    def load_prompt(prompt_type: str, sfn_blueprint_prompt: bool = False, **kwargs) -> Tuple[str, str]:
        """
        ...
        """
        try:
            if sfn_blueprint_prompt:
                # Dynamically import the appropriate prompt module
                module = importlib.import_module(f'sfn_blueprint.agent_prompts.{prompt_type}_prompts')
            else:
                module = importlib.import_module(f'agent_prompts.{prompt_type}_prompts')
