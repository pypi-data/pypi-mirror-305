...
class Infrastructure:
    ...
    def to_nested_dict(self):
        return {
            k: ([n.__dict__ for n in self.nodes] if k == 'nodes' else ([n.to_nested_dict() for n in self.maintenances] if k == 'maintenances' else v))
    ...
    def get_next_maintenance(self) -> Optional[Maintenance]:
        if self.maintenances:
            if hasattr(self.maintenances[0], 'start_time'):  # if it is the master this is a dict
                return sorted(self.maintenances, key=lambda x: to_datetime(x.start_time))[0]
            return Maintenance(**sorted(self.maintenances, key=lambda x: to_datetime(x.get('start_time')))[0])
        return None
