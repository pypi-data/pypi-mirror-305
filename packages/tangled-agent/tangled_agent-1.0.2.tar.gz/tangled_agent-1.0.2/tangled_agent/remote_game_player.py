...
class RemoteGamePlayer(GamePlayerBase):
    ...
    def __init__(self, player: GameAgentBase, game_id: str, host_url: str, force_new_credentials: bool = False):
        ...
        player.id = self.get_user_id(self.access_token)
    ...
    def join_game(self):
        ...
        with ConnectionClient(self.host_url, self.access_token) as api_instance:
            try:
                # Join the game
                join_game_request = {"game_token": self.game_id, "player_id": self.player.id+".default", "player_index": 0}
