...
class PGENWriter:
    ...
    def write(self):
        ...
        if self.__file.endswith(file_extension):
            self.__file = self.__file[:-5]
    ...
    def write_pvar(self):
        ...
        """
        log.info(f"Writing to {self.__file}.pvar")
        df = pl.DataFrame(
            {
        ...
        df.write_csv(f"{self.__file}.pvar", separator="\t")

    def write_psam(self):
        """
        Writes sample metadata to the .psam file.
        """
        log.info(f"Writing {self.__file}.psam")
        df = pl.DataFrame(
            {
        ...
        df.write_csv(f"{self.__file}.psam", separator="\t")

    def write_pgen(self):
        """
        Writes the genotype data to a .pgen file.
        """
        log.info(f"Writing to {self.__file}.pgen")
        ...
        with pg.PgenWriter(
            f"{self.__file}.pgen".encode("utf-8"),
