...
def admixmap(argv: List):
    ...
    if 6 in mspObj.chromosome:
        print("Removing HLA windows...")
        for indx in np.where(mspObj.chromosome==6)[0]:
            if ( (25477797 <= mspObj.physical_pos[indx][0] <= 36448354)
                or (25477797 <= mspObj.physical_pos[indx][1] <= 36448354) ):
                print("From chm", mspObj.chromosome[indx], "Window", indx, "has been removed.")
                hla_windows.append(indx)
        mspObj.drop_window(hla_windows)
    else:
    ...
    for ind in range(len(mspObj.haplotype_IDs))[::2]:
        if mspObj.haplotype_IDs[ind] in pheObj.cases_haplotypes:
    ...
    for window in range(len(cases_mspObj.chromosome)):
        # This could also be retrieved from controls_mspObj
        chm, spos, epos = (
            cases_mspObj.chromosome[window],
