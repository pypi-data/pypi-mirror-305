...
class VCFWriter:
    ...
    def __init__(self, snpobj: SNPObject, file: str, n_jobs: int = -1, phased: bool = True):
        """
        Parameters
        ----------
    ...
    def write(self, chrom_partition: bool = False):
        ...
        if self.__file.endswith(file_extension):
            self.__file_extension = self.__file[-4:]
        ...
        if self.__chrom_partition:
            ...
            for chrom in chroms:
                ...
                data_chrom = data.filter_snps(chrom=chrom, inplace=False)
    ...
    def write_chromosome_data(self, chrom, data_chrom):
        ...
        # Format output file
        if chrom == "All":
            file = self.__file + self.__file_extension
        else:
            file = f'{self.__file}_{chrom}{self.__file_extension}'
