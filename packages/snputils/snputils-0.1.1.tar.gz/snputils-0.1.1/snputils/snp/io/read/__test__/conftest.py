...
def data_path():
    ...
    files_urls = {
        "plink2_linux_x86_64_20240105.zip": "https://s3.amazonaws.com/plink2-assets/alpha5/plink2_linux_x86_64_20240105.zip",
    ...
    for file_name, url in files_urls.items():
        ...
        if (
            ...
            if not subset_file.exists():
                ...
                first_5_sample_ids = ["HG00096", "HG00097", "HG00099", "HG00100"]
                with open(subset_file, "w") as file:
                    file.write("\n".join(first_5_sample_ids))
...
def snpobj_vcf(data_path):
    return VCFReader(data_path + "/subset.vcf").read()


@pytest.fixture(scope="module")
def snpobj_bed(data_path):
    return BEDReader(data_path + "/bed/subset").read()


@pytest.fixture(scope="module")
def snpobj_pgen(data_path):
    return PGENReader(data_path + "/pgen/subset").read()
