...
__all__ = [
    ...
    "filter_universe",
    "save_timerange_sql_as_universe",
    "save_timeseries_sql_as_universe"
]
...
@_validate_args
def save_timerange_sql_as_universe(
    universename: str,
    ...
    con : str or SQLAlchemy Connectable
        Database URI as a string or SQLAlchemy Connectable object.
    ...
    -------
    dict
        Status of the universe creation request.
    """

    should_overwrite, err_msg = should_overwrite_universe(universename, "custom")
    if not should_overwrite:
        return err_msg

    # Convert SQLAlchemy Selectable to a query string if necessary
    ...
    # Convert SQLAlchemy Engine/Connectable to a string URI if necessary
    if isinstance(con, Connectable):
        uri = con.engine.url.render_as_string(hide_password=False)
    elif isinstance(con, str):
        uri = con
    else:
        raise TypeError(f'Invalid query type {type(con)}')
    ...
    try:
        response = post(url=f"{URL_UNIVERSES}/sql/timerange", params=params, body=payload)
        return response
...
@_validate_args
def save_timeseries_sql_as_universe(
    universename: str,
    query: Union[str, Selectable],
    con: Union[str, Connectable],
    idcolumn: list,
    ...
    # Make the POST request using the `post` utility function
    try:
        response = post(url=f"{URL_UNIVERSES}/sql/timeseries", params=params, body=payload)
        return response
    except Exception as err:
        print(f"An error occurred: {err}")
        return {'status': 'Failure', 'message': str(err)}
