...
class MedicalDatasetCreator():
  ...
  def __init__(self, directory = 'MultiCaRe', master_dataset_path = '', version = 2):
    ...
    if 'subcaption_id' in self.full_image_metadata_df.columns:
     self.full_image_metadata_df.rename({'subcaption_id': 'file'}, axis = 1, inplace = True)
    if int(self.version) != 1:
      self.full_image_metadata_df['file'] = self.full_image_metadata_df['file'].str.replace('.jpg', '.webp')
    ...
    if int(self.version) != 1:
      ...
      col_n = 5
    for list_column in self.full_image_metadata_df.columns[col_n:]:
      self.full_image_metadata_df[list_column] = self.full_image_metadata_df[list_column].apply(lambda x: ast.literal_eval(x) if ((type(x) == str) and (len(x) != 0) and (x[0] == '[') and (x[-1] == ']')) else x)
    if int(self.version) != 1:
      self.full_image_metadata_df['case_substring'] = self.full_image_metadata_df['case_substring'].apply(lambda x: ast.literal_eval(x) if ((type(x) == str) and ('[' in x)) else x)
    ...
    if int(self.version) == 1:
      ...
      self.image_label_list = [l for l in self.full_image_metadata_df.postprocessed_label_list.explode().unique() if type(l) == str]
    print('Done!')
  ...
  def create_dataset(self, dataset_name, filter_list, dataset_type = 'multimodal', image_token = None, multiplex_classification = False):
    ...
    - multiplex_classification (bool): If False (default value), the created dataset includes only one column with labels. If True, the column with labels will be present too, and also the created dataset will have one column per classification task.
    ...
    self.multiplex_classification = multiplex_classification
    ...
    for dct in self.image_filter_list:
      ...
      if dct['field'] == 'caption':
        ...
        if logical_operator == 'any':
          self.filtered_image_metadata_df = self.filtered_image_metadata_df[self.filtered_image_metadata_df['postprocessed_label_list'].apply(lambda x: any(elem in x for elem in dct['string_list']))]

        elif logical_operator == 'none':
          self.filtered_image_metadata_df = self.filtered_image_metadata_df[~self.filtered_image_metadata_df['postprocessed_label_list'].apply(lambda x: any(elem in x for elem in dct['string_list']))]

        elif logical_operator == 'all':
          self.filtered_image_metadata_df = self.filtered_image_metadata_df[self.filtered_image_metadata_df['postprocessed_label_list'].apply(lambda x: all(elem in x for elem in dct['string_list']))]
    ...
    if self.dataset_type in ['image', 'multimodal']:
      ...
      if not self.multiplex_classification:
        multilabel_columns = ['image_id', 'file', 'file_path', 'main_image', 'main_image_link', 'patient_id', 'license', 'file_size', 'split_during_preprocessing', 'caption', 'postprocessed_label_list']
        self.filtered_image_metadata_df = self.filtered_image_metadata_df[multilabel_columns].copy()
      self.filtered_image_metadata_df.rename({'main_image': 'source_image', 'main_image_link': 'source_image_link', 'postprocessed_label_list': 'label_list', 'patient_id': 'case_id'}, axis = 1, inplace = True)
...
'''
  ...
  def create_image_classification_dataset(self, dataset_dict):
    ...
    for k in subset_dict.keys():
      self.create_dataset(dataset_name = f"{dataset_dict['dataset_name']}_{k}", filter_list = subset_dict[k], dataset_type = 'image', image_token = None, multiplex_classification = False)

    dfs = []
    for k in subset_dict.keys():
      df = pd.read_json(f"{self.directory}/{dataset_dict['dataset_name']}_{k}/image_metadata.json", orient = 'records', lines = True)
      df['class'] = k
      df = df[['file_path', 'file', 'class']]
      dfs.append(df)
