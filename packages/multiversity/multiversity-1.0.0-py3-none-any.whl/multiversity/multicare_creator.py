...
class MulticareCreator():
  ...
  def __init__(self, email, annotated_ngram_path, input_owl_path, api_key = '', outcome_directory = '/content/drive/MyDrive/MultiCaRe_dataset', delete_previous_data = False,
    ...
    annotated_ngram_path (str): Path to the csv file containing the annotated ngrams.
    input_owl_path (str): Path to the input taxonomy file with a Decision Rainforest format (used for Multiplex classification).
    api_key (str, '' by default): API key from NCBI (available at https://www.ncbi.nlm.nih.gov/account/settings/).
    outcome_directory (str, '/content/drive/MyDrive/MultiCaRe_dataset' by default): Path to the folder where that will contain the dataset.
  ...
  def _create_caption_df(self):
    ...
    self.mdp.dataset['image_id'] = self.mdp.dataset.reset_index()['index'].apply(lambda x: f"image_{('0000000' + str(x))[-7:]}")
    ...
    # Sorting the columns
    cols = []
    new_column_order = ['image_id', 'file', 'file_path', 'main_image', 'image_component', 'patient_id', 'license', 'file_size', 'main_image_link',
  ...
  def update_dataset(self, previous_version_directory = ''):
    ...
    self.start_year = sorted(self.mdc.year_list)[-1]
