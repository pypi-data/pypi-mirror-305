...
def propagation(steps):
        ...
        for i in range(2,steps):
            ...
            for nb in range(1, nblocks+1):
                lattvacfx[nb, i]= xp.sum(x_arrays[latvacf_op.name][xmin[nb]+1:xmax[nb]+1-1,ymin[nb]+1:ymax[nb]+1-1,zmin[nb]+1:zmax[nb]+1-1,0])
                lattvacfy[nb, i]= xp.sum(x_arrays[latvacf_op.name][xmin[nb]+1:xmax[nb]+1-1,ymin[nb]+1:ymax[nb]+1-1,zmin[nb]+1:zmax[nb]+1-1,1])
                lattvacfz[nb, i]= xp.sum(x_arrays[latvacf_op.name][xmin[nb]+1:xmax[nb]+1-1,ymin[nb]+1:ymax[nb]+1-1,zmin[nb]+1:zmax[nb]+1-1,2])
                lattvacf[nb, i] = lattvacfx[nb, i]+lattvacfy[nb, i]+lattvacfz[nb, i]

                denstot[nb, i]=xp.sum(x_arrays[c.name][xmin[nb]+1:xmax[nb]+1-1,ymin[nb]+1:ymax[nb]+1-1,zmin[nb]+1:zmax[nb]+1-1])
                
                ReGtot[nb, i]=xp.sum(x_arrays[ReGs.name][xmin[nb]+1:xmax[nb]+1-1,ymin[nb]+1:ymax[nb]+1-1,zmin[nb]+1:zmax[nb]+1-1])/denstot[nb, i]
                ImGtot[nb, i]=xp.sum(x_arrays[ImGs.name][xmin[nb]+1:xmax[nb]+1-1,ymin[nb]+1:ymax[nb]+1-1,zmin[nb]+1:zmax[nb]+1-1])/denstot[nb, i]

                xvacfx[nb]=xp.sum(x_arrays[latvacf_op.name][xmin[nb]+1:xmax[nb]+1-1,ymin[nb]+1:ymax[nb]+1-1,zmin[nb]+1:zmax[nb]+1-1,0], axis=(1,2))
                xvacfy[nb]=xp.sum(x_arrays[latvacf_op.name][xmin[nb]+1:xmax[nb]+1-1,ymin[nb]+1:ymax[nb]+1-1,zmin[nb]+1:zmax[nb]+1-1,1], axis=(1,2))
                xvacfz[nb]=xp.sum(x_arrays[latvacf_op.name][xmin[nb]+1:xmax[nb]+1-1,ymin[nb]+1:ymax[nb]+1-1,zmin[nb]+1:zmax[nb]+1-1,2], axis=(1,2))
                
                xvacf[nb]= xvacfx[nb] + xvacfy[nb] + xvacfz[nb]
                
                yvacfx[nb]=xp.sum(x_arrays[latvacf_op.name][xmin[nb]+1:xmax[nb]+1-1,ymin[nb]+1:ymax[nb]+1-1,zmin[nb]+1:zmax[nb]+1-1,0], axis=(0,2))
                yvacfy[nb]=xp.sum(x_arrays[latvacf_op.name][xmin[nb]+1:xmax[nb]+1-1,ymin[nb]+1:ymax[nb]+1-1,zmin[nb]+1:zmax[nb]+1-1,1], axis=(0,2))
                yvacfz[nb]=xp.sum(x_arrays[latvacf_op.name][xmin[nb]+1:xmax[nb]+1-1,ymin[nb]+1:ymax[nb]+1-1,zmin[nb]+1:zmax[nb]+1-1,2], axis=(0,2))
                
                yvacf[nb] = yvacfx[nb] + yvacfy[nb] + yvacfz[nb]
                
                zvacfx[nb]=xp.sum(x_arrays[latvacf_op.name][xmin[nb]+1:xmax[nb]+1-1,ymin[nb]+1:ymax[nb]+1-1,zmin[nb]+1:zmax[nb]+1-1,0], axis=(0,1))
                zvacfy[nb]=xp.sum(x_arrays[latvacf_op.name][xmin[nb]+1:xmax[nb]+1-1,ymin[nb]+1:ymax[nb]+1-1,zmin[nb]+1:zmax[nb]+1-1,1], axis=(0,1))
                zvacfz[nb]=xp.sum(x_arrays[latvacf_op.name][xmin[nb]+1:xmax[nb]+1-1,ymin[nb]+1:ymax[nb]+1-1,zmin[nb]+1:zmax[nb]+1-1,2], axis=(0,1))
...
def main():
    ...
    # Ajouter un argument pour le fichier d'entrée
    parser.add_argument(
        "-i", "--input_file",
        type=str,
        required=True,
    ...
    # Utiliser l'argument du fichier d'entrée
    ...
    for nb in range(1, nblocks + 1):
        x_diff = xmax[nb] - xmin[nb] - 2
        y_diff = ymax[nb] - ymin[nb] - 2
        z_diff = zmax[nb] - zmin[nb] - 2 
        xmax[nb]=int(xmax[nb])
        xmin[nb]=int(xmin[nb])
        ymax[nb]=int(ymax[nb])
        ymin[nb]=int(ymin[nb])
    ...
    if electrode_nature=='particles':
        with open("particles_file", "r") as f:
            if block_names[nb]=='electrode':
                with open("fichier_modifie.xyz", "r") as f:
                    for line in f:
    ...
    for nb in range(1, nblocks+1):
        xvacfx[nb] = xp.sum(x_arrays[latvacf.name][xmin[nb]+1:xmax[nb]+1-1,ymin[nb]+1:ymax[nb]+1-1,zmin[nb]+1:zmax[nb]+1-1,0], axis=(1, 2))
        yvacfx[nb] = xp.sum(x_arrays[latvacf.name][xmin[nb]+1:xmax[nb]+1-1,ymin[nb]+1:ymax[nb]+1-1,zmin[nb]+1:zmax[nb]+1-1,0], axis=(0, 2))
        zvacfx[nb] = xp.sum(x_arrays[latvacf.name][xmin[nb]+1:xmax[nb]+1-1,ymin[nb]+1:ymax[nb]+1-1,zmin[nb]+1:zmax[nb]+1-1,0], axis=(0, 1))
        xvacfy[nb] = xp.sum(x_arrays[latvacf.name][xmin[nb]+1:xmax[nb]+1-1,ymin[nb]+1:ymax[nb]+1-1,zmin[nb]+1:zmax[nb]+1-1,1], axis=(1, 2))
        yvacfy[nb] = xp.sum(x_arrays[latvacf.name][xmin[nb]+1:xmax[nb]+1-1,ymin[nb]+1:ymax[nb]+1-1,zmin[nb]+1:zmax[nb]+1-1,1], axis=(0, 2))
        zvacfy[nb] = xp.sum(x_arrays[latvacf.name][xmin[nb]+1:xmax[nb]+1-1,ymin[nb]+1:ymax[nb]+1-1,zmin[nb]+1:zmax[nb]+1-1,1], axis=(0, 1))
        xvacfz[nb] = xp.sum(x_arrays[latvacf.name][xmin[nb]+1:xmax[nb]+1-1,ymin[nb]+1:ymax[nb]+1-1,zmin[nb]+1:zmax[nb]+1-1,2], axis=(1, 2))
        yvacfz[nb] = xp.sum(x_arrays[latvacf.name][xmin[nb]+1:xmax[nb]+1-1,ymin[nb]+1:ymax[nb]+1-1,zmin[nb]+1:zmax[nb]+1-1,2], axis=(0, 2))
        zvacfz[nb] = xp.sum(x_arrays[latvacf.name][xmin[nb]+1:xmax[nb]+1-1,ymin[nb]+1:ymax[nb]+1-1,zmin[nb]+1:zmax[nb]+1-1,2], axis=(0, 1))
        ...
        denstot[nb, 0] = xp.sum(x_arrays[c.name][xmin[nb]+1:xmax[nb]+1-1,ymin[nb]+1:ymax[nb]+1-1,zmin[nb]+1:zmax[nb]+1-1])
        lattvacfx[nb, 0]+= xp.sum(x_arrays[latvacf.name][xmin[nb]+1:xmax[nb]+1-1,ymin[nb]+1:ymax[nb]+1-1,zmin[nb]+1:zmax[nb]+1-1,0])
        lattvacfy[nb, 0]+= xp.sum(x_arrays[latvacf.name][xmin[nb]+1:xmax[nb]+1-1,ymin[nb]+1:ymax[nb]+1-1,zmin[nb]+1:zmax[nb]+1-1,1])
        lattvacfz[nb, 0]+= xp.sum(x_arrays[latvacf.name][xmin[nb]+1:xmax[nb]+1-1,ymin[nb]+1:ymax[nb]+1-1,zmin[nb]+1:zmax[nb]+1-1,2])
    ...
    for nb in range(1, nblocks+1):
        lattvacfx[nb, 1]= xp.sum(x_arrays[latvacf_op.name][xmin[nb]+1:xmax[nb]+1-1,ymin[nb]+1:ymax[nb]+1-1,zmin[nb]+1:zmax[nb]+1-1,0])
        lattvacfy[nb, 1]= xp.sum(x_arrays[latvacf_op.name][xmin[nb]+1:xmax[nb]+1-1,ymin[nb]+1:ymax[nb]+1-1,zmin[nb]+1:zmax[nb]+1-1,1])
        lattvacfz[nb, 1]= xp.sum(x_arrays[latvacf_op.name][xmin[nb]+1:xmax[nb]+1-1,ymin[nb]+1:ymax[nb]+1-1,zmin[nb]+1:zmax[nb]+1-1,2])
        ...
        zvacfx[nb]=xp.sum(x_arrays[latvacf_op.name][xmin[nb]+1:xmax[nb]+1-1,ymin[nb]+1:ymax[nb]+1-1,zmin[nb]+1:zmax[nb]+1-1,0], axis=(0,1))
        zvacfy[nb]=xp.sum(x_arrays[latvacf_op.name][xmin[nb]+1:xmax[nb]+1-1,ymin[nb]+1:ymax[nb]+1-1,zmin[nb]+1:zmax[nb]+1-1,0], axis=(0,1))
        zvacfz[nb]=xp.sum(x_arrays[latvacf_op.name][xmin[nb]+1:xmax[nb]+1-1,ymin[nb]+1:ymax[nb]+1-1,zmin[nb]+1:zmax[nb]+1-1,0], axis=(0,1))

        zvacf[nb] = zvacfx[nb] + zvacfy[nb] + zvacfz[nb]

        denstot[nb, 1]=xp.sum(x_arrays[c.name][xmin[nb]+1:xmax[nb]+1-1,ymin[nb]+1:ymax[nb]+1-1,zmin[nb]+1:zmax[nb]+1-1])
        ReGtot[nb, 1]=xp.sum(x_arrays[ReGs.name][xmin[nb]+1:xmax[nb]+1-1,ymin[nb]+1:ymax[nb]+1-1,zmin[nb]+1:zmax[nb]+1-1])/denstot[nb, 1]
        ImGtot[nb, 1]=xp.sum(x_arrays[ImGs.name][xmin[nb]+1:xmax[nb]+1-1,ymin[nb]+1:ymax[nb]+1-1,zmin[nb]+1:zmax[nb]+1-1])/denstot[nb, 1]
