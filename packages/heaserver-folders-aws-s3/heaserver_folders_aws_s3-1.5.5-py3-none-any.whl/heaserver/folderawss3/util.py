...
async def response_folder_as_zip(s3_client: S3Client, request: web.Request, bucket_name: str, folder_key: str) -> web.StreamResponse:
    ...
    class FixedSizeBuffer:
        ...
        def read(self, size: int = -1) -> bytes:
            with self.condition:
                while self.length == 0:
                    if self.eof:
                        logger.debug('Reading empty bytes')
                        return b''
                ...
                self.length -= size
                self.condition.notify_all()  # Notify any waiting threads
                logger.debug('Read %s bytes; length %s', len(result), self.length)
                return result
    ...
    try:
        ...
        from mypy_boto3_s3.type_defs import ObjectTypeDef, CommonPrefixTypeDef
        ...
        try:
            while True:
                ...
                if not data:
                    logger.debug('Done writing data to response')
                    break
