...
def shallow_merge_dicts(*data_dicts, **kwargs):
    ...
    :param kwargs: keyword arguments
     arg: skip_none_values: default True, whether to skip None values
     arg: add_lists: default False, whether lists should be combined/added together if both entries for merge are lists
...
def get_data(sub_dict, cwd=None, windows_pseudo_links=False):
    """
    ...
    if 'source' in data:  # then use external data
        data = get_data(*read_yaml(data['source'], cwd, return_path=True, windows_pseudo_links=windows_pseudo_links))
    elif 'include' in data:  # merge external data
        includes = data.pop('include')
        if isinstance(includes, string_types):
            includes = [includes]
        data = shallow_merge_dicts(*([get_data(*read_yaml(include, cwd, return_path=True, windows_pseudo_links=windows_pseudo_links))
...
def vpd_get(vpd, key, default=None, do_substitute=False, ff=None):
    ...
    :param do_substitute: whether to use vpd-inherent calls for argument substitution
