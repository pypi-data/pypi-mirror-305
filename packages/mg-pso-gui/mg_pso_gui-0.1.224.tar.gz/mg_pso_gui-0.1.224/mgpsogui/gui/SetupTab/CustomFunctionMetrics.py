...
class CustomFunctionsMetricsView(CTkScrollableFrame):
    ...
    def render(self):
        ...
        for key_value_pair in self.key_values:
            CTkOptionMenu(self.containerFrame, values=self.columns, variable=self.key_values[index]["name"], command=self.refresh).grid(row=row, column=1, padx=(5, 5), pady=(5, 5), sticky="ew")
