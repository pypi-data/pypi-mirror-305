...
class AggregateNode:
    ...
    def execute(self, node_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        access_token = form_data.get('accessToken') or os.getenv('ACCESS_TOKEN')
        ...
        try:
            if aggregate_method == 'Individual Fields':
                result = self.aggregate_individual_fields(form_data, node_data)
            elif aggregate_method == 'All Item Data (Into a Single List)':
                result = self.aggregate_all_item_data(form_data, node_data)
            else:
...
AggregateNodeNode = AggregateNode
