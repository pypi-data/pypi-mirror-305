...
def main(config):
    ...
    model = CLIPModel.from_pretrained(config['eval']['model_name'])
    processor = CLIPProcessor.from_pretrained(config['eval']['model_name'])

    # Set the device
    device = config['eval']['device']

    # Test images
    test_images_path = config['eval']['test_images_path']
    ...
    with mlflow.start_run(run_name=config['run_name']):
        ...
        mrr, eval_file_path = calculate_mrr(
            test_images,
            test_images_path,
            config['eval']['vector_db_path'],
            config['eval']['metadata_path'],
            processor,
            model,
            config['eval']['output_eval_file'],
        ...
        mlflow.log_artifact(config['eval']['vector_db_path'], artifact_path="evaluation")
        mlflow.log_artifact(config['eval']['metadata_path'], artifact_path="evaluation")
