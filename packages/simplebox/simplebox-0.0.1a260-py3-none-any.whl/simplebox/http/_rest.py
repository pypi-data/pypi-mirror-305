...
class RestContext(BaseContext):
    ...
    def put_all(self, **kwargs: dict[str, BaseRest]):
        self.__context.update(kwargs)

    def put(self, tag, rest: BaseRest):
        self.__context[tag] = rest
...
class RestWrapper:
    ...
    def __init__(self, flag=None, rest: BaseRest = None, file: str = None, server_list: list = None,
    ...
    def retry(self, times: int = None, interval: int = None, exit_code_range: list = None, exception_retry: bool = None,
        ...
        def __inner(func):
            ...
            def __wrapper(*args, **kwargs):
                times_ = times if isinstance(times, int) else self.__rest.retry_times
                interval_ = interval if isinstance(interval, int) else self.__rest.retry_interval
                exit_code_range_ = exit_code_range if isinstance(exit_code_range,
                                                                 list) else self.__rest.retry_exit_code_range
                ObjectsUtils.check_iter_type(exit_code_range_, int)
                exception_retry_ = exception_retry if isinstance(exception_retry,
                                                                 bool) else self.__rest.retry_exception_retry
                check_handler_ = check_handler if callable(check_handler) else self.__rest.retry_check_handler
        ...
        def __inner(func):
            ...
            def __wrapper(*args, **kwargs):
                self.__request(func=func, args=args, kwargs=kwargs, api_name=api_name, server_name=server_name,
                               ...
                               stats=stats, hooks=hooks, show_len=show_len, opts=kwargs.pop(_Constant.OPTS, RestOptions()))
    ...
    def __request(self, func, args, **kw):
        ...
        if inspect.ismethod(func):
            rest = func.__self__.context.get(self.__flag)
        elif len(args) > 0:
            if isinstance(args[0], ApiAware):
                rest = args[0].context.get(self.__flag)
