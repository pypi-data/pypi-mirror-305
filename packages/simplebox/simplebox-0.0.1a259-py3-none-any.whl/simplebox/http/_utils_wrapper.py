...
from ..exceptions import HttpException, LengthException
...
class _AstTools:
    ...
    def __visit_hock(self, node):
        for n in node.decorator_list:
            ...
            attr = ""
            if isinstance(n.func, Attribute):
                attr = n.func.value.id
...
def __bind(source, method, attr_name, attrs_map, decorators):
    if method:
        if not decorators:
            return
        source_name = source.__class__.__name__
        if len(decorators) > 1:
            raise LengthException(f"{source_name}.{attr_name} found multi decorators, must only one.")
        for decorator in decorators:
            for decorator_name, params in decorator.items():
                attr_key = params['instance']
                wrapper = attrs_map.get(attr_key) or attrs_map.get(f'{source_name}{attr_key}')
                if not wrapper:
                    continue
                method = method.__wrapped__ if hasattr(method, "__wrapped__") else method
                method = types.MethodType(method, source)
                if hasattr(wrapper, decorator_name):
                    setattr(source, attr_name,
                            getattr(wrapper, decorator_name)(*params['args'], **params['kwargs'])(method))
                else:
                    setattr(source, attr_name, method)


@_call_stack_check("register")
def dynamic_binding(source):
    ...
    attrs_map = source.__dict__
    all_decorators = _AstTools(source.__class__).get_decorators()
    for attr in attrs:
        method = methods.get(attr, None)
        __bind(source, method, attr, attrs_map, all_decorators.get(attr))


@_call_stack_check("manual_register")
def manual_binding(source, func, *decorators):
    for decorator in decorators[::-1]:
        func = decorator(func)
    name = func.__name__
    if name.startswith("__") and not name.endswith("__"):
        name = f"_{source.__class__.__name__}{name}"
    setattr(source, name, func)
