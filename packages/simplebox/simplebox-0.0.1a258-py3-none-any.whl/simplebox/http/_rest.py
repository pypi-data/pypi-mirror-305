...
from ._utils_wrapper import dynamic_binding, manual_binding
...
class BaseRest(metaclass=ABCMeta):
    ...
    def register(instance):
        ...
        **************************NOTE*****************************
        Both register and manual_register are designed to solve the problem that the decorator of RestWrapper cannot
        modify objects at runtime. The difference is that register only allows methods to use one decorator,
        manual_register allows multiple (assembly is more cumbersome), and it is recommended to use register
        **************************NOTE*****************************
    ...
    @staticmethod
    @abstractmethod
    def manual_register(instance, func, *decorators):
        """
        Unlike auto-registration, manual registration can use multiple decorators,
        but auto-registration only supports one decorator.
        ...
        **************************NOTE*****************************
        ...
        class Api:
            ...
            def __init__(self):
                self.rest1 = Rest()
                self.rest2 = Rest()
                manual_binding(self, self.__get2, self.rest1.request(1), self.rest2.request(2))
...
class RestWrapper(BaseRest):
    ...
    @staticmethod
    def register(instance):
        dynamic_binding(instance)

    @staticmethod
    def manual_register(instance, func, *decorators):
        manual_binding(instance, func, *decorators)
