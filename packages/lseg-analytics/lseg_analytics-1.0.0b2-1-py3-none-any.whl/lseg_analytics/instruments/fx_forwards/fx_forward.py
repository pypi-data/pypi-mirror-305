...
class FxForward:
    ...
    >>>         end_date=Date(date_moving_convention=DateMovingConvention.NEXT_BUSINESS_DAY),
    >>>     )
    >>> )
            ...
            if self._id:
                ...
                response = check_and_raise(
                    Client().fx_forward_resource.price(
                        body=FxForwardPriceRequest(parameters=parameters, market_data=market_data),
                        instrument_id=self._id,
                    )
                )
            else:
                ...
                response = check_and_raise(
                    Client().fx_forwards_resource.price(
                        body=FxForwardOnTheFlyPriceRequest(
                            definition=self.definition,
            ...
            if self._id:
                ...
                response = check_and_raise(
                    Client().fx_forward_resource.value(
                        body=FxForwardValuationRequest(parameters=parameters, market_data=market_data),
                        instrument_id=self._id,
                    )
                )
            else:
                ...
                response = check_and_raise(
                    Client().fx_forwards_resource.value(
                        body=FxForwardOnTheFlyValuationRequest(
                            definition=self.definition,
                            parameters=parameters,
                            market_data=market_data,
                        )
