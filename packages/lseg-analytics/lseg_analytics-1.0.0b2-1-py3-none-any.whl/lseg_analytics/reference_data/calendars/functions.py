...
def load(
    *,
    ...
    <lseg_analytics.reference_data.calendars.calendar.Calendar at 0x7f0198e3a450>

    Load by name and space.

    >>> load(name="EMU", space="LSEG")
    <lseg_analytics.reference_data.calendars.calendar.Calendar at 0x7f0198eb70d0>
    ...
    if not result:
        ...
        raise ResourceNotFound("Calendar", f"name={name} space={space}")
...
def compute_dates(
    *,
    ...
    calendars : List[str]
        An array of calendar reference strings for which the calculation should be done. Each string being composed of the
        space and name of a calendar. For example 'LSEG.UKG' is the string referencing the 'UKG' calendar stored in the 'LSEG' space.
    ...
    try:
        ...
        response = check_and_raise(
            Client().calendars_resource.compute_dates(
                body=ComputeDatesRequestCollection(
                    tenors=tenors,
    ...
    try:
        ...
        response = check_and_raise(
            Client().calendars_resource.count_periods(
                body=CountPeriodsRequestCollection(
                    start_date=start_date,
                    end_date=end_date,
    ...
    try:
        ...
        response = check_and_raise(
            Client().calendars_resource.generate_date_schedule(
                body=GenerateDateScheduleRequestCollection(
                    frequency=frequency,
                    start_date=start_date,
                    end_date=end_date,
    ...
    try:
        ...
        response = check_and_raise(
            Client().calendars_resource.generate_holidays(
                body=GenerateHolidaysRequestCollection(start_date=start_date, end_date=end_date, calendars=calendars)
            )
        )
