...
class TraderBackTesterUtils:
    ...
    def exp_cal(self, row:Any, x_org:pd.DataFrame, dat:str = 'date', nxt_exp:Any = 0, montly:Any = False) -> str:
        ...
        try:
            ...
            if (x_org.ticker.iloc[0] == 'BANKNIFTY') and (date >= '2024-03-01'):
                exp_day = 2
            ...
            if not monthly:
                ...
                while str(x) not in x_org.date.to_list():
                    if self.date_time.year == pd.to_datetime(x).date().year and not self.is_holiday(pd.to_datetime(x).date()): break
