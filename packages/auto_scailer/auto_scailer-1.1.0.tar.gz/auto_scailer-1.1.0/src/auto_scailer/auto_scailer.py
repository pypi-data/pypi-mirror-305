from auto_scailer.utils import get_limit, get_cpu_use, send_line_noti, get_log_path
import time
import os
...
def auto_scailer():
    ...
    while scale_cnt:
        ...
        with open(f"{usage_log_path}/usage.log", "a") as f:
            data = {"cpu_usage(%)":cu, "time":now(), "scale_cnt":scale_cnt, "cpu_use_status": "high" if cu >scale_out_value else "low" if cu <scale_in_value else "stable"}
            f.write(str(data))
            f.write("\n")
