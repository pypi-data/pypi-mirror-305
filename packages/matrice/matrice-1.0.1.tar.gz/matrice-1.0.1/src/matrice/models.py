...
class Model:
    ...
    def refresh(self):
        ...
        init_params = {
            ...
            'model_name': self.model_train_name,
        }
    ...
    def _get_model_train_by_name(self):
        ...
        >>>     print(f"Model data: {model_data}")
        """
        if self.model_train_name == "":
            print(
                "Model name not set for this Model train. Cannot perform the operation for Model without model name"
            )
            sys.exit(0)
        path = f"/v1/model/model_train/get_model_train_by_name?modelTrainName={self.model_train_name}"
        resp = self.rpc.get(path=path)
