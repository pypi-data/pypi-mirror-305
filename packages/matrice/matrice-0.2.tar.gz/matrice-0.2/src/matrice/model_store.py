...
def list_public_model_families(session, project_type="classification", page_size=10, page_num=0):
    ...
    Example
    -------
    >>> resp, error, message = model_store.list_public_model_families("classification")
    >>> if error:
    >>>     print(f"Error: {error}")
    >>> else:
...
def list_private_model_families(session, project_id=None, project_name=None, page_size=10, page_num=0): #TODO: api call
    """
    ...
    Example
    -------
    >>> resp, error, message = model_store.list_private_model_families("66912342583678074789d")
    >>> if error:
    >>>     print(f"Error: {error}")
    >>> else:
...
def list_public_model_archs(session, project_type="classification", page_size=10, page_num=0): #TODO: api call
    """
    ...
    Example
    -------
    >>> resp, error, message = model_store.list_public_model_archs("classification")
    >>> if error:
    >>>     print(f"Error: {error}")
    >>> else:
...
def list_private_model_archs(session, project_id=None, project_name=None, page_size=10, page_num=0): #TODO: api call
    """
    ...
    Example
    -------
    >>> resp, error, message = model_store.list_private_model_archs("66912342583678074789d")
    >>> if error:
    >>>     print(f"Error: {error}")
    >>> else:
...
def _get_all_models(session, project_id=None, project_name=None, project_type="classification"): #TODO: api call name
    """
...
def _get_all_model_families(session , project_id, project_name=None, project_type="classification"): #TODO: api call name
    """
...
def byom_status_summary(session, project_id, project_name): #TODO: api call name
    """
    ...
    Example
    -------
    >>> resp, error, message = model_store.byom_status_summary("66912342583678074789d")
    >>> if error:
    >>>     print(f"Error: {error}")
    >>> else:
...
def fetch_supported_runtimes_metrics(session, project_id, model_inputs, model_outputs):
    ...
    Example
    -------
    >>> resp, error, message = model_store.fetch_supported_runtimes_metrics(["image"], ["classification"])
    >>> if error:
    >>>     print(f"Error: {error}")
    >>> else:
...
def get_automl_config(session , project_id , experiment_id , model_count , recommended_runtime , performance_tradeoff , tuning_type="auto"):
    ...
    -------
    >>> session = Session()
    >>> model_archs, configs, counts = ModelArch.get_automl_config(
    ...     session=session,
...
class ModelArch:
    ...
    def get_train_config(self, experiment_id , tuning_type="default", model_checkpoint="auto"):
        ...
        #E = Experiment(self.session , experiment_name=experiment_name)
...
class ModelFamily: #TODO: api call names all
    ...
    >>> model_store = ModelFamily(session)
    """
    ...
    def __init__(self, session , model_family_id=None, model_family_name = None):
        ...
        resp , error, message = self.__get_model_family() #TODO: api call chceck response
        if error:
            print(f"Error: {error}")
            return
    ...
    def get_model_archs(self, model_name=None, model_key=None):
        ...
        Example
        -------
        >>> resp, error, message = model_store.get_model_info(model_name="ResNet-18", model_key="66912342583678074789d")
        >>> if error:
        >>>     print(f"Error: {error}")
        >>> else:
        >>>     print(f"Model info: {resp}")
...
class BYOM:
    ...
    #TODO: download the code, donot print the presigned URL
