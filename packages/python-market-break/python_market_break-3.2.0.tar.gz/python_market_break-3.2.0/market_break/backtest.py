    ...
    def limit(
        ...
        for i, row in trades.iterrows():
            ...
            for end in range(start + 1, row[EXIT]):
                ...
                if row[TYPE] == LONG:
                    take_profit = take and (bid[end] / ask[start] > current_take)
                    stop_loss = stop and (bid[end] / ask[start] < current_stop)

                else:
                    take_profit = take and (bid[start] / ask[end] > current_take)
                    stop_loss = stop and (bid[start] / ask[end] < current_stop)
